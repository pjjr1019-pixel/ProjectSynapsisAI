...
const escapeSingleQuoted = (value) => value.replace(/'/g, "''");
const buildQueryJournalScript = (rootPath) => `
...
`;
const buildReadJournalScript = (rootPath, startUsn) => `
$volume = '${escapeSingleQuoted(rootPath)}'
...
`;
const findPreviousCursor = (volume, previousCursors) => previousCursors.find((cursor) => cursor.volumeId === volume.id || cursor.rootPath === volume.rootPath) ?? null;
const findPreviousMonitorVolume = (volume, previousMonitor) => previousMonitor?.volumes.find((entry) => entry.volumeId === volume.id || entry.rootPath === volume.rootPath) ?? null;
const isAccessDeniedError = (value) => typeof value === "string" && /access is denied/i.test(value);
export const refreshVolumeJournalState = async (options, executor = runPowerShellJson) => {
    const observedAt = options.observedAt ?? new Date();
    const observedIso = observedAt.toISOString();
    const journalCursors = [];
    const monitorVolumes = [];
    for (const volume of options.volumes) {
        ...
        const journalInfo = await executor(buildQueryJournalScript(volume.rootPath), {
        ...
        const readResult = needsDeltaRead
            ? await executor(buildReadJournalScript(volume.rootPath, startCursor), {
                timeoutMs: USN_TIMEOUT_MS
            })
            : null;
        const journalReadable = !needsDeltaRead || Boolean(readResult?.ok);
        const nextCursor = readResult?.nextCursor ??
            journalInfo.nextUsn ??
            previousCursor?.cursor ??
            null;
        const lastSeenChangeAt = (readResult?.recordCount ?? 0) > 0
            ? readResult?.lastTimestamp ?? observedIso
            : previousMonitorVolume?.lastSeenChangeAt ?? null;
        const lastError = readResult && !readResult.ok
