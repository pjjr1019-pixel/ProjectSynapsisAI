...
const toActionProposals = (value: unknown): CapabilityActionProposal[] => {
...
const parseJsonObject = (raw: string): Record<string, unknown> | null => {
  ...
  try {
    const recovered = JSON.parse(candidate.slice(firstBrace, lastBrace + 1)) as unknown;
    return typeof recovered === "object" && recovered !== null ? (recovered as Record<string, unknown>) : null;
  } catch {
    return null;
  }
};

export const buildLocalAiEvalSystemPrompt = (card: CapabilityTestCard): string =>
  [
...
export const parseLocalAiEvalOutput = (raw: string): LocalAiEvalOutput => {
  ...
  const answerRecord =
    typeof parsed.answer_or_action === "object" && parsed.answer_or_action !== null
      ? (parsed.answer_or_action as Record<string, unknown>)
      : {};
  ...
  return {
    interpreted_task:
      typeof parsed.interpreted_task === "string" ? parsed.interpreted_task.trim() : "Task interpretation missing.",
