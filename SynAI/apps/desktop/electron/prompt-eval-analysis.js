...
const defaultCheckCategory = (check) => check.category ?? (check.kind === "bullet-count" || check.kind === "sentence-count" || check.kind === "line-prefixes" ? "format" : "content");
...
const cloneGroundingExpectations = (expectations) => expectations
    ...
    : undefined;
export const normalizePromptEvaluationCases = (cases) => cases
    .map((entry, index) => ({
...
export const buildPromptEvaluationRoutingReport = (diagnostics) => ({
...
export const countPromptEvaluationAssertions = (entry) => {
    const routingExpectations = entry.routingExpectations ?? {};
    const classifierExpectationCount = Object.values(routingExpectations.classifierCategories ?? {}).filter((value) => value !== undefined).length;
    const routingExpectationCount = Object.entries(routingExpectations).filter(([key, value]) => key !== "classifierCategories" && value !== undefined).length + classifierExpectationCount;
...
const buildSkippedCheckResults = (entry) => {
    ...
    for (const [category, expected] of Object.entries(routingExpectations.classifierCategories ?? {})) {
...
const evaluateClassifierCategoryExpectations = (entry, routing) => Object.entries(entry.routingExpectations?.classifierCategories ?? {})
...
export const evaluatePromptEvaluationCase = (entry, reply, routing, status, groundingSummary) => {
...
export const buildPromptEvaluationComparison = (current, previous) => {
    if (!previous) {
        return null;
    }
    ...
    return {
        ...
        summaryDelta: {
            ...
            qualityNeedsReviewCount: current.summary.qualityNeedsReviewCount - previous.summary.qualityNeedsReviewCount
        },
        cases: current.cases.map((entry) => {
            const previousEntry = previousCases.get(entry.id) ?? null;
            const qualityChange = previousEntry === null
                ? "new"
            ...
            return {
                ...
                routeChanged: previousEntry !== null && previousEntry.routing.routeFamily !== entry.routing.routeFamily,
