...
let ensureQueue: Promise<void> | null = null;
...
const hydrateDatabase = (parsed: Partial<SynAIDatabase>): SynAIDatabase => ({
...
const getCachedDatabase = async (): Promise<SynAIDatabase> => {
  if (cache) {
    return cache;
  }
...
export const loadDatabase = async (): Promise<SynAIDatabase> => {
  return cloneDatabase(await getCachedDatabase());
};

export const readDatabaseValue = async <T>(selector: (db: SynAIDatabase) => T): Promise<T> =>
  cloneValue(selector(await getCachedDatabase()));
