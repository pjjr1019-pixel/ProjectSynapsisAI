...
const governanceRuntimeRoot = (workspaceRoot: string): string =>
  path.join(workspaceRoot, ".runtime", "governance");

const officialKnowledgeRuntimeRoot = (workspaceRoot: string): string =>
  path.join(workspaceRoot, ".runtime", "awareness", "official-knowledge");

const governedHistoryBacklogPath = (workspaceRoot: string): string =>
  path.join(governanceRuntimeRoot(workspaceRoot), "governance-history", "latest-backlog.json");
...
const print = (json: boolean, value: OperatorOutput): void => {
  if (json) {
...
const loadOfficialKnowledge = async (workspaceRoot: string) => {
...
export const runOperatorCli = async (argv: string[] = process.argv.slice(2)): Promise<number> => {
  ...
  switch (command) {
    ...
    case "improvements": {
      ...
      if (subcommand === "backlog") {
        const backlog = await readJsonFile<Record<string, unknown>>(governedHistoryBacklogPath(workspaceRoot));
        print(json, {
          command,
          workspaceRoot,
          runtimeRoot,
