import type { DesktopActionResult, WorkflowExecutionResult } from "@contracts";
import type { GovernedTaskPlanResult, GovernedTaskVerification } from "./types";

export type GovernedTaskGapType =
  | "missing_intent_parser_rule"
  | "ambiguous_intent"
  | "missing_governance_rule"
  | "incorrect_governance_decision"
  | "missing_executor"
  | "incorrect_executor_selection"
  | "missing_preflight_check"
  | "missing_verifier"
  | "failed_verification_logic"
  | "execution_runtime_failure"
  | "missing_rollback"
  | "missing_knowledge_retrieval"
  | "missing_workflow_wrapper"
  | "approval_state_issue"
  | "missing_history_replay_rule"
  | "test_card_defect";

export interface GovernedTaskGapClassification {
  primary_gap: GovernedTaskGapType;
  secondary_gaps: GovernedTaskGapType[];
...
const hasFailureText = (text: string, needles: string[]): boolean => {
  const normalized = normalize(text);
  return needles.some((needle) => normalized.includes(normalize(needle)));
};

export interface GovernedTaskGapClassifierInput {
...
export const classifyGovernedTaskGap = (
  input: GovernedTaskGapClassifierInput
): GovernedTaskGapClassification => {
  const failureText = input.failureText ?? "";
  const route = input.route;
  const verification = input.verification;
  const executionResult = input.executionResult;
  const secondary: GovernedTaskGapType[] = [];
  let primary: GovernedTaskGapType = "execution_runtime_failure";
  ...
  if (route.decision === "clarify" && route.clarificationNeeded.length > 0) {
    ...
    hasFailureText(failureText, ["verification", "verify", "expected", "observed"]) ||
