...
const buildUiElement = (id: string, overrides: Partial<ScreenUiElement> = {}): ScreenUiElement => ({
  id,
  automationId: null,
  controlType: "Pane",
...
const buildScreenFixture = (): {
  foregroundWindow: ForegroundWindowSnapshot;
  uiTree: ScreenUiTreeSnapshot;
} => {
...
describe("screen awareness", () => {
  it("keeps Assist Mode explicit, visible, and free of raw secrets in context", async () => {
    await withWorkspace(async (workspaceRoot) => {
      ...
      expect(active.uiTree?.root?.children.map((child) => child.name)).toEqual(
        expect.arrayContaining(["Open Settings", "Save Changes", "Password"])
      );
      expect(active.uiTree?.redactedCount).toBe(1);
      expect(active.summary.highlights.controls).toEqual(
        expect.arrayContaining(["Open Settings (Button)", "Save Changes (Button)"])
      );
      ...
      const contextSection = buildScreenAwarenessContextSection(
        active,
  ...
  it("serves the safe screen-awareness endpoints on localhost", async () => {
    await withWorkspace(async (workspaceRoot) => {
      ...
      const engine = {
        ...
        startAssistMode: (options?: Parameters<typeof state.startAssist>[0]) => state.startAssist(options),
        stopAssistMode: (reason?: string) => state.stopAssist(reason),
        refresh: async () => ({}) as never,
        refreshFiles: async () => state.snapshot,
        markDigestDelivered: async () => null as never,
        appendEvent: async () => null as never,
      ...
      try {
        ...
        const foreground = (await foregroundResponse.json()) as { title: string | null; processName: string | null };
        ...
        const tree = (await treeResponse.json()) as { rootWindowTitle: string | null; redactedCount: number };
        ...
        expect(events.map((event) => event.type)).toEqual(
          expect.arrayContaining(["assist_started", "screen_frame_captured"])
        );
        ...
        const stopped = (await stopResponse.json()) as { visibleIndicator: boolean; summary: string };
