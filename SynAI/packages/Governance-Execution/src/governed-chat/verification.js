...
const summarize = (parts) => parts.filter((value) => Boolean(value)).join(" | ") || "Verification unavailable.";
const normalize = (value) => value
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
    .trim();
const containsAny = (value, needles) => needles.some((needle) => value.includes(needle));
const isDesktopActionSuccessful = (result) => result.status === "executed" || result.status === "simulated";
const isDesktopActionBlockedOrFailed = (result) => result.status === "failed" ||
...
const findMatchingService = (services, target) => {
    const normalizedTarget = normalize(target);
    const exact = services.find((service) => normalize(service.serviceName) === normalizedTarget || normalize(service.displayName) === normalizedTarget);
    if (exact) {
        return exact;
    }
...
const verifyServiceState = (kind, state) => {
...
const verifyDesktopActionFileMutation = async (route, result) => {
    ...
    return {
        ...
        reasons: isDesktopActionSuccessful(result)
...
const verifyDesktopActionProcess = async (route, result, machineAwarenessAfter) => {
...
const verifyDesktopActionService = async (_route, result, machineAwarenessAfter) => {
    const serviceName = result.target;
    const output = typeof result.output === "object" && result.output !== null ? result.output : null;
    const observedAfterState = typeof output?.after === "object" && output.after !== null ? output.after : null;
    const observedStateText = typeof observedAfterState?.state === "string"
    ...
    return {
        ...
        expected_state_summary: result.kind === "stop-service"
...
const verifyDesktopActionRegistry = async (_route, result) => {
    const output = typeof result.output === "object" && result.output !== null ? result.output : null;
    ...
    return {
        ...
        expected_state_summary: result.kind === "delete-registry-value"
            ? `Registry value ${valueName} should be removed.`
            : `Registry value ${valueName} should be updated.`
    };
};
const verifyDesktopActionUi = async (_route, result, screenAwarenessAfter) => {
    const output = typeof result.output === "object" && result.output !== null ? result.output : null;
    ...
    const eventEvidence = screenAwarenessAfter.recentEvents.some((event) => containsAny(event.type, ["click_observed", "active_window_changed", "ui_tree_refreshed", "hover_target_changed"]));
    const matchedTarget = Boolean(focusedName && focusedName.includes(target)) ||
    ...
    return {
        ...
        expected_state_summary: result.kind === "ui-type"
...
const verifyWorkflowResult = async (route, result) => {
...
export const verifyGovernedTaskExecution = async (input) => {
    ...
    if ("plan" in executionResult) {
        return verifyWorkflowResult(route, executionResult);
    }
    const desktopResult = executionResult;
    ...
    if (route.actionType?.includes("service") ||
    ...
    if (route.actionType?.includes("registry") ||
    ...
    if (route.actionType?.includes("workflow")) {
        return {
            ...
            reasons: isDesktopActionSuccessful(desktopResult)
...
export const summarizeVerification = (verification) => summarize([
