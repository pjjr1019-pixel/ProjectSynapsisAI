...
describe("windows powershell runner", () => {
    ...
    it("parses successful json output asynchronously", async () => {
        ...
        await expect(runPowerShellJson("Write-Output", {
            platform: "win32",
            spawnProcess
        })).resolves.toEqual({ machine: "test-machine" });
        expect(spawnProcess).toHaveBeenCalledWith("powershell.exe", expect.arrayContaining(["-NoProfile", "-Command", "Write-Output"]), expect.objectContaining({
            windowsHide: true
        }));
    });
    ...
    it("returns null when powershell exits with a non-zero code", async () => {
        ...
        await expect(runPowerShellJson("Get-Process", {
            platform: "win32",
            spawnProcess
        })).resolves.toBeNull();
    });
