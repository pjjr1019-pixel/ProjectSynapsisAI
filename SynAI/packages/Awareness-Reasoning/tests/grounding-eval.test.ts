...
import type { AwarenessQueryAnswer, GroundingSource } from "../src/contracts";
import { detectReasoningMode, groundAssistantReply } from "../src/reasoning";

const awarenessAnswer = (): AwarenessQueryAnswer =>
  ({
    id: "awareness-eval",
    query: "cpu",
  ...
  it("covers workspace, mixed, awareness, and conflicting evidence scenarios", async () => {
    ...
    for (const fixture of fixtures) {
      ...
      const grounded = await groundAssistantReply({
        ...
        sources: fixture.sources as GroundingSource[],
      ...
      const citedKinds = grounded.metadata.claims
        ...
        .filter((kind): kind is GroundingSource["kind"] => Boolean(kind));

      const citedKindSet = new Set(citedKinds);
      for (const kind of fixture.expected.topKinds) {
        expect(citedKindSet.has(kind)).toBe(true);
      }
      expect(grounded.metadata.summary.citationCoverage).toBeGreaterThanOrEqual(
        fixture.expected.minCitationCoverage
      );
      expect(acceptableConfidenceLevels[fixture.expected.confidence]).toContain(
        grounded.metadata.summary.overallConfidence
      );
