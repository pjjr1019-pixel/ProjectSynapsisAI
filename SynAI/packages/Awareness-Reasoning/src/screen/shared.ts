...
export const buildFreshness = (capturedAt: string, now = new Date()): FreshnessMetadata =>
...
export const compactJoin = (values: Array<string | null | undefined>, limit = 6): string =>
  values
    .filter((value): value is string => Boolean(value))
    .slice(0, limit)
    .join(", ");

export const describeScope = (scope: AssistModeScope | null): string => {
  switch (scope) {
    case "current-window":
...
export const describeCaptureMode = (captureMode: ScreenCaptureMode): string =>
  captureMode === "session" ? "session sampling" : "on-demand";

export const describeBounds = (bounds: ForegroundWindowSnapshot["bounds"]): string | null =>
  bounds ? `${bounds.x},${bounds.y} ${bounds.width}x${bounds.height}` : null;

export const queryIncludes = (query: string, keywords: string[]): boolean => {
...
export const isScreenRelevant = (query: string, awarenessMode: string, assistEnabled: boolean): boolean =>
  awarenessMode === "debug" ||
  (assistEnabled &&
    queryIncludes(query, SCREEN_CONTEXT_KEYWORDS)) ||
  (assistEnabled && normalizedQueryHasWindowIntent(query));

const normalizedQueryHasWindowIntent = (query: string): boolean =>
  queryIncludes(query, [
    "what window",
    "what app",
    "what am i looking at",
...
export const isInteractiveControl = (controlType: string | null | undefined): boolean => {
  if (!controlType) {
    return false;
  }
...
export const isSensitiveScreenValue = (value: string | null | undefined): boolean => {
  if (!value) {
    return false;
  }
...
export const isProtectedScreenElement = (element: Pick<ScreenUiElement, "isPassword" | "name" | "value" | "controlType" | "localizedControlType">): boolean =>
  element.isPassword ||
...
export const formatWindowSummary = (window: ForegroundWindowSnapshot | null | undefined): string | null => {
  if (!window) {
    return null;
  }
...
const pushUnique = (values: string[], candidate: string): void => {
  if (!values.includes(candidate)) {
    values.push(candidate);
  }
};

export const collectUiHighlights = (
  node: ScreenUiElement | null | undefined,
  options?: { limit?: number; includeContainers?: boolean }
): string[] => {
  const limit = options?.limit ?? 6;
  const highlights: string[] = [];

  const walk = (current: ScreenUiElement | null | undefined): void => {
...
export const countUiNodes = (node: ScreenUiElement | null | undefined): number => {
  if (!node) {
    return 0;
  }
...
export const countInteractiveUiNodes = (node: ScreenUiElement | null | undefined): number => {
  if (!node) {
    return 0;
  }
...
export const findFocusedUiElement = (node: ScreenUiElement | null | undefined): ScreenUiElement | null => {
  if (!node) {
    return null;
  }
...
export const findFirstInteractiveElement = (node: ScreenUiElement | null | undefined): ScreenUiElement | null => {
  if (!node) {
    return null;
  }
...
export const summarizeEvent = (event: AwarenessEvent): string =>
  `${event.type}${event.message ? `: ${event.message}` : ""}`;

export const summarizeRecentEvents = (events: AwarenessEvent[], limit = 5): string[] =>
  [...events]
    .slice(-limit)
...
export const buildHighlights = (snapshot: {
  ...
  return {
    activeWindow: formatWindowSummary(snapshot.foregroundWindow)
      ? [formatWindowSummary(snapshot.foregroundWindow) as string]
      : [],
...
export const buildScreenDiffSummary = (input: {
  ...
  const focusedElementChanged =
    (previousFocused?.name ?? previousFocused?.controlType ?? null) !==
    (currentFocused?.name ?? currentFocused?.controlType ?? null);
  const hoveredElementChanged =
    (previousHover?.name ?? previousHover?.controlType ?? null) !==
