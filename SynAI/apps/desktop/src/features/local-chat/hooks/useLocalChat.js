...
import { buildLiveUsageMessageMetadata, formatLiveUsageReply, isLiveUsageAnswer, LIVE_USAGE_REFRESH_MS } from "../utils/liveUsageReply";
const bridge = () => window.synai;
const screenBridge = () => bridge();
const awarenessBridge = () => bridge();
...
const loadPersistedSettings = () => {
    ...
    try {
        ...
        return {
            ...
            awarenessAnswerMode: parsed.awarenessAnswerMode === "llm-primary" || parsed.awarenessAnswerMode === "evidence-first"
...
const mergeAvailableModels = (...lists) => {
    const seen = new Set();
...
const createOptimisticMessage = (conversationId, role, content) => ({
...
const buildOptimisticMessages = (messages, conversationId, text, regenerate) => {
    ...
    if (regenerate) {
        ...
        const trimmedMessages = lastAssistantIndex === -1
            ? messages
...
const buildHealthCheckFeedback = (modelHealth) => {
...
const hydrateSettingsFromModel = (settings, modelHealth) => settings.selectedModel
    ? settings
    : {
        ...settings,
        selectedModel: modelHealth.model
    };
export const useLocalChat = (options = {}) => {
    ...
    const liveUsageTimerRef = useRef(null);
    const liveUsageSessionRef = useRef(null);
    ...
    const stopLiveUsageRefresh = (reason = "stop") => {
        void reason;
    ...
    const updateLiveUsageMessage = (session, answer) => {
        const refreshedAt = answer.generatedAt ?? new Date().toISOString();
        localChatStore.setState({
            messages: localChatStore.getState().messages.map((message) => message.id === session.messageId
                ? {
                    ...message,
    ...
    useEffect(() => {
        ...
        const scheduleAppHealthPoll = (delayMs = nextAppHealthDelay()) => {
        ...
        const flushStreamingContent = () => {
            ...
            localChatStore.setState({
                messages: s.messages.map((message) => message.id === pendingId ? { ...message, content } : message)
            });
        };
        const unsubscribeChatStream = bridge().subscribeChatStream((event) => {
            const currentState = localChatStore.getState();
            if (currentState.pendingRequestId !== event.requestId ||
        ...
        const unsubscribeReasoningTrace = bridge().subscribeReasoningTrace?.((event) => {
            const currentState = localChatStore.getState();
            if (currentState.pendingRequestId !== event.requestId ||
                currentState.activeConversationId !== event.conversationId) {
                return;
    ...
    const startAssistMode = async (options) => {
        try {
            const screenApi = screenBridge();
    ...
    const stopAssistMode = async (reason = "user-disabled") => {
        try {
            const screenApi = screenBridge();
    ...
    const sendMessage = async (text, regenerate = false, options) => {
        if (!state.activeConversationId || !text.trim()) {
            return;
        }
        stopLiveUsageRefresh("new-message");
        const resolvedUseWebSearch = options?.webMode === "on" ? true : options?.webMode === "off" ? false : undefined;
        const requestId = `chat-${Date.now()}-${Math.random().toString(16).slice(2)}`;
        const previousMessages = state.messages;
        const previousConversations = state.conversations;
        const optimistic = buildOptimisticMessages(state.messages, state.activeConversationId, text, regenerate);
        localChatStore.setState({
            loading: true,
    ...
    const refreshRetrievedMemory = async () => {
        if (!state.activeConversationId) {
            return;
        }
        const lastUser = [...state.messages].reverse().find((message) => message.role === "user");
        ...
        try {
            const awareContextPreview = await bridge().getContextPreview(state.activeConversationId, lastUser.content, state.settings.awarenessAnswerMode, {
                defaultEnabled: state.settings.advancedRagEnabled,
                defaultUseWeb: state.settings.defaultWebSearch && state.settings.webInRagEnabled,
                defaultShowTrace: state.settings.liveTraceVisible,
                workspaceIndexingEnabled: state.settings.workspaceIndexingEnabled
    ...
    const runPromptEvaluation = async (request) => {
        ...
        try {
            ...
            localChatStore.setState({
                promptEvaluationError: error instanceof Error ? error.message : "Failed to run prompt evaluation"
            });
        }
        finally {
