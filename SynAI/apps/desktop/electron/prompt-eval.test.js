...
import { buildPromptEvaluationChatHistoryPath, buildPromptEvaluationReportFileName, formatPromptEvaluationMarkdown, upsertPromptEvaluationChatHistory } from "./prompt-eval";
const report = {
...
describe("prompt evaluation markdown", () => {
    it("builds a readable timestamped report filename", () => {
        expect(buildPromptEvaluationReportFileName(report.suiteName, report.generatedAt)).toBe("20260409-191530Z-windows-grounding-eval.md");
    });
    it("builds the chat history path beside prompt-eval reports", () => {
        expect(buildPromptEvaluationChatHistoryPath(report.workspaceRoot)).toBe(path.join("C:/workspace", ".runtime", "prompt-evals", "chathistory.md"));
    });
    ...
    it("appends prompt/reply history entries for later gap analysis", () => {
        ...
        expect(markdown).toContain("- Use the requested Known heading. | Skipped because the prompt returned an error.");
    });
});
