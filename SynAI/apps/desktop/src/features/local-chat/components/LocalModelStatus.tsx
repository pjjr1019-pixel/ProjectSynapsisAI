...
export function LocalModelStatus({ modelHealth }: LocalModelStatusProps) {
  if (!modelHealth) {
    return <Card className="p-3 text-xs text-slate-400">Model status unavailable.</Card>;
  }
  ...
  return (
    <Card className="space-y-2 p-3">
      ...
      <p className="text-xs text-slate-400">{modelHealth.model}</p>
      <p className="text-xs text-slate-500">{modelHealth.baseUrl}</p>
      <p className="text-xs text-slate-500">Last checked {formatTime(modelHealth.checkedAt)}</p>
