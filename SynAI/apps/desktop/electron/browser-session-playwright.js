...
const collectSnapshot = async (page, runtimeRoot, backend = "playwright") => {
...
const resolvePage = async (context) => {
...
const findTargetFromDocument = async (page, query, kind) => {
    return await page.evaluate(({ queryValue, kindValue }) => {
        ...
        const selectors = kindValue === "type" || kindValue === "select"
...
const createPlaywrightHost = (runtimeRoot) => {
    ...
    const ensurePage = async (visible = true) => {
    ...
    const search = async (query, maxResults = 5) => {
        ...
        const results = await page
            .evaluate((limit) => Array.from(document.querySelectorAll("li.b_algo"))
            ...
            .filter((entry) => Boolean(entry.url)), maxResults)
            .catch(() => []);
    ...
    const type = async (selectorOrText, text, submit = false, visible = true) => {
        ...
        await page.evaluate(({ queryValue, value, submitValue }) => {
            const normalized = (candidate) => String(candidate || "")
                .toLowerCase()
                .replace(/[^a-z0-9]+/g, " ")
                .replace(/\s+/g, " ")
                .trim();
        ...
        }, { queryValue: query, value: text, submitValue: submit }).catch((error) => {
            throw error instanceof Error ? error : new Error(String(error));
        });
    ...
    const waitFor = async (selectorOrText, visible = true, timeoutMs = 5_000) => {
        const page = await ensurePage(visible);
        const query = selectorOrText.trim();
        try {
            ...
            }, query, { timeout: timeoutMs }).catch((error) => {
                throw error instanceof Error ? error : new Error(String(error));
            });
        }
        return await collectSnapshot(page, runtimeRoot);
    };
    const select = async (selectorOrText, value, visible = true) => {
        const page = await ensurePage(visible);
        const query = selectorOrText.trim();
        await page.evaluate(({ queryValue, valueToSelect }) => {
            const normalized = (candidate) => String(candidate || "")
                .toLowerCase()
                .replace(/[^a-z0-9]+/g, " ")
                .replace(/\s+/g, " ")
            ...
            if (selectElement.tagName === "SELECT" && selectElement.options) {
                const option = Array.from(selectElement.options).find((entry) => normalized(entry.value) === normalized(valueToSelect) ||
            ...
            element.click?.();
        }, { queryValue: query, valueToSelect: value }).catch((error) => {
            throw error instanceof Error ? error : new Error(String(error));
        });
        await page.waitForLoadState("networkidle", { timeout: 2_500 }).catch(() => undefined);
        return await collectSnapshot(page, runtimeRoot);
    };
    const download = async (url, fileName, visible = true) => {
        const page = await ensurePage(visible);
...
export const createPlaywrightWorkflowBrowserHost = (runtimeRoot) => createPlaywrightHost(runtimeRoot);
