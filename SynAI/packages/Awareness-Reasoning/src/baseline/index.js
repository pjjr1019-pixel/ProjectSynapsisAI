...
export const PHASE_ONE_PERMISSION_TIERS = ["Observe", "Open/Navigate"];
export const PHASE_ONE_ALLOWED_PRIVACY_SCOPE = "user-visible local content";
export const DEFAULT_AWARENESS_MODE = "observe";
...
const digestHash = (value) => createHash("sha256").update(value).digest("hex");
const jsonFingerprint = (value) => digestHash(JSON.stringify(value));
const MAX_STATUS_ENTRIES = 40;
const clone = (value) => JSON.parse(JSON.stringify(value));
export const canUsePermissionTier = (tier) => PHASE_ONE_PERMISSION_TIERS.includes(tier);
export const canAccessPrivacyScope = (requestedScope, allowedScope = PHASE_ONE_ALLOWED_PRIVACY_SCOPE) => SCORES[requestedScope] <= SCORES[allowedScope];
export const filterPrivacyScopedValues = (items, allowedScope = PHASE_ONE_ALLOWED_PRIVACY_SCOPE) => {
    const allowed = [];
    const blockedScopes = [];
...
const formatCommitSummaries = (raw, maxCommits) => {
    if (!raw) {
        return [];
    }
    return raw
...
const createEmptyWorkingTreeCounts = () => ({
    total: 0,
    staged: 0,
    unstaged: 0,
    untracked: 0,
...
const isConflictStatus = (status) => ["DD", "AU", "UD", "UA", "DU", "AA", "UU"].includes(status) ||
    status.includes("U");
const statusToken = (code) => {
    switch (code) {
        case "M":
            return "modified";
        case "A":
...
const summarizeWorkingTreeEntry = (stagedStatus, unstagedStatus, kind) => {
    const parts = [
...
const summarizeWorkingTree = (counts, entries, isTruncated) => {
    if (counts.total === 0) {
...
const parseWorkingTree = (raw, maxEntries = MAX_STATUS_ENTRIES) => {
    if (raw == null) {
        return {
            totalCount: 0,
            isTruncated: false,
...
export const resolveRepositoryRoot = (startDir) => {
...
export const captureRepoBaseline = async (options) => {
    ...
    const [branch, headSha, dirtyOutput, commitOutput] = await Promise.all([
        ...
        Promise.resolve(runGit(["log", "--pretty=format:%H%x09%h%x09%ad%x09%s", "--date=iso-strict", "-n", String(maxCommits)], workspaceRoot))
    ]);
    const fallbackMode = branch == null && headSha == null && dirtyOutput == null && commitOutput == null;
    const dirtyState = dirtyOutput == null ? "unknown" : dirtyOutput.length > 0 ? "dirty" : "clean";
...
export const captureMachineBaseline = (options = {}) => {
    const now = options.now ?? (() => new Date());
    const sampleNow = now();
...
export const buildSessionBaseline = (options) => {
    const now = options.now ?? (() => new Date());
...
export const buildAwarenessDigest = (input) => {
    ...
    const reason = input.includeReasonOverride ??
...
export const buildAwarenessPromptSection = (digest) => {
