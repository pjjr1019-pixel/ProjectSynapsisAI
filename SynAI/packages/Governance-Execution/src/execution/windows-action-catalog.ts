...
export type WindowsActionDefinition = DesktopActionProposal;
...
const formatPreviewToken = (value: string, fallback: string): string => {
...
const renderPreview = (
  template: string,
  values: { target: string; destinationTarget?: string | null; args?: string[] }
): string =>
  template
    .replace(/\{target\}/g, formatPreviewToken(values.target, "<target>"))
    .replace(
      /\{destination\}/g,
      formatPreviewToken(values.destinationTarget ?? "", "<destination>")
    )
    .replace(/\{args\}/g, values.args?.length ? values.args.map((arg) => formatPreviewToken(arg, arg)).join(" ") : "");

const createDefinition = (definition: WindowsActionDefinition): WindowsActionDefinition => ({
  ...definition,
  aliases: [...definition.aliases],
  preconditions: [...definition.preconditions]
});

const makeCommandPreview = (
  definition: WindowsActionDefinition,
  target?: string | null,
  destinationTarget?: string | null,
  args?: string[]
): string =>
  renderPreview(definition.commandPreview, {
    target: target?.trim() || definition.defaultTarget || definition.targetPlaceholder,
    destinationTarget,
    args
  }).trim();

const appSurfaceDefinition = (
  id: string,
  title: string,
  description: string,
  defaultTarget: string,
  aliases: string[],
  preconditions: string[] = ["Windows shell integration available"]
...
export const WINDOWS_ACTION_CATALOG: WindowsActionDefinition[] = [
  createDefinition({
  ...
  appSurfaceDefinition(
    "open-task-manager",
    "Open Task Manager",
    "Open Task Manager for process inspection and manual task control.",
    "taskmgr",
    ["task manager", "taskmgr", "process manager", "end task"]
  ),
  appSurfaceDefinition(
    "open-control-panel",
    "Open Control Panel",
    "Open the classic Control Panel shell.",
    "control",
    ["control panel", "classic control panel", "control.exe"]
  ),
  appSurfaceDefinition(
    "open-services",
    "Open Services",
    ...
    "services.msc",
    ["services", "service manager", "services.msc"]
  ),
  appSurfaceDefinition(
    "open-device-manager",
    "Open Device Manager",
    "Open Device Manager for hardware inspection.",
    "devmgmt.msc",
  ...
  appSurfaceDefinition(
    "open-event-viewer",
    "Open Event Viewer",
    "Open Event Viewer for system log inspection.",
    "eventvwr.msc",
  ...
  appSurfaceDefinition(
    "open-task-scheduler",
    "Open Task Scheduler",
    "Open Task Scheduler for scheduled task inspection.",
    "taskschd.msc",
  ...
  appSurfaceDefinition(
    ...
    "Open Computer Management",
    "Open the consolidated Computer Management console.",
    "compmgmt.msc",
  ...
  appSurfaceDefinition(
    ...
    "Open Registry Editor",
    "Open Registry Editor without making changes.",
    "regedit",
...
export const listWindowsActionDefinitions = (): WindowsActionDefinition[] =>
  ACTION_MATCH_ORDER.map((entry) => ({
    ...entry,
    aliases: [...entry.aliases],
    preconditions: [...entry.preconditions]
  }));

export const findWindowsActionDefinitionById = (id: string): WindowsActionDefinition | null =>
  ACTION_MATCH_ORDER.find((entry) => entry.id === id) ?? null;

export const buildWindowsActionPreview = (
  action: WindowsActionDefinition,
  overrides: { target?: string | null; destinationTarget?: string | null; args?: string[] } = {}
): string =>
  makeCommandPreview(action, overrides.target, overrides.destinationTarget, overrides.args);

export const suggestWindowsActionFromPrompt = (prompt: string): WindowsActionDefinition | null => {
  ...
  for (const action of ACTION_MATCH_ORDER) {
    if (
      action.aliases.some((alias) => normalized.includes(normalize(alias))) ||
...
export const inferWindowsActionKinds = (): Record<DesktopActionKind, { scope: ActionScope; targetKind: DesktopActionTargetKind; riskClass: DesktopActionRiskClass }> => ({
