...
import { CAPABILITY_CARD_SCHEMA_VERSION, CAPABILITY_CARD_SCHEMA_VERSIONS, CAPABILITY_PLATFORMS, CAPABILITY_RISK_LEVELS, CAPABILITY_VERIFIER_TYPES } from "./types";
const isRecord = (value) => typeof value === "object" && value !== null && !Array.isArray(value);
const asStringArray = (value) => Array.isArray(value) ? value.filter((entry) => typeof entry === "string").map((entry) => entry.trim()) : [];
const normalizeContextRequirements = (value) => {
    ...
    return value
        ...
        maxChars: typeof entry.maxChars === "number" && Number.isFinite(entry.maxChars)
            ? Math.max(128, Math.floor(entry.maxChars))
            : undefined
    }));
};
const validateContextRequirements = (requirements, label, errors) => {
    ...
    for (const entry of requirements) {
        ...
        if ((entry.source === "awareness.live-usage" ||
...
export const validateCapabilityCard = (rawCard) => {
    const errors = [];
    if (!isRecord(rawCard)) {
        return ["Card must be an object."];
    }
    const card = rawCard;
    if (!CAPABILITY_CARD_SCHEMA_VERSIONS.includes(card.schema_version)) {
        errors.push(`schema_version must be one of: ${CAPABILITY_CARD_SCHEMA_VERSIONS.join(", ")}.`);
    }
    const requiredStringFields = ["id", "name", "category", "description", "prompt", "success_definition"];
    for (const field of requiredStringFields) {
        if (typeof card[field] !== "string" || card[field].trim().length === 0) {
            errors.push(`${field} must be a non-empty string.`);
        }
    }
    if (!CAPABILITY_VERIFIER_TYPES.includes(card.verifier_type)) {
        errors.push(`verifier_type must be one of: ${CAPABILITY_VERIFIER_TYPES.join(", ")}.`);
    }
    if (!CAPABILITY_RISK_LEVELS.includes(card.risk_level)) {
    ...
    if (card.platform != null && !CAPABILITY_PLATFORMS.includes(card.platform)) {
    ...
    if (card.task_type != null && !taskTypes.includes(card.task_type)) {
        errors.push(`task_type must be one of: ${taskTypes.join(", ")}.`);
    }
    const decisions = ["allow", "allow_with_verification", "require_approval", "deny", "clarify", "plan_only"];
    if (card.expected_governance_decision != null &&
        !decisions.includes(card.expected_governance_decision)) {
        errors.push(`expected_governance_decision must be one of: ${decisions.join(", ")}.`);
    }
    if (card.expected_risk_tier != null && !CAPABILITY_RISK_LEVELS.includes(card.expected_risk_tier)) {
...
export const normalizeCapabilityCard = (rawCard) => {
    const record = (isRecord(rawCard) ? rawCard : {});
    ...
    return {
        schema_version: typeof record.schema_version === "string" &&
            CAPABILITY_CARD_SCHEMA_VERSIONS.includes(record.schema_version)
        ...
        verifier_type: record.verifier_type ?? "substring-regex",
        ...
        retry_policy: {
            maxAttempts: typeof retryPolicy.maxAttempts === "number" && Number.isFinite(retryPolicy.maxAttempts)
                ? Math.max(1, Math.floor(retryPolicy.maxAttempts))
                : 1,
            retryDelayMs: typeof retryPolicy.retryDelayMs === "number" && Number.isFinite(retryPolicy.retryDelayMs)
        ...
        platform: typeof record.platform === "string" && CAPABILITY_PLATFORMS.includes(record.platform)
            ? record.platform
            : undefined,
        ...
        timeout_ms: typeof record.timeout_ms === "number" && Number.isFinite(record.timeout_ms)
            ? Math.max(500, Math.floor(record.timeout_ms))
            : undefined,
        expected_artifacts: asStringArray(record.expected_artifacts),
        task_type: typeof record.task_type === "string" &&
            [
                "inspection",
                "safe-action",
                "approval-action",
        ...
        expected_governance_decision: typeof record.expected_governance_decision === "string" &&
            ["allow", "allow_with_verification", "require_approval", "deny", "clarify", "plan_only"].includes(record.expected_governance_decision)
            ? record.expected_governance_decision
            : undefined,
        expected_risk_tier: typeof record.expected_risk_tier === "string" &&
            CAPABILITY_RISK_LEVELS.includes(record.expected_risk_tier)
            ? record.expected_risk_tier
            : undefined,
        ...
        timeout: typeof record.timeout === "number" && Number.isFinite(record.timeout)
            ? Math.max(500, Math.floor(record.timeout))
            : undefined,
        notes: typeof record.notes === "string" ? record.notes : undefined
    };
};
export const parseCapabilityCard = (rawCard, sourcePath) => {
...
export const loadCapabilityCardFromFile = async (filePath) => {
