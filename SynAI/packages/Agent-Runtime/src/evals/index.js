...
const buildEvalIssue = (message, code, details) => ({
    message,
    code,
    details,
});
export const buildNoopRuntimeEvalCases = () => [
    {
        id: 'allow-echo',
...
export const evaluateAgentRuntimeCase = (input, run) => {
    const issues = [];
    if (run.result.status !== input.expectedStatus) {
        issues.push(buildEvalIssue('Runtime status did not match the expected outcome.', 'status-mismatch', {
            expected: input.expectedStatus,
            actual: run.result.status,
        }));
    }
    if (input.expectedPolicyDecisionType &&
        run.policyDecision.type !== input.expectedPolicyDecisionType) {
        issues.push(buildEvalIssue('Policy decision type did not match expectation.', 'policy-mismatch', {
            expected: input.expectedPolicyDecisionType,
            actual: run.policyDecision.type,
        }));
    }
    if (input.expectedVerificationStatus &&
        run.verification.status !== input.expectedVerificationStatus) {
        issues.push(buildEvalIssue('Verification status did not match expectation.', 'verification-mismatch', {
            expected: input.expectedVerificationStatus,
            actual: run.verification.status,
        }));
    }
    if (typeof input.expectedPlanStepCount === 'number' &&
        run.plannedSteps.length !== input.expectedPlanStepCount) {
        issues.push(buildEvalIssue('Planned step count did not match expectation.', 'plan-step-count-mismatch', {
    ...
    if (input.expectedCheckpointPhase && run.checkpoint.phase !== input.expectedCheckpointPhase) {
        issues.push(buildEvalIssue('Checkpoint phase did not match expectation.', 'checkpoint-phase-mismatch', {
    ...
    if (input.expectedContinuationMode && run.checkpoint.continuation?.mode !== input.expectedContinuationMode) {
        issues.push(buildEvalIssue('Checkpoint continuation mode did not match expectation.', 'checkpoint-continuation-mismatch', {
    ...
    if (input.expectedContinuationMode && run.result.continuation?.mode !== input.expectedContinuationMode) {
        issues.push(buildEvalIssue('Result continuation mode did not match expectation.', 'result-continuation-mismatch', {
            expected: input.expectedContinuationMode,
            actual: run.result.continuation?.mode ?? null,
        }));
    }
    if (typeof input.expectedContinuationResumable === 'boolean' &&
        run.checkpoint.continuation?.resumable !== input.expectedContinuationResumable) {
        issues.push(buildEvalIssue('Checkpoint resumable flag did not match expectation.', 'checkpoint-resumable-mismatch', {
    ...
    if (typeof input.expectedContinuationResumable === 'boolean' &&
        ...
        issues.push(buildEvalIssue('Result resumable flag did not match expectation.', 'result-resumable-mismatch', {
            expected: input.expectedContinuationResumable,
...
export const runAgentRuntimeEvalSuite = async (cases, runtime = {
