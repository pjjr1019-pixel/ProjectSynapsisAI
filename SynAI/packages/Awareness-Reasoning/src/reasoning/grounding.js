...
const clip = (value, maxChars = MAX_SOURCE_EXCERPT_CHARS) => value.trim().length <= maxChars ? value.trim() : `${value.trim().slice(0, maxChars - 1)}...`;
...
const uniqueStrings = (items) => unique(items.filter(Boolean));
const extractNumbers = (value) => uniqueStrings(Array.from(value.matchAll(/\b\d+(?:\.\d+)?%?\b/g), (match) => match[0] ?? ""));
const confidenceFromStatus = (status, fallback = "medium") => {
...
const labelForEvidenceRef = (ref) => ref.label ?? `${ref.kind} evidence`;
const sourceText = (source) => `${source.title}\n${source.label}\n${source.excerpt}`;
const sourceCountByKind = (sources) => sources.reduce((counts, source) => {
    counts[source.kind] += 1;
    return counts;
}, createEmptyKindCounts());
const sourceIdForWebResult = (result, index) => `web:${result.url || result.title}:${index}`;
const awarenessExcerpt = (answer) => clip([
    answer.summary,
    ...
    .filter(Boolean)
    .join(" | "));
const officialHitsFor = (officialKnowledge, awarenessQuery) => {
...
export const buildGroundingSourceCatalog = (input) => {
    const sources = [];
    const seen = new Set();
...
export const segmentAnswerClaims = (answerText) => {
    const lines = answerText
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter(Boolean);
    const claims = [];
    const headingLikeLine = (value) => /^(?:built now|not built yet|trigger|action|surfacing|extraction|importance|retrieval|summary|why it matters|key facts|unclear|next)$/i.test(value.trim());
    const looksCommandLike = (value) => /\bms-settings:|control\.exe|\.cpl\b|\.msc\b|regedit(?:\.exe)?\b|hklm\\|hkcu\\|hklm:|hkcu:/i.test(value);
...
export const createGroundingVerifierMessages = (answerText, claims, sources) => {
    const sourceCatalog = sources
        .map((source) => `${source.id} | kind=${source.kind} | title=${source.title} | label=${source.label} | excerpt=${clip(source.excerpt, MAX_VERIFIER_SOURCE_EXCERPT_CHARS)}`)
        .join("\n");
...
export const parseGroundingVerifierResponse = (raw, claimTexts, validSourceIds) => {
...
const claimLooksInference = (claimText) => /\b(may|might|could|likely|probably|appears|suggests|unclear|uncertain|why it matters)\b/i.test(claimText);
const scoreSourceForClaim = (claimText, source) => {
...
const detectConflictDescription = (sources, claimText) => {
    if (sources.length < 2) {
        return null;
    }
...
const buildHeuristicClaims = (claimTexts, sources) => {
    const claims = [];
    const conflicts = [];
    for (const [index, claimText] of claimTexts.entries()) {
        ...
        const status = sourceIds.length === 0
            ? "unsupported"
            : conflictDescription
                ? "conflicted"
                : inference
                    ? "inference"
                    : "grounded";
        const confidence = status === "unsupported" || status === "conflicted"
            ? "low"
            : status === "inference"
...
const commandTokensFor = (value) => uniqueStrings(Array.from(value.matchAll(/(?:ms-settings:[^\s,;]+|control\.exe\s+[^\s,;]+|[a-z0-9_-]+\.cpl\b|[a-z0-9_-]+\.msc\b|regedit(?:\.exe)?\b|hk(?:lm|cu)[^ \n\r\t,;]*)/gi), (match) => normalizeLine(match[0] ?? "")));
const directClaimMatch = (claimText, evidenceText) => {
...
const bestMatchingSourceIds = (sourceIds, fallbackIds, limit = 2) => {
    if (sourceIds.length > 0) {
        return sourceIds.slice(0, limit);
    }
    return fallbackIds.slice(0, limit);
};
export const buildDeterministicAwarenessGrounding = (answerText, awarenessQuery, sources) => {
    ...
    const findMatchingSourceIds = (claimText, sourceKind) => sources
    ...
    const claims = claimTexts.map((claimText, index) => {
        const claimId = `claim:${index}`;
        const matchesVerified = awarenessQuery.bundle.verifiedFindings.some((item) => directClaimMatch(claimText, item)) ||
            awarenessQuery.bundle.officialVerified.some((item) => directClaimMatch(claimText, item));
        const matchesInference = claimLooksInference(claimText) ||
        ...
        const officialOnly = awarenessQuery.bundle.officialVerified.some((item) => directClaimMatch(claimText, item)) ||
...
const mergeVerifierWithHeuristics = (claimTexts, parsed, heuristic) => {
    const claims = claimTexts.map((claimText, index) => {
        ...
        const status = parsedClaim.status === "unsupported" && fallback.sourceIds.length > 0 ? fallback.status : parsedClaim.status;
        return {
            id: fallback.id,
            text: claimText,
            sourceIds,
    ...
    const conflicts = parsed.conflicts.length > 0
...
const usedSourceIdsFor = (claims) => uniqueStrings(claims.flatMap((claim) => claim.sourceIds));
export const buildGroundingSummary = (input) => {
    ...
    const citationCoverage = claimCount === 0 ? 0 : Number((input.claims.filter((claim) => claim.sourceIds.length > 0).length / claimCount).toFixed(2));
    const staleUsedSourceCount = usedSources.filter((source) => source.freshness && !source.freshness.isFresh).length;
    const topSourceIds = Object.entries(input.claims.reduce((counts, claim) => {
        ...
        .slice(0, 3);
    const strongAwarenessSupport = input.awarenessConfidenceLevel === "high" && (input.awarenessTraceCount ?? 0) >= 2;
    ...
    if (claimCount > 0 && usedSourceIds.length > 0) {
        ...
        else if (staleUsedSourceCount === 0 &&
...
export const groundAssistantReply = async (input) => {
    ...
    const claimExtractionMode = input.deterministicAwareness
