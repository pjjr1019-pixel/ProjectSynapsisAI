...
import { buildCapabilityCliArgs, discoverCapabilityCards, parseCapabilityCliSummary, resolveCardArtifactPath } from "./core";
const execFileAsync = promisify(execFile);
const controllerId = "synaiCapabilityTests";
const controllerLabel = "SynAI Capability Tests";
const npmExecutable = process.platform === "win32" ? "npm.cmd" : "npm";
const runCapabilityCli = async (workspaceRoot, args) => {
    try {
...
const runOperatorCli = async (workspaceRoot, args) => {
    try {
...
const firstWorkspaceRoot = () => vscode.workspace.workspaceFolders?.[0]?.uri.fsPath ?? null;
        ...
        for (const testItem of selected) {
            ...
            try {
                ...
                if (!result || result.status === "failed") {
                    ...
                    run.failed(testItem, new vscode.TestMessage(artifactPath
                ...
                run.errored(testItem, new vscode.TestMessage(error instanceof Error ? error.message : String(error)));
            }
        }
        run.end();
    };
    controller.createRunProfile("Run Capability Test", vscode.TestRunProfileKind.Run, runHandler, true);
    context.subscriptions.push(vscode.commands.registerCommand("synaiCapability.refreshTests", refreshTests), vscode.commands.registerCommand("synaiCapability.runFailed", async () => {
        const workspaceRoot = firstWorkspaceRoot();
        if (!workspaceRoot) {
            return;
        }
        ...
        const summary = await runCapabilityCli(workspaceRoot, args);
        void vscode.window.showInformationMessage(`Capability rerun complete: ${summary.totals.passed} passed / ${summary.totals.failed} failed`);
        ...
        operatorOutput.appendLine(`Approvals: ${JSON.stringify(summary.approvalQueue?.totals ?? {})}`);
        operatorOutput.appendLine(`Knowledge: ${JSON.stringify(summary.officialKnowledge ?? {})}`);
