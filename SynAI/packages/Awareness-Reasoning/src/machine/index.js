...
import { buildControlPanelMap, buildRegistryZoneMap, buildSettingsMap, searchControlPanelEntries, searchRegistryZoneEntries, searchSettingsMapEntries, toControlPanelSummary, toMapSummary, toRegistryZoneSummary } from "./maps";
...
const compactJoin = (values, limit = MAX_CONTEXT_LIST_ITEMS) => values
...
const buildFreshness = (capturedAt, now) => createFreshnessMetadata(capturedAt, capturedAt, now);
const sortProcesses = (processes) => [...processes].sort((a, b) => {
...
const sortServices = (services) => [...services].sort((a, b) => {
...
const sortStartupEntries = (entries) => [...entries].sort((a, b) => {
    const sourceOrder = {
        "startup-folder": 0,
        run: 1,
        runonce: 2,
...
const sortApps = (apps) => [...apps].sort((a, b) => a.name.localeCompare(b.name));
const EVENT_LOG_LEVEL_PRIORITY = {
    critical: 0,
    error: 1,
    warning: 2,
    information: 3,
...
const normalizeEventLogLevel = (value) => {
...
const sortEventLogs = (entries) => [...entries].sort((a, b) => {
...
const countEventLogLevels = (entries) => {
    const counts = {
...
const toCountOrZero = (value) => {
    if (typeof value === "number" && Number.isFinite(value) && value >= 0) {
        return Math.floor(value);
    }
    return 0;
};
const normalizeEventLogCounts = (sourceCounts, entries, totalCount) => {
    ...
    const knownLevelsTotal = normalized.critical +
        normalized.error +
        normalized.warning +
...
const buildEmptyEventLogSnapshot = (capturedAt) => ({
    capturedAt,
...
const enrichInstalledApps = (installedAppsSnapshot, startupSnapshot, processSnapshot) => {
    const apps = installedAppsSnapshot.apps.map((app) => {
        ...
        const associatedProcesses = processSnapshot.processes.filter((process) => matchTokens([process.name, process.executablePath, process.commandLine, process.windowTitle].filter(Boolean).join(" "), tokens));
        const startupReferences = buildAssociatedStartups(app, [
...
const enrichStartup = (startupSnapshot, processSnapshot, installedAppsSnapshot) => {
    const launcherHints = [
        ...startupSnapshot.folderEntries,
        ...startupSnapshot.registryEntries,
        ...startupSnapshot.scheduledTaskEntries
        ...
        const associatedProcess = processSnapshot.processes.find((process) => matchTokens([process.name, process.executablePath, process.commandLine, process.windowTitle].filter(Boolean).join(" "), tokens));
        const associatedApp = installedAppsSnapshot.apps.find((app) => matchTokens([app.name, app.publisher, app.installLocation, app.uninstallCommand].filter(Boolean).join(" "), tokens));
        return {
            ...entry,
            source: "launcher-hint",
    ...
    return {
        ...
        launcherHints: [...startupSnapshot.launcherHints, ...launcherHints]
            .filter((entry, index, array) => index ===
            array.findIndex((candidate) => candidate.name === entry.name &&
...
const summarizeSettings = (settingsMap) => settingsMap.entries.slice(0, MAX_CONTEXT_LIST_ITEMS).map(toMapSummary);
const summarizeControlPanels = (controlPanelMap) => controlPanelMap.entries.slice(0, MAX_CONTEXT_LIST_ITEMS).map(toControlPanelSummary);
const summarizeRegistryZones = (registryZoneMap) => registryZoneMap.zones.slice(0, MAX_CONTEXT_LIST_ITEMS).map(toRegistryZoneSummary);
const summarizeProcesses = (processSnapshot) => sortProcesses(processSnapshot.processes)
    .slice(0, MAX_CONTEXT_LIST_ITEMS)
    .map((process) => {
    const memoryMb = process.memoryBytes ? Math.round(process.memoryBytes / (1024 * 1024)) : null;
    const cpuLabel = process.cpuPercent != null
...
const summarizeServices = (serviceSnapshot) => sortServices(serviceSnapshot.services)
    .slice(0, MAX_CONTEXT_LIST_ITEMS)
    .map((service) => `${service.serviceName} [${service.state}, ${service.startupType}]`);
const summarizeStartup = (startupSnapshot) => sortStartupEntries([
    ...startupSnapshot.folderEntries,
    ...startupSnapshot.registryEntries,
    ...startupSnapshot.scheduledTaskEntries,
    ...startupSnapshot.launcherHints
...
const summarizeApps = (installedAppsSnapshot) => sortApps(installedAppsSnapshot.apps)
    .slice(0, MAX_CONTEXT_LIST_ITEMS)
    .map((app) => app.name);
const summarizeEventLogs = (eventLogSnapshot) => sortEventLogs(eventLogSnapshot.entries)
    .slice(0, MAX_CONTEXT_LIST_ITEMS)
    .map((entry) => {
...
const summarizeUpdates = (updateCorrelation) => (updateCorrelation?.recentUpdates ?? [])
    .slice(0, MAX_CONTEXT_LIST_ITEMS)
    .map((entry) => (entry.kb ? `${entry.kb} (${entry.title})` : entry.title));
const buildHighlights = (snapshot) => ({
...
const findStartupEntriesForProcess = (startupSnapshot, process, appName) => {
    const tokens = extractTokens(process.name, process.executablePath, process.commandLine, process.windowTitle, appName);
    const entries = [
        ...startupSnapshot.folderEntries,
        ...startupSnapshot.registryEntries,
        ...startupSnapshot.scheduledTaskEntries,
    ...
    return uniqueValues(entries
        .filter((entry) => matchTokens([entry.name, entry.command, entry.target, entry.location].filter(Boolean).join(" "), tokens))
        .map((entry) => `${entry.source}:${entry.name}`)).slice(0, 4);
};
...
export const buildMachineCorrelationGraph = (snapshot) => {
    const processLinks = snapshot.processSnapshot.processes.map((process) => {
        const app = snapshot.installedAppsSnapshot.apps.find((candidate) => candidate.associatedProcessIds.includes(process.pid) ||
            candidate.associatedProcessNames.includes(process.name));
        const linkedServices = uniqueValues(snapshot.serviceSnapshot.services
    ...
    const appLinks = snapshot.installedAppsSnapshot.apps.map((app) => ({
        ...
        linkedServices: uniqueValues(snapshot.serviceSnapshot.services
...
export const buildMachineAwarenessSummary = (snapshot) => {
...
const queryIncludes = (query, keywords) => {
    const normalized = normalizeText(query);
    return keywords.some((keyword) => normalized.includes(normalizeText(keyword)));
};
const shouldIncludeMachineContext = (query, awarenessMode) => awarenessMode === "debug" ||
    queryIncludes(query, [
        "windows",
        "machine",
        "system",
...
const lookupSections = (snapshot, query) => {
    const sections = [];
    ...
    if (queryIncludes(query, relevanceKeywords.eventLogs)) {
        ...
        sections.push(`Event logs: ${snapshot.eventLogSnapshot.totalCount} entries (${errors} errors, ${warnings} warnings)${highlights ? ` | ${highlights}` : ""}`);
    }
    if (queryIncludes(query, relevanceKeywords.updates) && snapshot.updateCorrelation) {
        ...
        sections.push(`Windows updates: build ${snapshot.updateCorrelation.currentBuild ?? "unknown"}${snapshot.updateCorrelation.releaseChannel ? ` | channel ${snapshot.updateCorrelation.releaseChannel}` : ""}${recentUpdates ? ` | recent ${recentUpdates}` : ""}${knownIssues ? ` | known issues ${knownIssues}` : ""}${whatsNew ? ` | what's new ${whatsNew}` : ""}`);
    }
    if (queryIncludes(query, relevanceKeywords.settings)) {
        const matchedSettings = searchSettingsMapEntries(query, 4);
        const settingsSummary = matchedSettings.length > 0
    ...
    if (queryIncludes(query, relevanceKeywords.control)) {
        ...
        const controlSummary = matchedControlPanels.length > 0
    ...
    if (queryIncludes(query, relevanceKeywords.registry)) {
        ...
        const registrySummary = matchedZones.length > 0
...
export const buildMachineAwarenessContextSection = (snapshot, latestUserMessage, awarenessMode = "observe") => {
...
const trimContainer = (entries, limit) => {
...
export const captureMachineAwarenessSnapshot = async (options = {}) => {
    ...
    const [systemIdentity, processSnapshot, serviceSnapshot, startupSnapshot, installedAppsSnapshot, eventLogSnapshot, recentUpdatesResult] = await Promise.all([
    ...
    const trimmedProcesses = trimContainer(sortProcesses(cloneSnapshot(ensureArray(processSnapshot.processes))), options.maxProcesses ?? DEFAULT_MAX_PROCESSES);
    const trimmedServices = trimContainer(sortServices(cloneSnapshot(ensureArray(serviceSnapshot.services))), options.maxServices ?? DEFAULT_MAX_SERVICES);
    const trimmedFolderEntries = trimContainer(sortStartupEntries(cloneSnapshot(ensureArray(startupSnapshot.folderEntries))), options.maxStartupEntries ?? DEFAULT_MAX_STARTUP_ENTRIES);
    const trimmedRegistryEntries = trimContainer(sortStartupEntries(cloneSnapshot(ensureArray(startupSnapshot.registryEntries))), options.maxStartupEntries ?? DEFAULT_MAX_STARTUP_ENTRIES);
    const trimmedScheduledEntries = trimContainer(sortStartupEntries(cloneSnapshot(ensureArray(startupSnapshot.scheduledTaskEntries))), options.maxStartupEntries ?? DEFAULT_MAX_STARTUP_ENTRIES);
    const trimmedLauncherHints = trimContainer(sortStartupEntries(cloneSnapshot(ensureArray(startupSnapshot.launcherHints))), options.maxStartupEntries ?? DEFAULT_MAX_STARTUP_ENTRIES);
    const trimmedApps = trimContainer(sortApps(cloneSnapshot(ensureArray(installedAppsSnapshot.apps))), options.maxInstalledApps ?? DEFAULT_MAX_INSTALLED_APPS);
    const normalizedEventEntries = sortEventLogs(cloneSnapshot(ensureArray(eventLogSnapshot.entries)).map((entry) => ({
        ...entry,
        level: normalizeEventLogLevel(entry.level)
    })));
    const trimmedEventLogs = trimContainer(normalizedEventEntries, options.maxEventLogEntries ?? DEFAULT_MAX_EVENT_LOG_ENTRIES);
    ...
    const normalizedStartup = {
        ...
        totalCount: trimmedFolderEntries.totalCount +
    ...
    const normalizedEventLogs = {
        ...eventLogSnapshot,
        capturedAt,
        freshness,
        windowStartAt: eventLogSnapshot.windowStartAt || capturedAt,
        windowEndAt: eventLogSnapshot.windowEndAt || capturedAt,
        logs: [...new Set(ensureArray(eventLogSnapshot.logs).filter((value) => typeof value === "string" && value.trim().length > 0))],
        entries: trimmedEventLogs.entries,
        totalCount: Math.max(eventLogSnapshot.totalCount, trimmedEventLogs.totalCount),
        isTruncated: Boolean(eventLogSnapshot.isTruncated) ||
            trimmedEventLogs.isTruncated ||
            eventLogSnapshot.totalCount > trimmedEventLogs.entries.length,
        counts: normalizeEventLogCounts(eventLogSnapshot.counts, trimmedEventLogs.entries, Math.max(eventLogSnapshot.totalCount, trimmedEventLogs.totalCount))
    };
...
export { buildControlPanelMap, buildRegistryZoneMap, buildSettingsMap, createWindowsMachineInventorySource, searchControlPanelEntries, searchRegistryZoneEntries, searchSettingsMapEntries, toControlPanelSummary, toMapSummary, toRegistryZoneSummary };
