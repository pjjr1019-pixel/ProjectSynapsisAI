...
const cloneGovernanceCommand = (command) => JSON.parse(JSON.stringify(command));
const toResultStatus = (commandStatus) => {
...
const buildSummary = (status, decision, validationReason) => {
    switch (status) {
        case "executed":
...
const defaultExecuteRequest = async () => ({
    simulated: true,
    summary: "No execution handler registered. Returning simulation only."
});
export const createGovernanceCommandBus = (options = {}) => {
    ...
    const enqueueGovernanceCommand = (request) => {
    ...
    const processNextGovernanceCommand = async () => {
        ...
        if (decision.outcome === "require-approval" && !command.request.dryRun) {
            const validation = approvalLedger.validateApprovalToken(command.request.approvalToken, command.commandHash, now());
            if (!validation.valid) {
    ...
    const getGovernanceCommandStatus = (commandId) => {
        const command = commands.get(commandId);
...
export const enqueueGovernanceCommand = (request) => defaultGovernanceCommandBus.enqueueGovernanceCommand(request);
export const processNextGovernanceCommand = () => defaultGovernanceCommandBus.processNextGovernanceCommand();
export const getGovernanceCommandStatus = (commandId) => defaultGovernanceCommandBus.getGovernanceCommandStatus(commandId);
