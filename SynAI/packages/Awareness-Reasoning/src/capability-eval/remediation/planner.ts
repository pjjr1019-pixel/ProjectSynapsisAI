...
export interface RemediationPlannerInput {
  ...
  cardFilePath: string;
  gap: CapabilityGapClassification;
  verifier: CapabilityVerifierResult;
  execution: LocalAiEvalExecutionResult;
}

const summarizeVerifierReasons = (verifier: CapabilityVerifierResult): string =>
  verifier.reasons.length > 0 ? verifier.reasons.slice(0, 3).join(" | ") : "No explicit verifier reasons.";
...
export const planCapabilityRemediation = (
  input: RemediationPlannerInput
): CapabilityRemediationPlan => {
  ...
  switch (gap.primary_gap) {
    case "missing-knowledge":
      return {
        ...
        proposed_patch_summary:
          "Add canonical examples/snippets for this capability domain and strengthen prompt contract with explicit grounding hints.",
        risk_level: "medium",
    ...
    case "missing-retrieval":
      ...
      return {
        ...
        proposed_patch_summary:
          "Adjust retrieval metadata/index hints and enrich card context requirements to force deterministic evidence availability.",
        risk_level: "medium",
        approval_requirement: "operator-approval",
        follow_up_tests_to_rerun: [card.id],
        auto_patch: {
    ...
    case "missing-tool":
      return {
        ...
        proposed_patch_summary:
          "Expose missing safe tool bindings and update card allowed_tools/tool-selection verifier expectations.",
        risk_level: "high",
        approval_requirement: "operator-approval",
        follow_up_tests_to_rerun: [card.id],
        auto_patch: null
    ...
    case "missing-workflow-skill":
      return {
        ...
        proposed_patch_summary:
          "Add workflow wrapper for action planning and bind explicit workflow/tool expectations in card verifier config.",
        risk_level: "medium",
        approval_requirement: "operator-approval",
        follow_up_tests_to_rerun: [card.id],
        auto_patch: null
    ...
    case "bad-reasoning-planning":
      return {
        ...
        proposed_patch_summary:
          "Tighten structured-output planner contract and optionally provide richer deterministic context in card definitions.",
        risk_level: "medium",
        approval_requirement: "operator-approval",
        follow_up_tests_to_rerun: [card.id],
        auto_patch: null
    ...
    case "ambiguous-prompt-insufficient-context": {
      const missingHints =
        execution.output.missing_information.length > 0
      ...
      return {
        ...
        concrete_file_targets: [cardFilePath],
        proposed_patch_summary:
          "Add inline required_context clarification entry so the model has explicit constraints and missing information is reduced.",
        risk_level: "low",
        approval_requirement: "none",
        follow_up_tests_to_rerun: [card.id],
        auto_patch: {
    ...
    case "verifier-limitation":
      return {
        ...
        proposed_patch_summary:
          "Improve verifier constraints/hook reliability and rerun before changing model behavior.",
        risk_level: "low",
        approval_requirement: "none",
        follow_up_tests_to_rerun: [card.id],
        auto_patch: null
