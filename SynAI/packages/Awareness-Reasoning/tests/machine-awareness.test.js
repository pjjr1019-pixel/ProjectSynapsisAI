...
import { buildMachineAwarenessContextSection, buildMachineAwarenessSummary, captureMachineAwarenessSnapshot, createFixtureMachineInventorySource, findControlPanelEntry, findRegistryZoneEntry, findSettingsMapEntry } from "../src/machine";
    ...
    eventLogs: {
        ...
        entries: [
            ...
            },
            {
                ...
                opcode: null,
                machineName: "test-machine",
                message: "OneDrive.exe experienced a transient hang.",
                processId: 2222
            },
            {
                id: "event-3",
...
describe("machine awareness", () => {
    it("captures structured machine snapshots and keeps summaries compact", async () => {
        ...
        expect(snapshot.eventLogSnapshot.totalCount).toBe(3);
        ...
        expect(snapshot.systemIdentity.hardware.cpuLoadPercent).toBe(37);
        expect(snapshot.summary.summary).toContain("test-machine");
        expect(snapshot.summary.summary).toContain("cpu load 37%");
        expect(snapshot.summary.summary.length).toBeLessThan(220);
        expect(snapshot.summary.counts.eventLogErrors).toBe(2);
        expect(snapshot.summary.highlights.processes[0]).toContain("SynAI.exe#1111");
        expect(snapshot.summary.highlights.eventLogs[0]).toContain("ERROR");
    });
    ...
    it("serves the machine inventory endpoints on localhost", async () => {
        await withWorkspace(async (workspaceRoot) => {
            ...
            try {
                ...
                const eventLogResponse = await fetch(`${api.baseUrl}/api/awareness/system/event-logs`);
                expect(eventLogResponse.status).toBe(200);
                const eventLogs = (await eventLogResponse.json());
                expect(eventLogs.totalCount).toBe(3);
                expect(eventLogs.counts.error).toBe(2);
            }
            finally {
                await api.close();
            }
    ...
    it("builds a compact machine-aware context section for relevant queries", async () => {
        ...
        const eventSection = buildMachineAwarenessContextSection(snapshot, "show me recent event log errors", "observe");
        expect(eventSection).toContain("Event logs:");
