...
export interface ScreenRuntimePaths {
  runtimeRoot: string;
  screenRoot: string;
...
export interface ScreenCaptureOptions {
  ...
  now?: () => Date;
  defaultScope?: AssistModeScope;
  defaultTargetLabel?: string | null;
  defaultCaptureMode?: ScreenCaptureMode;
  defaultSampleIntervalMs?: number | null;
  maxRecentEvents?: number;
}

export interface ScreenAwarenessState {
  paths: ScreenRuntimePaths;
  get snapshot(): ScreenAwarenessSnapshot;
  get status(): ScreenAwarenessStatus;
  refresh(reason?: string): Promise<ScreenAwarenessSnapshot>;
  startAssist(options?: StartAssistModeOptions): Promise<ScreenAwarenessSnapshot>;
  stopAssist(reason?: string): Promise<ScreenAwarenessSnapshot>;
  close(): Promise<void>;
}

const DEFAULT_MAX_RECENT_EVENTS = 20;
const DEFAULT_CAPTURE_MODE: ScreenCaptureMode = "on-demand";
...
const buildRuntimePaths = (runtimeRoot: string): ScreenRuntimePaths => {
...
const createEvidenceRef = (pathValue: string, kind: EvidenceRef["kind"], label: string): EvidenceRef => ({
  id: pathValue,
  kind,
  label,
  path: pathValue
...
const persistSnapshot = async (paths: ScreenRuntimePaths, snapshot: ScreenAwarenessSnapshot): Promise<void> => {
  await writeJson(paths.currentScreenPath, snapshot);
  await writeJson(paths.latestSummaryPath, snapshot.summary);
};

const appendEvents = async (eventsPath: string, events: AppendAwarenessEventInput[]): Promise<AwarenessEvent[]> => {
  const persisted = await Promise.all(events.map((event) => appendAwarenessEvent(eventsPath, event)));
  return persisted;
};

const makeFingerprint = (snapshot: ScreenAwarenessSnapshot): string =>
  createHash("sha1")
    .update(
      JSON.stringify({
...
const trimEvents = (events: AwarenessEvent[], limit = DEFAULT_MAX_RECENT_EVENTS): AwarenessEvent[] =>
  events.slice(-limit);

const createAssistMode = (
  input: {
    enabled: boolean;
...
const emptySnapshot = (
  ...
  const recentEvents: AwarenessEvent[] = [];
  const snapshot: ScreenAwarenessSnapshot = {
    capturedAt: now.toISOString(),
    freshness: buildFreshness(now.toISOString(), now),
    assistMode,
...
const buildStatus = (snapshot: ScreenAwarenessSnapshot): ScreenAwarenessStatus => ({
  ...
  blockedScopes: snapshot.recentEvents
    ...
    .flatMap((event) => {
      ...
      return typeof scope === "string" ? [scope as PrivacyScope] : [];
    }),
  supportedScopes: ["current-window", "selected-app", "chosen-display"],
  lastEventType: snapshot.recentEvents.at(-1)?.type ?? null
});

const buildSummary = (snapshot: ScreenAwarenessSnapshot): ScreenAwarenessSnapshot["summary"] => ({
...
const compareWindow = (
  left: ForegroundWindowSnapshot | null,
  right: ForegroundWindowSnapshot | null
): boolean =>
  Boolean(
    left &&
      right &&
...
const compareNode = (
  left: ScreenUiTreeSnapshot["elementUnderCursor"],
  right: ScreenUiTreeSnapshot["elementUnderCursor"]
): boolean => {
  if (!left || !right) {
    return false;
  }
...
const buildEvents = (input: {
  ...
  reason: string;
  paths: ScreenRuntimePaths;
  previousSnapshot: ScreenAwarenessSnapshot | null;
  nextSnapshot: ScreenAwarenessSnapshot;
}): AppendAwarenessEventInput[] => {
  const events: AppendAwarenessEventInput[] = [];
  ...
  if (
    input.nextSnapshot.foregroundWindow &&
...
const captureOnce = async (
  source: ScreenCaptureSource,
  current: ScreenAwarenessSnapshot,
  paths: ScreenRuntimePaths,
  sessionId: string,
  reason: string,
  now: Date
): Promise<ScreenAwarenessSnapshot> => {
...
export const initializeScreenAwareness = async (
  ...
  const existing = await readJsonIfExists<ScreenAwarenessSnapshot>(paths.currentScreenPath);
  if (existing) {
  ...
  let snapshot = existing
    ? {
        ...
        foregroundWindow: null,
        uiTree: emptyTree(
          existing.assistMode.scope ?? options.defaultScope ?? "current-window",
          existing.assistMode.targetLabel ?? options.defaultTargetLabel ?? null,
          now()
        ),
        frame: emptyFrame(
          existing.assistMode.scope ?? options.defaultScope ?? "current-window",
          existing.assistMode.targetLabel ?? options.defaultTargetLabel ?? null,
          existing.assistMode.captureMode ?? options.defaultCaptureMode ?? DEFAULT_CAPTURE_MODE,
          now()
        ),
        recentEvents: []
      }
  ...
  const refresh = async (reason = "manual"): Promise<ScreenAwarenessSnapshot> => {
  ...
  const startAssist = async (assistOptions?: StartAssistModeOptions): Promise<ScreenAwarenessSnapshot> => {
    ...
    const persisted = await appendEvents(paths.eventsPath, [
      {
        ...
        affectedAreas: ["screen", "assist"] as AwarenessArea[],
        confidence: 1,
  ...
  const stopAssist = async (reason = "assist-disabled"): Promise<ScreenAwarenessSnapshot> => {
    stopPolling();
    const currentNow = now();
...
export const buildScreenAwarenessContextSection = (
  snapshot: ScreenAwarenessSnapshot | null | undefined,
  ...
  const likelyControl =
    snapshot.summary.highlights.controls.find((entry) => /(next|continue|ok|save|apply|submit|done|allow|confirm)/i.test(entry)) ??
