...
const waitForLoad = async (window, timeoutMs = 30_000) => {
...
const collectSnapshot = async (window) => {
    const text = await window.webContents.executeJavaScript(`(() => String(document.body ? document.body.innerText : "").slice(0, 8000))()`, true);
    const links = await window.webContents.executeJavaScript(`(() => Array.from(document.querySelectorAll('a[href]')).slice(0, 8).map((a) => ({
      title: String(a.innerText || a.textContent || a.getAttribute('aria-label') || '').trim(),
      url: String(a.href || '')
    })).filter((entry) => entry.url))()`, true);
    return {
...
const normalizeKeys = (keys) => keys.map((key) => key.trim()).filter(Boolean);
const createElectronFallbackBrowserHost = () => {
    let browserWindow = null;
    const ensureWindow = (visible = false) => {
    ...
    const open = async (url, visible = false) => {
    ...
    const search = async (query, maxResults = 5) => {
        ...
        const results = await window.webContents.executeJavaScript(`(() => Array.from(document.querySelectorAll('li.b_algo')).slice(0, ${Math.max(1, Math.min(maxResults, 8))}).map((item) => {
    ...
    const click = async (selectorOrText, visible = true) => {
        const window = ensureWindow(visible);
        if (visible) {
            window.show();
        }
    ...
    const type = async (selectorOrText, text, submit = false, visible = true) => {
        const window = ensureWindow(visible);
        if (visible) {
            window.show();
        }
    ...
    const hotkey = async (keys, visible = true) => {
        const window = ensureWindow(visible);
        if (visible) {
            window.show();
        }
        ...
        const keyCode = key.length === 1 ? key.toUpperCase() : key.replace(/^key/i, "").replace(/^digit/i, "").trim() || key;
        window.focus();
...
export const createElectronWorkflowBrowserHost = (options) => {
    ...
    const wrap = (operation, fallback) => operation().catch(async (error) => {
    ...
    return {
        search: async (query, maxResults) => await wrap(() => playwrightHost.search(query, maxResults), () => getFallback().search(query, maxResults)),
        open: async (url, visible) => await wrap(() => playwrightHost.open(url, visible), () => getFallback().open(url, visible)),
        openTab: async (url, visible) => await wrap(async () => playwrightHost.openTab ? await playwrightHost.openTab(url, visible) : await playwrightHost.open(url, visible), async () => getFallback().open(url, visible)),
        playYoutube: async (query) => await wrap(() => playwrightHost.playYoutube(query), () => getFallback().playYoutube(query)),
        click: async (selectorOrText, visible) => await wrap(() => playwrightHost.click(selectorOrText, visible), () => getFallback().click(selectorOrText, visible)),
        type: async (selectorOrText, text, submit, visible) => await wrap(() => playwrightHost.type(selectorOrText, text, submit, visible), () => getFallback().type(selectorOrText, text, submit, visible)),
        hotkey: async (keys, visible) => await wrap(() => playwrightHost.hotkey(keys, visible), () => getFallback().hotkey(keys, visible)),
        waitFor: async (selectorOrText, visible, timeoutMs) => await wrap(async () => (playwrightHost.waitFor ? await playwrightHost.waitFor(selectorOrText, visible, timeoutMs) : await playwrightHost.click(selectorOrText, visible)), async () => getFallback().click(selectorOrText, visible)),
        select: async (selectorOrText, value, visible) => await wrap(async () => (playwrightHost.select ? await playwrightHost.select(selectorOrText, value, visible) : await playwrightHost.type(selectorOrText, value, true, visible)), async () => getFallback().type(selectorOrText, value, true, visible)),
        download: async (url, fileName, visible) => await wrap(async () => (playwrightHost.download ? await playwrightHost.download(url, fileName, visible) : await playwrightHost.open(url, visible)), async () => getFallback().open(url, visible)),
        snapshot: async (visible) => await wrap(async () => (playwrightHost.snapshot ? await playwrightHost.snapshot(visible) : await playwrightHost.open("about:blank", visible)), async () => getFallback().open("about:blank", visible)),
        close: async () => {
