...
const boilerplateDocPattern = /(skip to main content|table of contents|download microsoft edge|ask learn|this browser is no longer supported)/i;
const isNoisyAwarenessLine = (value) => {
    const normalized = normalizeLine(value);
    return normalized.length > 220 || boilerplateDocPattern.test(normalized);
};
const focusTokensForQuery = (query) => {
...
const selectRelevantFollowUpLines = (lines, query, primaryLine, maxLines) => {
...
const formatHotspotReply = (answer) => {
    ...
    const bullets = hotspots
        .slice(0, 3)
        .map((entry) => `- ${entry.rank}. ${entry.label}: ${entry.resourceAmount}${entry.resourceShare != null ? ` (${Math.round(entry.resourceShare * 100)}% of ${shareLabel})` : ""}`);
...
const formatGeneralAwarenessReply = (answer) => {
...
export const formatAwarenessReply = (answer) => {
    ...
    if (answer.intent.family === "settings-control-panel" ||
