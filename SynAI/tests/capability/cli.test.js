...
import { parseCapabilityCliArgs } from "../../packages/Awareness-Reasoning/src/capability-eval/cli";
describe("capability cli parser", () => {
    it("parses run command with card id and mode", () => {
        const parsed = parseCapabilityCliArgs(["run", "--card-id", "windows.highest-ram-process", "--mode", "sandbox-apply", "--auto-remediate"], "C:/repo/SynAI");
    ...
    it("parses json/token flags and issue-token command", () => {
        const parsedRun = parseCapabilityCliArgs(["run", "--card-id", "windows.highest-ram-process", "--approval-token", "{\"tokenId\":\"1\",\"commandHash\":\"x\",\"approver\":\"a\",\"issuedAt\":\"i\",\"expiresAt\":\"e\",\"signature\":\"s\"}", "--json"], "C:/repo/SynAI");
        expect(parsedRun.json).toBe(true);
        expect(parsedRun.approvalToken).toContain("\"tokenId\"");
        const parsedIssue = parseCapabilityCliArgs(["issue-token", "--command-hash", "abc123", "--approved-by", "qa"], "C:/repo/SynAI");
        ...
        const parsedHash = parseCapabilityCliArgs(["promotion-hash", "--card-id", "windows.test", "--target-path", "packages/Capability-Catalog/cards/windows/test.json"], "C:/repo/SynAI");
        ...
        const parsedHistory = parseCapabilityCliArgs(["mine-history", "--max-findings", "3", "--json"], "C:/repo/SynAI");
