...
import {
  detectFailureFeedback,
  resolveBrowserNavigationTarget,
  resolveProcessFollowUpPrompt
} from "@awareness/target-knowledge";
import type { GovernedHistoryFinding, GovernedTaskPlanRequest, GovernedTaskPlanResult } from "./types";
...
const isBrowserAutomationPrompt = (normalized: string): boolean =>
...
const isServiceControlPrompt = (normalized: string): boolean =>
  containsAny(normalized, [
    "start service",
    "stop service",
    "restart service",
...
const isRegistryControlPrompt = (normalized: string): boolean =>
  containsAny(normalized, [
    "registry value",
    "registry key",
    "registry editor",
...
const isUiAutomationPrompt = (normalized: string): boolean =>
  containsAny(normalized, [
    "click ",
    "click the",
    "press ",
...
const riskTierFromAction = (actionType: string | null): ChatGovernedTaskRiskTier => {
  if (!actionType) {
    return "tier-0";
  }
...
const defaultApprovalState = (reason: string | null, approvedBy: string | null = null): ChatGovernedTaskApprovalState => ({
...
const detectConfirmation = (normalized: string): boolean =>
  /^(approve|approved|confirm|confirmed|proceed|proceed with it|yes|yes please|go ahead|do it|run it|continue)$/.test(
    normalized
  );

const detectAnswerOnlyPrompt = (normalized: string): boolean => {
  const actionWords = [
    "open",
    "launch",
    "start",
...
const detectWorkflowIntent = (normalized: string): boolean =>
  containsAny(normalized, [
    "research",
    "report",
    "computer health",
...
const detectDesktopIntent = (normalized: string): boolean =>
  containsAny(normalized, [
    "open ",
    "launch ",
    "start ",
...
const detectAmbiguity = (normalized: string): boolean => {
...
const detectApprovalNeed = (normalized: string): string | null => {
...
const isSystemNavigationPrompt = (normalized: string): boolean =>
  containsAny(normalized, [
    "open settings",
...
const isBrowserNavigationPrompt = (rawText: string): boolean => resolveBrowserNavigationTarget(rawText) !== null;

const isApplicationLaunchPrompt = (normalized: string): boolean =>
  (containsAny(normalized, ["open", "launch", "start", "run"]) ||
...
const isFileWorkflowPrompt = (normalized: string): boolean =>
  containsAny(normalized, [
    "open folder",
    "open directory",
    "open file directory",
...
const isProcessWorkflowPrompt = (normalized: string): boolean =>
  containsAny(normalized, ["kill", "terminate", "end task", "stop task"]) && containsAny(normalized, ["process", "processes", "task manager"]);

const isCloseWorkflowPrompt = (normalized: string): boolean =>
  containsAny(normalized, ["close", "quit", "exit", "shut down", "shutdown"]);

const detectExecutor = (actionType: string | null): ChatGovernedTaskExecutor => {
  if (!actionType) {
    return "answer-only";
  }
...
const summarizeSignals = (signals: string[]): string =>
  signals.length > 0 ? signals.join(" | ") : "No special routing signals matched.";

const buildClarificationPlan = (
  request: GovernedTaskPlanRequest,
  reason: string
...
const resolveHistoryReplay = (
  normalizedText: string,
  historyFindings: GovernedHistoryFinding[]
): GovernedHistoryFinding | null => {
...
export interface GovernedChatRouterInput {
  request: GovernedTaskPlanRequest;
  historyFindings?: GovernedHistoryFinding[];
}
...
export const routeGovernedChatTask = async (
  ...
  if (detectAmbiguity(normalizedText) && !processFollowUpResolution && !explicitFailureFeedback) {
    return buildClarificationPlan(
      input.request,
      "The request is executable, but the target is ambiguous. Please name the exact file, folder, app, process, or setting."
    );
  }
  ...
  const systemNavigationApprovalExempt =
  ...
  const workflowIntent =
    (detectWorkflowIntent(normalizedText) && !registryNavigationIntent) ||
    ...
    uiAutomationIntent;
  const desktopIntent =
    detectDesktopIntent(normalizedText) ||
    serviceControlIntent ||
    registryControlIntent ||
    uiAutomationIntent ||
    systemNavigationIntent;
  ...
  const decision: ChatGovernedTaskDecision = approvalRequired ? "require_approval" : workflowIntent || desktopIntent ? "allow_with_verification" : "allow";
  ...
  if (!workflowIntent && !desktopIntent) {
    return buildClarificationPlan(
      input.request,
      "I can answer this directly, but I do not yet have a deterministic executable path for it."
    );
  }

  if (approvalRequired && riskTier === "tier-4") {
    return buildDeniedPlan(
      input.request,
      "The request maps to a prohibited or too-broad system-changing action.",
      "tier-4-block"
    );
  }
...
export const buildGovernedTaskMetadata = (requestId: string, route: GovernedTaskPlanResult): ChatGovernedTaskMetadata => ({
  requestId,
