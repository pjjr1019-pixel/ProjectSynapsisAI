...
## Purpose
Provide the canonical package architecture for the typed SynAI agent runtime. The runtime now includes the original foundation plus the smallest real 5-layer implementation that can be wired into SynAI without breaking existing chat, memory, workflow, governance, or eval behavior.

## Current Scope
- Canonical contracts live in `SynAI/packages/Agent-Runtime/src/contracts/agent-runtime.contracts.ts`.
- The canonical runtime implementation lives in `SynAI/packages/Agent-Runtime/src/*`.
- `@synai-agent` remains a thin compatibility import surface over the canonical package.
...
- Resume and recover are checkpoint-backed; when exact continuation is not possible, the runtime records a typed reconstructive continuation instead of pretending a full replay.
...
- Keep the runtime implementation in `SynAI/packages/Agent-Runtime/src`.
- Keep SynAI-facing compatibility imports behind the `@synai-agent` alias.
