...
const summarizeTrend = (label, previous, current) => {
...
const buildMetricSample = (snapshot) => {
    const hardware = snapshot.systemIdentity.hardware;
    const ramUsedBytes = hardware.memory.totalBytes > 0 && hardware.memory.availableBytes != null
    ...
    const topDiskProcess = topBy(snapshot.processSnapshot.processes, (process) => (process.ioReadBytesPerSec ?? 0) + (process.ioWriteBytesPerSec ?? 0) || (process.ioReadBytes ?? 0) + (process.ioWriteBytes ?? 0));
    const primaryDrive = hardware.drives.find((drive) => drive.deviceId?.toUpperCase().startsWith("C:")) ?? hardware.drives[0] ?? null;
...
export const updateRollingMetrics = (current, snapshot, now = new Date()) => {
...
const sameProcess = (left, right, read) => {
    if (!left || !right) {
        return null;
    }
...
const topProcessName = (snapshot, selector) => {
    if (!snapshot) {
        return null;
    }
    const process = selector === "ram"
...
const topEventProvider = (snapshot) => {
    if (!snapshot) {
        return null;
    }
...
export const buildRecurringPatterns = (input) => {
    ...
    const repeatedRam = sameProcess(input.current.machineAwareness, previous, (snapshot) => topProcessName(snapshot, "ram"));
    if (repeatedRam) {
        patterns.push({
    ...
    const repeatedCpu = sameProcess(input.current.machineAwareness, previous, (snapshot) => topProcessName(snapshot, "cpu"));
    ...
    const repeatedProvider = sameProcess(input.current.machineAwareness, previous, (snapshot) => topEventProvider(snapshot));
    if (repeatedProvider) {
        patterns.push({
...
const buildRepoDeltaHighlight = (current, previous) => {
    if (!previous) {
...
const buildUpdateHighlight = (snapshot) => {
...
export const buildStartupDigest = (input) => {
    ...
    return {
        ...
        safeNextAction: input.current.repo.workingTree.totalCount > 0
            ? "Review the current repo and hotspot deltas."
            : "Ask what changed, what is using resources, or what stands out."
    };
};
export const buildRollingMetricsSummaryLines = (rollingMetrics) => {
    if (!rollingMetrics) {
        return [];
    }
