...
import type { CapabilityPatchMode, CapabilityRunnerOptions, CapabilityRunSummary } from "../types";

export interface ParsedCapabilityCliArgs {
  command: "run" | "rerun-failed" | "issue-token" | "promotion-hash" | "mine-history";
  cardIds: string[];
...
const nextArg = (args: string[], index: number, flag: string): string => {
...
const parseMode = (value: string): CapabilityPatchMode => {
...
export const parseCapabilityCliArgs = (
  argv: string[],
  cwd = process.cwd()
): ParsedCapabilityCliArgs => {
  const args = [...argv];
  ...
  const command: ParsedCapabilityCliArgs["command"] =
    commandToken === "rerun-failed"
      ? "rerun-failed"
  ...
  let mode: CapabilityPatchMode = "proposal-only";
  let dryRun = false;
  ...
  let json = false;
  let cardsRoot: string | undefined;
...
const toRunnerOptions = (args: ParsedCapabilityCliArgs): CapabilityRunnerOptions => {
...
const printSummary = (summary: CapabilityRunSummary): void => {
  const lines = [
    ...
    ...summary.cardResults.map(
      (card) =>
        `- ${card.status.toUpperCase()} ${card.cardId} | artifact: ${card.artifactDir}${
          card.sandbox?.rerunResult ? ` | rerun: ${card.sandbox.rerunResult.passed ? "pass" : "fail"}` : ""
        }`
    )
  ];
...
export const runCapabilityCli = async (argv: string[]): Promise<number> => {
  let useJson = false;
  try {
      ...
      if (parsed.json) {
        ...
        console.log(
          [
            ...
            ...result.findings.map(
              (finding) =>
                `- ${finding.recovered_intent} | repeated ${finding.repeated_request_count} | ${finding.suggested_gap}`
            )
          ].join("\n")
        );
      }
    ...
    if (parsed.command === "promotion-hash") {
      ...
      const hash = hashGovernanceCommand(
        createGovernedPromotionHashInput(parsed.cardIds[0], parsed.targetPath)
      );
      if (parsed.json) {
        // eslint-disable-next-line no-console
        console.log(
