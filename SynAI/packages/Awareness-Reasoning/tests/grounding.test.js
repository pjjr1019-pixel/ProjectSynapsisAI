...
import { buildGroundingSourceCatalog, groundAssistantReply, segmentAnswerClaims } from "../src/reasoning/grounding";
const freshness = {
...
describe("grounding", () => {
    it("skips section titles but keeps structured bullet claims", () => {
        const claims = segmentAnswerClaims(["Built now", "- local Ollama chat", "Not built yet", "- cloud sync and multi-agent systems"].join("\n"));
    ...
    it("grounds short deterministic Windows settings answers from synthetic awareness bundle sources", async () => {
        ...
        expect(grounded.metadata.matchedEvidenceIds).toEqual(expect.arrayContaining(["awareness:bundle:answer-1"]));
