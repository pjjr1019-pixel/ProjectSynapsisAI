...
import { buildWorkflowPlan } from "./workflow-planner";
import { createElectronWorkflowBrowserHost } from "./browser-session";
const normalizePrompt = (value) => value
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
    .trim();
const containsAny = (value, needles) => needles.some((needle) => value.includes(needle));
const resolveOutputPath = (plan, workspaceRoot) => plan.targetPaths[0] ?? (plan.artifacts[0]?.fileName ? path.resolve(workspaceRoot, plan.artifacts[0].fileName) : null);
...
const collectRollbackRecord = (target, rollback) => {
    if (rollback) {
        target.push(rollback);
    }
};
const createWorkflowVerificationRecord = (input) => {
    const failedStep = input.stepResults.find((step) => step.status === "failed" || step.status === "blocked" || step.status === "denied") ?? null;
    const passed = (input.status === "executed" || input.status === "simulated") &&
        !failedStep &&
...
const buildCompensationSummary = (step, result) => result.status === "executed"
...
const runBrowserInteractionStep = async (step, browserHost) => {
...
const buildSourcesSection = (webResults, browserResults) => {
    const sources = [];
    if (webResults?.status === "used") {
        for (const result of webResults.results.slice(0, 5)) {
            sources.push(`- [${result.title}](${result.url}) - ${result.sourceFamily ?? "web"} | ${result.source}${result.publishedAt ? ` | ${result.publishedAt}` : ""}`);
        }
    }
...
const createMaintenanceReviewMarkdown = (input) => {
    const driveLines = input.machine?.systemIdentity.hardware.drives.length
        ? input.machine.systemIdentity.hardware.drives.slice(0, 4).map((drive) => `- ${drive.deviceId}${drive.volumeLabel ? ` (${drive.volumeLabel})` : ""}: ${formatBytes(drive.freeBytes)} free / ${formatBytes(drive.totalBytes)} total`)
        : ["- No drive snapshot available."];
    const startupLines = input.machine?.startupSnapshot.folderEntries.length ||
    ...
    const processLines = input.machine?.processSnapshot.processes.length
        ? input.machine.processSnapshot.processes.slice(0, 5).map((process) => `- ${process.name} (PID ${process.pid}) | ${process.windowTitle ?? "no window"} | ${formatBytes(process.memoryBytes)}`)
        : ["- No process snapshot available."];
    const serviceLines = input.machine?.serviceSnapshot.services.length
        ? input.machine.serviceSnapshot.services.slice(0, 5).map((service) => `- ${service.displayName} [${service.state}, ${service.startupType}]`)
        : ["- No service snapshot available."];
    const installedAppLines = input.machine?.installedAppsSnapshot.apps.length
        ? input.machine.installedAppsSnapshot.apps.slice(0, 5).map((app) => `- ${app.name}${app.version ? ` (${app.version})` : ""}`)
...
const createWorkflowStepRequest = (plan, step, workspaceRoot, approvedBy, approvalToken, dryRun, actions) => {
    if (!step.actionId) {
        return null;
    }
...
export const createWorkflowOrchestrator = (options) => {
    ...
    const browserHost = options.browserHost ?? createElectronWorkflowBrowserHost({ runtimeRoot, preferPlaywright: true });
    ...
    const issueWorkflowApproval = async (plan, approvedBy, ttlMs) => {
        const request = {
    ...
    const executePlan = async (plan, approvedBy, dryRun, workflowId) => {
        const stepResults = [];
        ...
        let failure = null;
        const pushProgress = (status, currentStepId, currentStepIndex, summary) => {
            emitProgress({
                workflowId,
                workflowHash: plan.workflowHash,
                plan,
        ...
        for (const [index, step] of plan.steps.entries()) {
            ...
            try {
                if (step.kind === "confirm-approval") {
                    ...
                    stepResults.push({
                        ...
                        summary: webContext?.status === "used"
                    ...
                    stepResults.push({
                        ...
                        kind: step.kind,
                        status: "executed",
                        summary: typeof step.metadata?.action === "string" && step.metadata.action === "type"
                ...
                else if ((step.kind === "desktop-action" ||
                    step.kind === "service-action" ||
                    step.kind === "registry-action" ||
                    step.kind === "ui-action") &&
                    step.actionId) {
                    const request = createWorkflowStepRequest(plan, step, workspaceRoot, approvedBy, null, dryRun, options.desktopActions);
                    if (!request) {
            ...
            const stepStatus = actionResult.status === "executed"
                ...
                : actionResult.status === "simulated"
                    ...
                    const maintenanceMode = typeof plan.artifacts[0]?.metadata?.mode === "string"
                        ? String(plan.artifacts[0].metadata.mode)
                        : "cleanup";
                    const reportBody = plan.family === "computer-health-report"
                ...
                pushProgress(dryRun ? "simulated" : "executed", step.id, index, stepResults[stepResults.length - 1]?.summary ?? step.summary);
            }
            catch (error) {
        ...
        if (failure) {
            ...
            const summary = failure.status === "denied"
        ...
        const summary = plan.family === "research-report"
    ...
    const executeWorkflow = async (request) => {
        ...
        const command = {
            ...
            handler: async () => {
                const executionResult = await executePlan(plan, request.approvedBy ?? "workflow-operator", Boolean(request.dryRun), plan.requestId);
                return {
        ...
        void approvalQueue?.record({
            ...
            scope: null,
            targetKind: null,
            status: result.status === "executed" || result.status === "simulated"
                ? "consumed"
                : result.status === "denied"
                    ? "denied"
                    : "blocked",
