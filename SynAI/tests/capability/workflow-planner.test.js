...
import { buildWorkflowPlan } from "../../apps/desktop/electron/workflow-planner";
const chromeApp = {
    name: "Google Chrome",
...
describe("workflow planner", () => {
    it("builds a research workflow that saves a report in Documents", async () => {
        const plan = await buildWorkflowPlan("research the current state of AI and write a report and save it in my documents", buildContext({
        ...
        expect(plan.steps.map((step) => step.kind)).toEqual(expect.arrayContaining(["collect-web", "browser-open", "write-markdown"]));
    });
    it("builds a computer-health report workflow that saves to Desktop", async () => {
        const plan = await buildWorkflowPlan("give me a report on my computers health and save it in a md file on my desktop", buildContext());
        ...
        expect(plan.steps.map((step) => step.kind)).toEqual(expect.arrayContaining(["collect-machine", "collect-file", "write-markdown"]));
    });
    ...
    it("builds the remaining history-failed workflows for registry, files, closing, and desktop organization", async () => {
        ...
        expect(organizePlan.steps.map((step) => step.kind)).toEqual(expect.arrayContaining(["collect-machine", "collect-file", "write-markdown"]));
    });
    it("builds a bulk desktop organization workflow from the actual desktop contents", async () => {
        ...
        try {
            const plan = await buildWorkflowPlan("move everything on my desktop into a new folder named organize laster", buildContext({
                desktopPath,
                fileAwareness: {
                    counts: {
                        files: 3,
    ...
    it("builds a file workflow that creates, renames, and moves a file", async () => {
        const plan = await buildWorkflowPlan("create a file on my desktop rename it to bob then move it into my document folder", buildContext());
        expect(plan.family).toBe("file-management");
    ...
    it("changes the workflow hash when the plan content changes", async () => {
        const planA = await buildWorkflowPlan("research the current state of AI and write a report and save it in my documents", buildContext({
            recentWebContext: buildWebContext("State of AI 2026")
        }));
        const planB = await buildWorkflowPlan("research the current state of AI and write a report and save it in my documents", buildContext({
