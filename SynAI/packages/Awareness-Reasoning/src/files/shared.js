...
export const jsonFingerprint = (value) => digestHash(JSON.stringify(value));
export const normalizePathForMatch = (value) => path.normalize(value);
export const lowerPath = (value) => normalizePathForMatch(value).toLowerCase();
export const pathSegments = (value) => lowerPath(value)
    .split(/[\\/]+/)
    .filter(Boolean);
export const compactPath = (value, segments = 2) => {
...
export const createObservationFreshness = (capturedAt, observedAt, staleAfterMs = FILE_FRESHNESS_WINDOW_MS, now = new Date()) => {
...
export const isWithinPath = (candidate, parentPath) => {
...
export const walkAncestors = (folderPath, rootPath, callback) => {
...
export const buildFileAwarenessRuntimePaths = (runtimeRoot) => ({
    runtimeRoot,
...
export const extensionFromPath = (filePath) => {
    const ext = path.extname(filePath).toLowerCase();
    return ext || null;
};
export const inferMediaKind = (extension) => {
    if (!extension) {
        return null;
    }
    return mediaExtToKind[extension.toLowerCase()] ?? null;
};
export const inferMimeType = (extension, mediaKind) => {
...
export const inferContentHint = (extension, mediaKind) => {
    if (mediaKind) {
        return mediaKind;
    }
    if (!extension) {
    ...
    if ([".ts", ".tsx", ".js", ".jsx", ".css", ".html", ".json", ".xml", ".yaml", ".yml", ".md", ".txt", ".csv", ".log"].includes(extension)) {
        return "text";
    }
...
export const inferTags = (filePath, mediaKind) => {
    const tokens = path
        .basename(filePath)
        .replace(/\.[^.]+$/, "")
        .split(/[^a-z0-9]+/i)
    ...
    const tags = new Set(tokens.slice(0, 4));
    if (mediaKind) {
        tags.add(mediaKind);
    }
    return [...tags];
};
export const classifyPrivacyScope = (filePath, rootSource, scanRoot) => {
...
export const isPubliclyVisibleScope = (privacyScope) => privacyScope === "public metadata" || privacyScope === "user-visible local content";
export const shouldHashFile = (filePath, sizeBytes, privacyScope) => {
...
export const chooseRoots = (options) => {
    const candidates = [];
...
export const createImagePreviewRef = (filePath) => ({
    id: filePath,
    kind: "file",
    label: "preview",
    path: filePath
