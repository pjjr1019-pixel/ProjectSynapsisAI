...
const buildWorkflowExecutionResult = (plan) => {
    const stepResult = {
        id: "step-1",
        kind: "write-markdown",
        status: "executed",
...
const buildDesktopActionHost = () => ({
    launchProgram: vi.fn(async (target) => ({ target })),
    openTarget: vi.fn(async (target) => ({ target })),
    createFile: vi.fn(async (target, content) => ({
        target,
    ...
    terminateProcess: vi.fn(async (target, targetKind, force) => ({
        target,
        targetKind,
        force
    })),
    ...
    closeWindowGracefully: vi.fn(async (target) => ({ target })),
    focusWindow: vi.fn(async (target) => ({ target }))
});
describe("governed chat service", () => {
    it("clarifies ambiguous destructive prompts instead of executing them", async () => {
        ...
        expect(result.assistantReply).toContain("exact target");
