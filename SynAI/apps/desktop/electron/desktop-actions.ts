...
const execPowerShellScript = async (script: string): Promise<{ stdout: string; stderr: string }> =>
...
const resolveProgramTarget = (target: string, workspaceRoot: string): string => {
  const trimmed = target.trim();
  if (!trimmed) {
    throw new Error("target is required");
  }
...
const scoreInstalledApp = (app: InstalledAppEntry, target: string): number => {
...
const resolveInstalledApp = (
  snapshot: InstalledAppsSnapshot | null | undefined,
  target: string
): InstalledAppEntry | null => {
  if (!snapshot) {
    return null;
  }
...
const renderPreview = (definition: WindowsActionDefinition, request: DesktopActionRequest): string => {
...
const buildRequestMetadata = (
  ...
  normalizedDestinationTarget: string | null
...
const normalizeDesktopActionRequest = (
  request: DesktopActionRequest,
  workspaceRoot: string
): { proposal: WindowsActionDefinition | null; normalizedTarget: string; normalizedDestinationTarget: string | null } => {
  ...
  const normalizedDestinationTarget =
    request.destinationTarget && request.destinationTarget.trim().length > 0
...
const buildClarificationNeededDesktopResult = (
  request: DesktopActionRequest,
  workspaceRoot: string,
  ...
  return {
    ...
    verification: createVerificationRecord(
      summary,
      {
        kind: request.kind,
        missingFields: ["target"],
...
const buildRollbackBackupPath = (runtimeRoot: string, commandId: string, target: string): string =>
  path.join(runtimeRoot, "rollback", commandId, sanitizePathSegment(target));
...
const extractRollbackRecord = (value: unknown): ExecutionRollbackRecord | null => {
  if (!value || typeof value !== "object") {
    return null;
  }

  const rollback = (value as { rollback?: unknown }).rollback;
  if (!rollback || typeof rollback !== "object") {
    return null;
  }

  const record = rollback as ExecutionRollbackRecord;
...
const normalizeServiceState = (value: string | null | undefined): string =>
  value?.trim().toLowerCase() ?? "";

const resolveServiceRollbackAction = (
...
export interface DesktopActionHost {
  launchProgram(target: string, args: string[], workingDirectory?: string | null): Promise<unknown>;
  openTarget(target: string): Promise<unknown>;
  focusWindow(target: string, targetKind: DesktopActionTargetKind): Promise<unknown>;
  closeWindowGracefully(target: string, targetKind: DesktopActionTargetKind): Promise<unknown>;
  createFile(target: string, content: string): Promise<unknown>;
  createFolder(target: string): Promise<unknown>;
  deletePath(target: string, recursive: boolean): Promise<unknown>;
  renamePath(target: string, destination: string): Promise<unknown>;
  movePath(target: string, destination: string): Promise<unknown>;
  inspectProcess(target: string, targetKind: DesktopActionTargetKind): Promise<unknown>;
  terminateProcess(target: string, targetKind: DesktopActionTargetKind, force: boolean): Promise<unknown>;
  controlService(target: string, action: "start" | "stop" | "restart"): Promise<unknown>;
  setRegistryValue(target: string, valueName: string, value: string, valueKind?: string | null): Promise<unknown>;
  deleteRegistryValue(target: string, valueName: string): Promise<unknown>;
  invokeUiAction(
    target: string,
  ...
  openTaskManager(): Promise<unknown>;
}

export const createWindowsDesktopActionHost = (): DesktopActionHost => ({
...
`;
    const { stdout } = await execPowerShellScript(script);
    return stdout.trim().length > 0 ? (JSON.parse(stdout) as unknown) : { target, action };
  },
...
`;
    const { stdout } = await execPowerShellScript(script);
    return stdout.trim().length > 0 ? (JSON.parse(stdout) as unknown) : { target, valueName, value };
  },
...
`;
    const { stdout } = await execPowerShellScript(script);
    return stdout.trim().length > 0 ? (JSON.parse(stdout) as unknown) : { target, valueName };
  },
...
const createRollbackRecord = (record: ExecutionRollbackRecord): ExecutionRollbackRecord => ({
  ...record,
...
const createVerificationRecord = (
  summary: string,
...
const buildApprovalQueueRecord = (
  request: DesktopActionRequest,
  commandHash: string,
...
const performDesktopAction = async (
  request: DesktopActionRequest,
  host: DesktopActionHost,
  workspaceRoot: string,
  runtimeRoot: string,
  commandId: string,
  getInstalledAppsSnapshot: () => InstalledAppsSnapshot | null
  ...
  switch (request.kind) {
    case "launch-program":
      return {
        ...
        output: await host.launchProgram(
    ...
    case "ui-hotkey":
      ...
      return {
        ...
        output: {
          ...asRecord(
            await host.invokeUiAction(
              normalized.normalizedTarget,
              request.kind === "ui-click" ? "click" : request.kind === "ui-type" ? "type" : "hotkey",
              {
            ...
            keys: Array.isArray(request.metadata?.keys)
              ? request.metadata.keys.filter((entry): entry is string => typeof entry === "string")
              : null,
    ...
    case "restart-service":
      ...
      const serviceOutput = await host.controlService(
        normalized.normalizedTarget,
        request.kind === "start-service" ? "start" : request.kind === "stop-service" ? "stop" : "restart"
      );
      const serviceResult = serviceOutput as {
        action?: string;
        serviceName?: string;
        displayName?: string;
      ...
      const rollbackAction = resolveServiceRollbackAction(
        serviceResult.before?.state ?? null,
        serviceResult.after?.state ?? null
      );
    ...
    case "set-registry-value": {
      ...
      const registryResult = (await host.setRegistryValue(
        normalized.normalizedTarget,
        valueName,
        requestedValue,
        valueKind
    ...
    case "delete-registry-value": {
      ...
      const valueName = typeof request.metadata?.valueName === "string" ? request.metadata.valueName : "Default";
      const valueKind = typeof request.metadata?.valueKind === "string" ? request.metadata.valueKind : "String";
      const registryResult = (await host.deleteRegistryValue(normalized.normalizedTarget, valueName)) as {
        targetPath?: string;
        valueName?: string;
        previousValue?: unknown;
        deleted?: boolean;
    ...
    case "terminate-process":
      ...
      return {
        ...
        output: await host.terminateProcess(
          normalized.normalizedTarget,
          request.targetKind,
          Boolean(request.metadata?.force)
        )
      };
    case "uninstall-app": {
      if (!isWindows) {
...
const toDesktopActionResult = (
  ...
  verification:
    ...
    createVerificationRecord(
      ...
      response.status === "executed" || response.status === "simulated"
    ),
  error: response.error
});

export interface DesktopActionServiceOptions {
  workspaceRoot: string;
  runtimeRoot: string;
...
export interface DesktopActionService {
  listDesktopActions: () => DesktopActionProposal[];
  suggestDesktopAction: (prompt: string) => DesktopActionProposal | null;
...
export const createDesktopActionService = (options: DesktopActionServiceOptions): DesktopActionService => {
  ...
  const listDesktopActions = (): DesktopActionProposal[] =>
    listWindowsActionDefinitions().map((definition) => ({
      ...definition,
      aliases: [...definition.aliases],
      preconditions: [...definition.preconditions]
    }));

  const suggestDesktopAction = (prompt: string): DesktopActionProposal | null => {
  ...
  const issueDesktopActionApproval = (
    request: DesktopActionRequest,
    approvedBy: string,
    ttlMs?: number
  ): ApprovalToken => {
  ...
  const executeDesktopAction = async (request: DesktopActionRequest): Promise<DesktopActionResult> => {
    let normalized: {
      proposal: WindowsActionDefinition | null;
      normalizedTarget: string;
      normalizedDestinationTarget: string | null;
    };
    try {
      ...
      if (isMissingTargetError(error)) {
        return buildClarificationNeededDesktopResult(
          request,
          workspaceRoot,
  ...
  const buildRollbackDesktopRequest = (command: GovernanceCommand): DesktopActionRequest | null => {
    ...
    const metadata = (command.request.metadata ?? {}) as Record<string, unknown>;
    ...
    const originalKind = (typeof metadata.kind === "string" ? metadata.kind : originalProposal?.kind) as DesktopActionKind | undefined;
    ...
    const originalTargetKind = (typeof metadata.targetKind === "string" ? metadata.targetKind : originalProposal?.targetKind) as DesktopActionTargetKind | undefined;
    const originalScope = (typeof metadata.scope === "string" ? metadata.scope : originalProposal?.scope) as DesktopActionRequest["scope"] | undefined;
    ...
    const serviceRollbackAction = typeof rollbackRecord.metadata?.rollbackAction === "string"
      ? (rollbackRecord.metadata.rollbackAction as "start" | "stop")
      : null;
    ...
    switch (originalKind) {
      ...
      case "restart-service": {
        ...
        const inverseKind: DesktopActionKind = serviceRollbackAction === "start" ? "start-service" : "stop-service";
        return {
  ...
  const rollbackDesktopAction = async (
    commandId: string,
    approvedBy: string,
    dryRun = false
  ...
  return {
    ...
    getGovernanceCommandStatus: (commandId: string) => commandBus.getGovernanceCommandStatus(commandId)
  };
};

export const resolveDefaultDesktopActionPaths = (workspaceRoot: string): {
  runtimeRoot: string;
  workspaceRoot: string;
} => ({
