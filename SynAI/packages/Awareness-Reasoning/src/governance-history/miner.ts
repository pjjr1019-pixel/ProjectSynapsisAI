...
const isTaskLike = (value: string): boolean =>
  /(?:\b(open|launch|start|close|kill|terminate|stop|delete|remove|create|rename|move|install|uninstall|research|report|save|play|inspect|check|fix|clean|organize)\b)/i.test(
    value
  );
...
const extractSignature = (userMessage: string, assistantMessage: ChatMessage | null): string => {
...
const intentKey = (userMessage: string): string => {
...
const scoreImpact = (userMessage: string, failureSignature: string, repeatedCount: number): number => {
  let score = 0.5;
...
const suggestExecutor = (userMessage: string, assistantMessage: ChatMessage | null): GovernedHistoryFinding["suggested_executor"] => {
  const normalized = normalize(userMessage);
  if (
    resolveBrowserNavigationTarget(userMessage) ||
...
const buildCardDraft = (finding: GovernedHistoryFinding): Partial<GovernedExecutionEvalCard> => ({
...
export interface GovernedHistoryMiningResult {
  findings: GovernedHistoryFinding[];
  groupedCardDrafts: Partial<GovernedExecutionEvalCard>[];
  sourceConversationIds: string[];
}

export interface GovernedHistoryMiningOptions {
  artifactsRoot?: string;
  maxFindings?: number;
}

const appendJsonl = async (filePath: string, records: unknown[]): Promise<void> => {
...
export const mineGovernedHistory = async (
  options: GovernedHistoryMiningOptions = {}
): Promise<GovernedHistoryMiningResult> => {
  ...
  const grouped = new Map<string, GovernedHistoryFinding>();
  ...
  for (const record of records) {
    ...
    for (let index = 0; index < messages.length; index += 1) {
      ...
      if (failureFeedback) {
        ...
        if (!current) {
          const finding: GovernedHistoryFinding = {
  ...
  const groupedCardDrafts = limitedFindings
    ...
    .filter((card): card is Partial<GovernedExecutionEvalCard> => Boolean(card));

  if (options.artifactsRoot) {
    const auditDir = path.join(options.artifactsRoot, "governance-history");
    await mkdir(auditDir, { recursive: true });
    await writeFile(
      path.join(auditDir, "candidate-cards.json"),
      `${JSON.stringify(
        {
    ...
    await writeFile(
      path.join(auditDir, "latest-backlog.json"),
      `${JSON.stringify(
        {
          startedAt: new Date().toISOString(),
