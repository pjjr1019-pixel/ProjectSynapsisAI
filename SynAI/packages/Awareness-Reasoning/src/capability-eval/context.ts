...
import type { AwarenessEngine } from "../bootstrap";
import type { AwarenessQueryAnswer } from "../contracts/awareness";
import type {
  CapabilityContextBundle,
  CapabilityContextEvidence,
  CapabilityContextRequirement,
  CapabilityTestCard
} from "./types";

interface CapabilityRetrievalHintEntry {
  queryAugments?: string[];
  preferredTags?: string[];
  expectedSources?: string[];
}

interface CapabilityRetrievalHints {
  cards?: Record<string, CapabilityRetrievalHintEntry>;
}

const summarizeText = (value: string, maxChars = 600): string => {
...
const toEvidence = (
  requirement: CapabilityContextRequirement,
  payload: unknown,
  summary: string,
  missing: boolean
): CapabilityContextEvidence => ({
...
const resolveWorkspacePath = (workspaceRoot: string, candidatePath: string): string =>
  path.isAbsolute(candidatePath) ? candidatePath : path.join(workspaceRoot, candidatePath);

const toAwarenessSummary = (answer: AwarenessQueryAnswer): string =>
  [
    answer.summary,
...
export interface ResolveCapabilityContextOptions {
  workspaceRoot: string;
  awarenessEngine: AwarenessEngine | null;
  awarenessRefresh?: boolean;
}

const loadCapabilityRetrievalHints = async (
  workspaceRoot: string
): Promise<CapabilityRetrievalHints> => {
  ...
  return parsed as CapabilityRetrievalHints;
};

const resolveOneContext = async (
  requirement: CapabilityContextRequirement,
  options: ResolveCapabilityContextOptions,
  card: CapabilityTestCard,
  retrievalHints: CapabilityRetrievalHints
): Promise<CapabilityContextEvidence> => {
  ...
  if (requirement.source === "workspace.search") {
    ...
    if (!result || result.hits.length === 0) {
      return toEvidence(
        ...
        `No workspace hits for "${effectiveQuery}".`,
        true
      );
    }
    ...
    return toEvidence(
      ...
      `Workspace hits: ${result.hits
        ...
        .join(" | ")}${queryAugments.length > 0 ? ` | augments: ${queryAugments.join(", ")}` : ""}`,
      false
    );
  }

  if (
    requirement.source === "awareness.live-usage" ||
...
export const resolveCapabilityContext = async (
  card: CapabilityTestCard,
  options: ResolveCapabilityContextOptions
): Promise<CapabilityContextBundle> => {
  const retrievalHints = await loadCapabilityRetrievalHints(options.workspaceRoot);
  const required = await Promise.all(
    card.required_context.map((requirement) =>
      resolveOneContext(requirement, options, card, retrievalHints)
    )
  );
  const optional = await Promise.all(
    card.optional_context.map((requirement) =>
      resolveOneContext(requirement, options, card, retrievalHints)
    )
  );
