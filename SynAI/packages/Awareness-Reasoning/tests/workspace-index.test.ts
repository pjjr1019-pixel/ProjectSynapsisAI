...
describe("workspace index retrieval", () => {
  ...
  it("reuses unchanged file chunks across incremental refreshes", async () => {
    ...
    try {
      ...
      const firstIndex = JSON.parse(await readFile(indexPath, "utf8")) as {
        files: Array<{ relativePath: string; chunkIds: string[] }>;
      };
      ...
      const secondIndex = JSON.parse(await readFile(indexPath, "utf8")) as {
        files: Array<{ relativePath: string; chunkIds: string[] }>;
      };
