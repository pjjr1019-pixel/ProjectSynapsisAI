...
const api = {
    ...
    issueDesktopActionApproval: (request, approvedBy, ttlMs) => ipcRenderer.invoke(IPC_CHANNELS.desktopActionApprove, request, approvedBy, ttlMs),
    executeDesktopAction: (request) => ipcRenderer.invoke(IPC_CHANNELS.desktopActionExecute, request),
    rollbackDesktopAction: (commandId, approvedBy, dryRun) => ipcRenderer.invoke(IPC_CHANNELS.rollbackDesktopAction, commandId, approvedBy, dryRun),
    suggestWorkflow: (prompt) => ipcRenderer.invoke(IPC_CHANNELS.workflowPlanSuggest, prompt),
    issueWorkflowApproval: (plan, approvedBy, ttlMs) => ipcRenderer.invoke(IPC_CHANNELS.workflowApprove, plan, approvedBy, ttlMs),
    ...
    setOfficialKnowledgeSourceEnabled: (sourceId, enabled) => ipcRenderer.invoke(IPC_CHANNELS.officialKnowledgeSourceUpdate, sourceId, enabled),
    refreshOfficialKnowledgeSource: (sourceId) => ipcRenderer.invoke(IPC_CHANNELS.officialKnowledgeSourceRefresh, sourceId),
    getContextPreview: (conversationId, latestUserMessage, awarenessAnswerMode, ragOptions) => ipcRenderer.invoke(IPC_CHANNELS.contextPreview, conversationId, latestUserMessage, awarenessAnswerMode, ragOptions),
