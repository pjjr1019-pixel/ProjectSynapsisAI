...
## Completed
- `SynAI/packages/Agent-Runtime/src` is the canonical source of truth for the 5-layer runtime contracts and implementation.
...
- SynAI desktop integration now routes runtime calls through Electron IPC and preload, keeps `@synai-agent` as a thin compatibility alias, and exposes a compact operator-facing inspection card in the desktop app.
- Runtime checkpoints and results now carry explicit continuation metadata so resume and recover can honestly distinguish exact versus reconstructive continuation.
...
- Privileged behavior stays in the Electron main process and governed execution packages rather than moving into a second compatibility implementation or the renderer.
