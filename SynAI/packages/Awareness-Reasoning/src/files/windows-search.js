...
import { classifyPrivacyScope, createObservationFreshness, extensionFromPath, extensionToMimeType, inferMediaKind, inferTags, normalizePathForMatch } from "./shared";
...
const buildWindowsSearchScript = (query, options) => {
    ...
    const serializedScopes = options.volumeRoots
        ...
        .join(", ");
    const mediaOnlyClause = options.mode === "media"
...
const mapHitToFileEntry = (hit, observedAt) => {
...
const mapHitToMediaEntry = (entry, observedAt) => {
    if (!entry.mediaKind) {
        return null;
    }
    const capturedAt = observedAt.toISOString();
...
export const queryWindowsIndexedEntries = async (query, options = {}) => {
    ...
    const rawResult = await execute(buildWindowsSearchScript(trimmedQuery, {
        limit,
