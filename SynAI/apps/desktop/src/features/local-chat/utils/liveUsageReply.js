import { parseResourceUsageTargets, pickResourceUsageFindings } from "../../../../../../packages/Awareness-Reasoning/src/reasoning/resource-usage";
export const LIVE_USAGE_REFRESH_MS = 2_000;
const normalizeLine = (value) => value.replace(/\s+/g, " ").trim();
const lineForLabel = (lines, label) => lines.find((line) => line.toLowerCase().startsWith(label.toLowerCase())) ?? null;
export const isLiveUsageAnswer = (answer) => answer?.intent.family === "live-usage";
export const buildLiveUsageMessageMetadata = (answer, query, refreshedAt = answer.generatedAt) => ({
    awareness: {
...
export const formatLiveUsageReply = (answer, _refreshedAt = answer.generatedAt) => {
    ...
    const targetedLines = pickResourceUsageFindings([cpuLine, ramLine, gpuLine, diskLine, uptimeLine].filter((line) => Boolean(line)), selection);
