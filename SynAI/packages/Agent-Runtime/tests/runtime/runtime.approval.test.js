...
import { createActionProposal, createActionResult, createAgentTask, } from '@agent-runtime/core';
...
describe('Agent runtime approval handling', () => {
    it('returns denied without executing the adapter when approval validation fails', async () => {
        ...
        const run = await runtime.runTask(createAgentTask({
        ...
        expect(run.auditTrail.map((event) => `${event.stage}:${event.event}`)).toEqual(expect.arrayContaining(['policy:denied', 'executor:skipped', 'result:denied']));
    });
