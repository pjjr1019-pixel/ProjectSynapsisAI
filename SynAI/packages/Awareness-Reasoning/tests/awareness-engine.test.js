...
import { buildAwarenessDigest, buildLastReportedBaseline, buildSessionBaseline, captureMachineBaseline, captureRepoBaseline, canUsePermissionTier, filterPrivacyScopedValues } from "../src/baseline";
...
import { buildMachineAwarenessSummary, captureMachineAwarenessSnapshot, createFixtureMachineInventorySource } from "../src/machine";
import { SYSTEM_STATS_QUERY_VARIANTS, buildStorageClarification, buildAwarenessQueryAnswer, buildAwarenessQueryContextSection, routeAwarenessIntent } from "../src/reasoning";
...
const buildMachineAwarenessFixture = async () => captureMachineAwarenessSnapshot({
    now: fixedNow,
...
const buildPaths = (workspaceRoot) => {
...
const createDeliveredSnapshot = async (workspaceRoot, sessionId) => {
...
describe("awareness engine", () => {
    ...
    it("summarizes the changelog when asking whats new even if the git delta is flat", async () => {
        await withWorkspace(async (workspaceRoot) => {
            ...
            await mkdir(path.dirname(seeded.currentSessionPath), { recursive: true });
            await writeFile(seeded.currentSessionPath, `${JSON.stringify(seeded.snapshot, null, 2)}\n`, "utf8");
            await writeFile(path.join(workspaceRoot, "CHANGELOG.md"), [
                "# Changelog",
                "",
                "## 2026-04-08",
                "",
    ...
    it("captures structured repo working-tree deltas from git status output", async () => {
        ...
        expect(repoCapture.repo.workingTree.entries.map((entry) => entry.path)).toEqual(expect.arrayContaining([
    ...
    it("serves volume, folder, and monitor-status file-awareness endpoints", async () => {
        await withWorkspace(async (workspaceRoot) => {
            ...
            try {
                ...
                const folderResponse = await fetch(`${api.baseUrl}/api/awareness/files/folder?path=${encodeURIComponent(folderPath)}`);
    ...
    it("routes awareness intents and keeps uncertainty explicit when evidence is weak", async () => {
        await withWorkspace(async (workspaceRoot) => {
            ...
            expect(routeAwarenessIntent("using the current repo docs and settings copy, compare the two awareness answer modes").family).toBe("repo-change");
            ...
            expect(weakAnswer?.bundle.uncertainty.join(" ")).toContain("No previous session snapshot was available.");
    ...
    it("returns ranked resource hotspots with direct process and grouped program views", async () => {
        await withWorkspace(async (workspaceRoot) => {
            ...
            expect(typoHotspot?.bundle.resourceHotspots?.length).toBe(Math.min(4, engine.currentSnapshot.machineAwareness.processSnapshot.totalCount));
            ...
            const groupedInstalledApps = [
                {
                    ...engine.currentSnapshot.machineAwareness.installedAppsSnapshot.apps.find((app) => app.name === "Microsoft Edge"),
    ...
    it("switches between evidence-first and llm-primary pipelines for non-awareness prompts", async () => {
        await withWorkspace(async (workspaceRoot) => {
            ...
            const awarenessSection = buildAwarenessQueryContextSection(evidenceFirst, "tell me a joke about dragons");
        ...
        await withWorkspace(async (workspaceRoot) => {
            const alternateMachineSource = {
                ...
                captureProcesses: async () => {
                    ...
                    const adjusted = base.processes.map((process, index) => index === 0
                        ? {
                            ...process,
                            pid: 9090,
            ...
            expect(primaryAnswer?.bundle.verifiedFindings.join(" ")).not.toBe(alternateAnswer?.bundle.verifiedFindings.join(" "));
        });
    });
    it("answers file, process, service, performance, and on-screen questions from compact evidence", async () => {
        await withWorkspace(async (workspaceRoot) => {
            ...
            const performanceContextSection = buildAwarenessQueryContextSection(resourceHotspotAnswer ?? null, "what is using the most RAM?");
    ...
    it("serves awareness query, summary, and diagnostic endpoints", async () => {
        await withWorkspace(async (workspaceRoot) => {
            ...
            try {
                ...
                const liveUsage = (await liveUsageResponse.json());
                ...
                const summaryResponse = await fetch(`${api.baseUrl}/api/awareness/summary?scope=current-machine&mode=short`);
                ...
                const eventDiagnostic = (await eventsResponse.json());
    ...
    it("builds rolling metrics, startup digests, and runtime sampler health", async () => {
        await withWorkspace(async (workspaceRoot) => {
            ...
            expect(engine.getStatus().startupDigest?.highlights.some((highlight) => highlight.includes("Latest Windows update"))).toBe(true);
    ...
    it("builds correlation graphs and recurring patterns across sessions", async () => {
        await withWorkspace(async (workspaceRoot) => {
            ...
            expect(secondEngine.machineAwareness.recurringPatterns?.some((pattern) => pattern.title.toLowerCase().includes("recurring"))).toBe(true);
        });
    });
