...
describe('Agent runtime service lifecycle', () => {
    it('supports inspect, cancel, recover, and resume from persisted checkpoints without rerunning', async () => {
        ...
        const blockedRun = await runtime.startTask(createAgentTask({
        ...
        expect(resumed?.auditTrail.map((event) => `${event.stage}:${event.event}`)).toEqual(expect.arrayContaining(['runtime:resumed', 'checkpoint:saved']));
    });
});
