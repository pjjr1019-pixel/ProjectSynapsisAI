...
import type { AwarenessEngine } from "../src/bootstrap";
import {
...
const createTinyPng = (width = 1, height = 1): Buffer => {
...
const buildRuntimeRoot = (workspaceRoot: string): string => path.join(workspaceRoot, ".runtime", "awareness", "files");

const seedBaseWorkspace = async (
  workspaceRoot: string,
  options?: {
    includeNotes?: boolean;
    includeLargeFile?: boolean;
  }
): Promise<{ secretPath: string; downloadsPath: string; documentsPath: string; largeFilePath: string | null; notesPath: string | null }> => {
...
const initializeWorkspaceAwareness = async (workspaceRoot: string) =>
  initializeFileAwareness({
    workspaceRoot,
    roots: [workspaceRoot],
...
describe("file awareness", () => {
  ...
  it("refreshes incrementally and records visible recent changes", async () => {
    await withWorkspace(async (workspaceRoot) => {
      ...
      const recentChanges = JSON.parse(recentChangesFile) as Array<{ type: string; path: string }>;
  ...
  it("serves the file and media awareness API surfaces on localhost", async () => {
    await withWorkspace(async (workspaceRoot) => {
      ...
      const engine = {
        ...
        refreshFiles: async (reason?: string) => state.refresh(reason),
        markDigestDelivered: async () => ({}) as any,
        appendEvent: async () => ({}) as any,
        close: async () => undefined
      } as unknown as AwarenessEngine;
      ...
      try {
        ...
        const roots = (await rootsResponse.json()) as Array<{ path: string; included: boolean }>;
        ...
        const searchResult = (await searchResponse.json()) as { files: Array<{ path: string }>; folders: Array<{ path: string }> };
        ...
        const largest = (await largestResponse.json()) as Array<{ path: string }>;
        expect(largest[0]?.path).toBe(path.normalize(seeded.largeFilePath ?? ""));

        const recentResponse = await fetch(`${api.baseUrl}/api/awareness/files/recent`);
        expect(recentResponse.status).toBe(200);
        const recent = (await recentResponse.json()) as Array<{ path: string; type: string }>;
        expect(recent.length).toBeGreaterThan(0);

        const folderSummaryResponse = await fetch(
          `${api.baseUrl}/api/awareness/files/folder-summary?path=${encodeURIComponent(seeded.downloadsPath)}`
        );
        expect(folderSummaryResponse.status).toBe(200);
        const folderSummary = (await folderSummaryResponse.json()) as { path: string; topFiles: Array<{ path: string }> };
        ...
        const mediaResult = (await mediaResponse.json()) as { results: Array<{ path: string; width: number | null; height: number | null }> };
        ...
        const refreshed = (await refreshResponse.json()) as { recentChanges: Array<{ path: string }> };
