...
const slugify = (value) => value
    ...
    .replace(/^-+|-+$/g, "")
    .slice(0, 48);
const toFence = (value, language = "") => {
    const content = value.trim() || "(empty)";
    const fence = content.includes("```") ? "````" : "```";
    return `${fence}${language}\n${content}\n${fence}`;
};
const escapeTableCell = (value) => value.replace(/\|/g, "\\|").trim();
const formatFailureGroups = (report) => {
...
const formatRoute = (routeFamily, routeConfidence) => routeFamily === "none"
    ? "none"
    : `${routeFamily}${routeConfidence != null ? ` @ ${routeConfidence.toFixed(2)}` : ""}`;
const formatPolicySignals = (signals) => {
...
const formatPromptEvaluationChatHistoryEntry = (report) => {
    ...
    for (const entry of report.cases) {
        lines.push(`### ${entry.label} (${entry.difficulty})`, "", `- Status: ${entry.status}`, `- Quality: ${entry.qualityStatus}`, `- Route: ${formatRoute(entry.routing.routeFamily, entry.routing.routeConfidence)}`, `- Source scope: ${entry.routing.sourceScope ?? "none"}`, "", "#### Prompt", "", toFence(entry.prompt), "", "#### Reply", "", toFence(entry.reply), "", "#### Failed Checks", "", ...formatFailedChecks(entry), "");
    }
    return lines.join("\n").trim();
};
export const buildPromptEvaluationReportFileName = (suiteName, generatedAt) => {
...
export const formatPromptEvaluationMarkdown = (report) => {
    const lines = [
        `# ${report.suiteName}`,
        "",
        "## Run Settings",
    ...
    for (const entry of report.cases) {
        const passedChecks = entry.checkResults.filter((check) => check.passed).length;
        lines.push("", `## ${entry.label} (${entry.difficulty})`, "", `- Status: ${entry.status}`, `- Quality: ${entry.qualityStatus}`, `- Started: ${entry.startedAt}`, `- Completed: ${entry.completedAt}`, `- Duration: ${formatDuration(entry.durationMs)}`, `- Model status: ${entry.modelStatus.status}`, `- Model detail: ${entry.modelStatus.detail ?? "none"}`, `- Route: ${formatRoute(entry.routing.routeFamily, entry.routing.routeConfidence)}`, `- Raw route: ${formatRoute(entry.routing.rawRouteFamily, entry.routing.rawRouteConfidence)}`, `- Source scope: ${entry.routing.sourceScope ?? "none"}`, `- Awareness used: ${entry.routing.awarenessUsed ? "yes" : "no"}`, `- Deterministic awareness: ${entry.routing.deterministicAwareness ? "yes" : "no"}`, `- Generic writing suppression: ${entry.routing.genericWritingPromptSuppressed ? "yes" : "no"}`, `- Cleanup bypassed: ${entry.routing.cleanupBypassed ? "yes" : "no"}`, `- Routing suppression: ${entry.routing.routingSuppressionReason ?? "none"}`, `- Raw signals: ${formatPolicySignals(entry.routing.policyDiagnostics?.rawSignals)}`, `- Fallback signals: ${formatPolicySignals(entry.routing.policyDiagnostics?.fallbackSignals)}`, `- Classifier: ${formatClassifierCategories(entry.routing.policyDiagnostics?.classifier)}`, `- Chosen policy: ${entry.routing.policyDiagnostics?.chosenPolicy
            ...
            : "none"}`, `- Suppression reasons: ${formatPolicySignals(entry.routing.policyDiagnostics?.suppressionReasons)}`, `- Reasoning mode: ${entry.routing.reasoningMode ?? "not captured"}`, `- Retrieved source summary: ${entry.routing.retrievedSourceSummary
            ...
            : "not captured"}`, ...(entry.routing.retrievedSourceSummary?.workspacePaths?.length
            ? [`- Workspace paths: ${entry.routing.retrievedSourceSummary.workspacePaths.join(", ")}`]
            : []), `- Checks passed: ${passedChecks}/${entry.checkResults.length}`, entry.traceSummary
            ? `- Trace confidence: ${entry.traceSummary.confidence} | grounded sources: ${entry.traceSummary.groundedSourceCount}`
            : "- Trace confidence: not captured", "", "### Prompt", "", toFence(entry.prompt), "", "### Reply", "", toFence(entry.reply), "", "### Checks", "", ...(entry.checkResults.length > 0
            ? entry.checkResults.map((check) => `- ${check.passed ? "PASS" : "FAIL"} | ${check.description} | ${check.detail}`)
            : ["- No checks configured."]));
    }
    return `${lines.join("\n")}\n`;
};
export const buildPromptEvaluationReportPath = (workspaceRoot, suiteName, generatedAt) => path.join(workspaceRoot, ".runtime", "prompt-evals", buildPromptEvaluationReportFileName(suiteName, generatedAt));
export const buildPromptEvaluationChatHistoryPath = (workspaceRoot) => path.join(workspaceRoot, ".runtime", "prompt-evals", "chathistory.md");
export const upsertPromptEvaluationChatHistory = (existingMarkdown, report) => {
