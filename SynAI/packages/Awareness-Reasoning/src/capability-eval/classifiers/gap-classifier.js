...
export const classifyCapabilityGap = (input) => {
    const { card, execution, verifier } = input;
    const reasonsText = verifier.reasons.join(" | ");
    const secondary = [];
    let primary = "bad-reasoning-planning";
    let confidence = 0.62;
    ...
    if (includesAny(reasonsText, ["custom-hook verifier", "unsupported verifier", "invalid required command regex"]) ||
    ...
    else if (execution.output.answer_or_action.mode === "refusal" &&
    ...
    else if (includesAny(reasonsText, ["Missing required tool", "Forbidden tool/workflow selected"]) ||
    ...
    else if ((card.verifier_type === "command-action-selection" ||
    ...
    else if (card.verifier_type === "action-risk-class" ||
    ...
    else if (execution.request.context.retrievalStats.requiredResolved === 0 &&
    ...
    else if (includesAny(reasonsText, ["Missing required phrase", "Expected exact match"]) &&
        missingInfoCount === 0 &&
