...
import type { WebSearchContext, WebSearchResult, WebSearchSourceFamily } from "../contracts/memory";
...
const tokenizeQuery = (query: string): string[] => normalizeSearchQuery(query).split(" ").filter(Boolean);

const isTimeSensitiveQuery = (query: string, force = false): boolean =>
  force ||
  RECENT_QUERY_PATTERNS.some((pattern) => pattern.test(query)) ||
  VOLATILE_TOPIC_PATTERNS.some((pattern) => pattern.test(query));

export const shouldUseRecentWebSearch = (query: string, force = false): boolean => isTimeSensitiveQuery(query, force);
...
const decodeSearchUrl = (value: string): string => {
...
const inferSourceFamily = (url: string, fallback: WebSearchSourceFamily = "web"): WebSearchSourceFamily => {
  try {
...
const inferSearchRelevance = (query: string, text: string): number => {
...
const parseDuckDuckGoHtml = (html: string, maxResults: number): WebSearchResult[] => {
  const resultBlocks =
    html.match(/<div class="result__body"[\s\S]*?<\/div>\s*<\/div>/gi) ??
...
const fetchDuckDuckGoHtml = async (query: string, config: WebSearchConfig): Promise<WebSearchResult[]> => {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), config.timeoutMs);
...
const dedupeSearchResults = (results: WebSearchResult[]): WebSearchResult[] => {
...
const scoreSearchResult = (query: string, result: WebSearchResult): number => {
...
const rankSearchResults = (query: string, results: WebSearchResult[]): WebSearchResult[] =>
...
export const resolveRecentWebContext = async (
  ...
  try {
    const [newsResults, webResults] = await Promise.all([
      fetchGoogleNewsRss(trimmedQuery, config).catch(() => [] as WebSearchResult[]),
      fetchDuckDuckGoHtml(trimmedQuery, config).catch(() => [] as WebSearchResult[])
    ]);
