...
const buildAdapter = (workspaceRoot) => ({
    getWorkspaceRoot: () => workspaceRoot,
    execute: async (card) => {
        const hasClarification = card.required_context.some((entry) => entry.source === "inline" && typeof entry.value === "string" && entry.value.includes("Deterministic clarification"));
        return {
            request: {
                prompt: card.prompt,
                systemPrompt: "system",
...
describe("governed promotion e2e smoke", () => {
    it("blocks promotion without token and promotes with a valid token", async () => {
        ...
        try {
            ...
            const commandHash = hashGovernanceCommand(createGovernedPromotionHashInput("e2e.governed-promotion", "packages/Capability-Catalog/cards/governed-card.json"));
