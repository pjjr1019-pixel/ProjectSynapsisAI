...
import { buildCapabilityCliArgs, discoverCapabilityCards, parseCapabilityCliSummary, resolveCardArtifactPath } from "../../apps/vscode-capability-testing/src/core";
describe("vscode capability extension smoke helpers", () => {
    it("discovers capability cards for Test Explorer listing", async () => {
        ...
        await writeFile(path.join(cardsRoot, "one.json"), `${JSON.stringify({ id: "windows.one", name: "One" }, null, 2)}\n`, "utf8");
        await writeFile(path.join(cardsRoot, "two.json"), `${JSON.stringify({ id: "windows.two", name: "Two" }, null, 2)}\n`, "utf8");
        try {
    ...
    it("parses JSON summary output and resolves artifact links", () => {
        const summary = parseCapabilityCliSummary(JSON.stringify({
            runId: "run-1",
