...
const toStringArray = (value) => {
    ...
    return value.map((item) => toStringOrNull(item)).filter((item) => Boolean(item));
};
...
const buildNodeIdentityFallback = () => {
    const now = new Date();
...
const mapStartupEntry = (entry) => ({
    ...
    source: entry.source === "run" || entry.source === "runonce" || entry.source === "scheduled-task" || entry.source === "launcher-hint"
        ? entry.source
        : "startup-folder",
...
const mapInstalledApp = (app) => ({
    ...
    sources: Array.isArray(app.sources)
        ? app.sources.map((value) => toStringOrNull(value)).filter((item) => Boolean(item))
        : toStringArray(app.sources),
    associatedProcessIds: [],
    associatedProcessNames: [],
    startupReferences: []
});
const mapEventLogLevel = (value) => {
...
const captureSystemIdentity = async () => {
    const now = new Date();
...
`, {
    ...
    return {
        ...
        hardware: {
            ...
            memory: {
                totalBytes: toNumberOrNull(identity.hardware?.memory?.totalBytes) ??
                    fallback.hardware.memory.totalBytes,
                availableBytes: toNumberOrNull(identity.hardware?.memory?.availableBytes) ??
                    fallback.hardware.memory.availableBytes,
                freeBytes: toNumberOrNull(identity.hardware?.memory?.freeBytes) ?? fallback.hardware.memory.freeBytes
            },
            cpu: {
                ...
                logicalCores: toNumberOrNull(identity.hardware?.cpu?.logicalCores) ?? fallback.hardware.cpu.logicalCores,
                speedMHz: toNumberOrNull(identity.hardware?.cpu?.speedMHz) ?? fallback.hardware.cpu.speedMHz
            },
            cpuLoadPercent: toNumberOrNull(identity.hardware?.cpuLoadPercent) ?? fallback.hardware.cpuLoadPercent,
            ...
            radioState: {
                bluetoothEnabled: toBooleanOrNull(identity.hardware?.radioState?.bluetoothEnabled) ??
                    fallback.hardware.radioState.bluetoothEnabled,
                wifiEnabled: toBooleanOrNull(identity.hardware?.radioState?.wifiEnabled) ??
                    fallback.hardware.radioState.wifiEnabled,
                airplaneModeEnabled: toBooleanOrNull(identity.hardware?.radioState?.airplaneModeEnabled) ??
...
const captureProcesses = async () => {
    ...
    const raw = await runPowerShellJson(processScript, {
...
const captureServices = async () => {
    const capturedAt = new Date().toISOString();
    const raw = await runPowerShellJson(serviceScript, {
...
const captureStartup = async () => {
    const capturedAt = new Date().toISOString();
    const raw = await runPowerShellJson(startupScript, {
...
const captureInstalledApps = async () => {
    const capturedAt = new Date().toISOString();
    const raw = await runPowerShellJson(installedAppsScript, {
...
const captureEventLogs = async () => {
    const capturedAt = new Date().toISOString();
    const raw = await runPowerShellJson(eventLogScript, {
...
const captureRecentUpdates = async () => {
    const raw = await runPowerShellJson(updatesScript, {
...
export const createWindowsMachineInventorySource = () => ({
