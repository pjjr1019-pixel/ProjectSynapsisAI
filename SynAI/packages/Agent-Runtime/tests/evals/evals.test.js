...
const createDesktopActionTask = (input) => createAgentTask({
    id: input.id,
    title: input.title,
    metadata: {
...
const createScenarioAdapter = () => ({
    ...
    async propose(request) {
        ...
        const bindingHash = scenario === 'approval-binding-mismatch'
            ? `${request.bindingHash}-adapter`
            : request.bindingHash;
        const approvalBinding = scenario === 'approval-binding-mismatch'
            ? {
        ...
        return createActionProposal({
            ...
            risk: scenario === 'live-medium-block'
                ? 'medium'
...
const runScenarioTask = async (task) => {
    ...
    const runtime = createAgentRuntimeService({
        ...
        validateApproval: scenario === 'approval-validator-denied'
            ? async () => ({
                valid: false,
...
describe('runAgentRuntimeEvalSuite', () => {
    ...
    it('covers denied, approval mismatch, and live-risk paths deterministically', async () => {
        ...
        const report = await runAgentRuntimeEvalSuite(cases, {
