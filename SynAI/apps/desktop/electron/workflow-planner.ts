...
import {
  extractTargetsFromResolvedPrompt,
  resolveBrowserNavigationTarget as resolveCanonicalBrowserNavigationTarget,
  resolveDesktopOrganizationTarget,
  resolveKnownApplicationTarget as resolveCanonicalApplicationTarget
} from "@awareness/target-knowledge";
...
const uniqueStrings = (values: Array<string | null | undefined>): string[] =>
  [...new Set(values.filter((value): value is string => Boolean(value)).map((value) => value.trim()).filter(Boolean))];

const hashWorkflowPlan = (plan: Omit<WorkflowPlan, "workflowHash">): string => {
...
const isFileTask = (prompt: string): boolean => {
...
const isCreateTask = (prompt: string): boolean => /\b(create|make|new)\b/.test(normalizePrompt(prompt));

const isOpenTask = (prompt: string): boolean => /\b(open|show|view|browse)\b/.test(normalizePrompt(prompt));

const isRenameTask = (prompt: string): boolean =>
  /\b(rename|name it|call it|save as|save it as)\b/.test(normalizePrompt(prompt));

const isMoveTask = (prompt: string): boolean =>
  /\b(move|relocate|put it in|put it into|send it to)\b/.test(normalizePrompt(prompt));

const isDeleteTask = (prompt: string): boolean =>
  /\b(delete|remove|trash)\b/.test(normalizePrompt(prompt));

const isCloseTask = (prompt: string): boolean =>
  /\b(close|quit|exit|shut down|shutdown)\b/.test(normalizePrompt(prompt));

const isHealthReportTask = (prompt: string): boolean =>
  /(?:computer[s]?|system|pc|machine).{0,24}\bhealth\b|\bhealth report\b/.test(normalizePrompt(prompt));

const isBrowserAutomationPrompt = (prompt: string): boolean => {
  const normalized = normalizePrompt(prompt);
...
const isServiceControlPrompt = (prompt: string): boolean => {
  const normalized = normalizePrompt(prompt);
  return containsAny(normalized, ["service", "services", "print spooler", "windows update service"]);
};

const isRegistryControlPrompt = (prompt: string): boolean => {
  const normalized = normalizePrompt(prompt);
...
const isUiAutomationPrompt = (prompt: string): boolean => {
  const normalized = normalizePrompt(prompt);
  return containsAny(normalized, ["click", "type", "press", "hotkey", "keyboard shortcut", "select", "choose", "fill"]);
};

const extractBrowserNavigationTarget = (prompt: string): string | null => {
  return resolveCanonicalBrowserNavigationTarget(prompt)?.url ?? null;
};

const isBrowserNavigationPrompt = (prompt: string): boolean => {
  return extractBrowserNavigationTarget(prompt) !== null;
};

const resolveKnownApplicationTarget = (prompt: string): string | null => {
  return resolveCanonicalApplicationTarget(prompt)?.target ?? null;
};

const extractQuotedValue = (prompt: string): string | null => {
...
const extractPromptValue = (prompt: string, patterns: RegExp[], fallback: string): string => {
...
const normalizeExecutableTarget = (value: string | null | undefined): string | null => {
...
const pickTargetFolder = (prompt: string, context: WorkflowPlanningContext): WorkflowSaveLocation => {
  const normalized = normalizePrompt(prompt);
...
const summarizeMachine = (machine: MachineAwarenessSnapshot | null): string | null =>
  machine
    ? [
...
const summarizeFile = (file: FileAwarenessSummary | null): string | null =>
  file ? [`files ${file.counts.files}`, `folders ${file.counts.folders}`, `media ${file.counts.media}`].join(" | ") : null;

const summarizeScreen = (screen: ScreenAwarenessSnapshot | null): string | null =>
  screen
    ? [
...
const evidenceFromWeb = (web: WebSearchContext | null): WorkflowEvidence[] =>
  web && web.status === "used"
...
const candidateApps = (machine: MachineAwarenessSnapshot | null, target: string): MachineAwarenessSnapshot["installedAppsSnapshot"]["apps"][number][] => {
  if (!machine) {
    return [];
  }
...
const resolveCreateTargetName = (prompt: string, fallback: string): string =>
  extractPromptValue(
    prompt,
    [
...
const resolveRenameTargetName = (prompt: string, fallback: string): string =>
  extractPromptValue(
    prompt,
    [
...
const resolveRenameSourceName = (prompt: string, fallback: string): string =>
  extractPromptValue(
    prompt,
    [
...
const resolveMoveTargetName = (prompt: string, fallback: string): string =>
  extractPromptValue(
    prompt,
    [
...
const resolveMoveSourceName = (prompt: string, fallback: string): string =>
  extractPromptValue(
    prompt,
    [
...
const resolveOpenTargetName = (prompt: string, fallback: string): string =>
  extractPromptValue(
    prompt,
    [
...
const resolveDeleteTargetName = (prompt: string, fallback: string): string =>
  extractPromptValue(
    prompt,
    [
...
const resolveApplicationTargetName = (prompt: string, fallback: string): string =>
  resolveKnownApplicationTarget(prompt) ??
  extractPromptValue(
    prompt,
    [
...
const isGenericProcessTarget = (value: string): boolean =>
  ["task", "process", "app", "application", "program", "thing", "item", "window"].includes(normalizePrompt(value));

const resolvePathRoot = (prompt: string, context: WorkflowPlanningContext, fallback: WorkflowSaveLocation): string => {
  const normalized = normalizePrompt(prompt);
  if (containsAny(normalized, ["desktop"])) {
...
const resolveSystemNavigationAction = (
  prompt: string
): { actionId: DesktopActionKind; target: string } | null => {
  const normalized = normalizePrompt(prompt);
  const surfaces: Array<{ actionId: DesktopActionKind; tests: RegExp[]; target: string }> = [
    {
...
const buildPlan = (base: Omit<WorkflowPlan, "workflowHash">): WorkflowPlan => ({
  ...base,
...
const researchPlan = (
  ...
  const basePlan = {
    ...
    family: "research-report" as const,
...
const healthPlan = (prompt: string, context: WorkflowPlanningContext): WorkflowPlan => {
  const folder = pickTargetFolder(prompt, context);
...
const browserPlan = (prompt: string, _context: WorkflowPlanningContext): WorkflowPlan => {
  const query =
    prompt.match(/\bplay\b(.+?)(?:\bon\b|\bin\b|\busing\b|$)/i)?.[1]?.trim() ||
...
const browserNavigationPlan = (prompt: string, _context: WorkflowPlanningContext): WorkflowPlan => {
...
const resolveBrowserInteractionMode = (
  prompt: string
): {
  action: "click" | "type" | "hotkey" | "open-tab" | "wait" | "select" | "download";
  target: string;
  text: string | null;
  keys: string[];
  value: string | null;
  ...
  if (containsAny(normalized, ["wait for", "wait until", "pause until", "hold until", "stay on this page until"])) {
    return {
      action: "wait",
      target: extractPromptValue(
        prompt,
        [/\b(?:wait for|wait until|pause until|hold until)\s+(.+)$/i],
        "browser state"
      ),
      text: null,
      keys: [],
  ...
  if (containsAny(normalized, ["select", "choose", "pick", "dropdown", "combo box"])) {
    return {
      ...
      target: extractPromptValue(
        prompt,
        [/\b(?:select|choose|pick)\s+(?:the\s+)?([a-z0-9._-]+(?:\s+[a-z0-9._-]+){0,5})/i],
        "browser select"
      ),
      text: null,
      keys: [],
...
const resolveServiceAction = (
  prompt: string
): { actionId: "start-service" | "stop-service" | "restart-service"; target: string } | null => {
  const normalized = normalizePrompt(prompt);
  if (!containsAny(normalized, ["service", "services", "print spooler", "windows update service"])) {
    return null;
  }

  const actionId: "start-service" | "stop-service" | "restart-service" = containsAny(normalized, ["restart", "cycle"])
    ? "restart-service"
    : containsAny(normalized, ["stop", "disable", "turn off"]) && !containsAny(normalized, ["start"])
      ? "stop-service"
      : "start-service";

  const target = extractPromptValue(
    prompt,
    [
...
const resolveRegistryAction = (
  prompt: string
): { actionId: "set-registry-value" | "delete-registry-value"; key: string; valueName: string; value: string } | null => {
  const normalized = normalizePrompt(prompt);
  if (!containsAny(normalized, ["registry", "regedit", "registry key", "registry value"])) {
    return null;
  }

  const actionId: "set-registry-value" | "delete-registry-value" = containsAny(normalized, ["delete", "remove"]) && !containsAny(normalized, ["set", "change", "modify", "update"])
    ? "delete-registry-value"
    : "set-registry-value";
  const key = extractPromptValue(
    prompt,
    [
  ...
  const valueName = extractPromptValue(
    prompt,
    [
  ...
  const value = extractPromptValue(
    prompt,
    [
...
const resolveUiAction = (
  prompt: string
): { actionId: "ui-click" | "ui-type" | "ui-hotkey"; target: string; value: string | null; keys: string[]; submit: boolean } | null => {
  const normalized = normalizePrompt(prompt);
...
const browserInteractionPlan = (prompt: string, context: WorkflowPlanningContext): WorkflowPlan => {
  const normalized = normalizePrompt(prompt);
...
const serviceControlPlan = (prompt: string, context: WorkflowPlanningContext): WorkflowPlan => {
  const normalized = normalizePrompt(prompt);
  ...
  return buildPlan({
    ...
    family: "service-control",
    summary:
      action.actionId === "restart-service"
...
const registryControlPlan = (prompt: string, context: WorkflowPlanningContext): WorkflowPlan => {
  const normalized = normalizePrompt(prompt);
  ...
  return buildPlan({
    ...
    family: "registry-control",
    summary:
      action.actionId === "delete-registry-value"
...
const uiAutomationPlan = (prompt: string, context: WorkflowPlanningContext): WorkflowPlan => {
  const normalized = normalizePrompt(prompt);
...
const applicationPlan = (prompt: string, context: WorkflowPlanningContext): WorkflowPlan => {
  const normalized = normalizePrompt(prompt);
  ...
  const launchTarget =
    launchTargetCandidates.find((candidate) => /[\\/]|(?:\.exe|\.com|\.bat|\.cmd)$/i.test(candidate)) ??
    resolvedTarget;
  return buildPlan({
    requestId: randomUUID(),
    prompt,
...
const resolveCloseTargetName = (
  prompt: string,
  context: WorkflowPlanningContext
): { target: string; candidates: ReturnType<typeof candidateProcesses>; appCandidates: ReturnType<typeof candidateApps> } | null => {
  const target = extractPromptValue(
    prompt,
    [
  ...
  const synaiForegroundTarget = (() => {
    ...
    if (
      (foregroundProcess && normalizePrompt(foregroundProcess).includes("synai")) ||
...
const closeAppPlan = (prompt: string, context: WorkflowPlanningContext): WorkflowPlan => {
  const normalized = normalizePrompt(prompt);
...
const isWindowControlPrompt = (prompt: string): boolean => {
  const normalized = normalizePrompt(prompt);
  return containsAny(normalized, ["focus ", "bring to front", "switch to", "activate ", "show window", "foreground window"]);
};

const resolveWindowControlTargetName = (
  prompt: string,
  context: WorkflowPlanningContext
): { target: string; candidates: ReturnType<typeof candidateProcesses> } | null => {
  const target = extractPromptValue(
    prompt,
    [
...
const windowControlPlan = (prompt: string, context: WorkflowPlanningContext): WorkflowPlan => {
  const normalized = normalizePrompt(prompt);
...
const isBulkDesktopOrganizationPrompt = (prompt: string): boolean => {
  const normalized = normalizePrompt(prompt);
  return (
...
const bulkDesktopOrganizationPlan = async (prompt: string, context: WorkflowPlanningContext): Promise<WorkflowPlan> => {
  const normalized = normalizePrompt(prompt);
  ...
  const movableEntries = desktopEntries.filter(
    (entry) =>
      entry.name !== resolution.folderName &&
...
const maintenancePromptMode = (
  normalized: string
): "cleanup" | "organization" | "startup-review" | "disk-space-triage" | "performance-triage" => {
...
const maintenanceReviewPlan = (prompt: string, context: WorkflowPlanningContext): WorkflowPlan => {
  const normalized = normalizePrompt(prompt);
...
const systemNavigationPlan = (prompt: string, context: WorkflowPlanningContext): WorkflowPlan => {
  const normalized = normalizePrompt(prompt);
...
const filePlan = (prompt: string, context: WorkflowPlanningContext): WorkflowPlan => {
  const normalized = normalizePrompt(prompt);
  ...
  const primaryKind: "file" | "folder" = containsAny(normalized, ["folder", "directory"]) && !containsAny(normalized, ["file"]) ? "folder" : "file";
  ...
  const resolvedOpenName =
    openName && !containsAny(normalizedOpenName, ["file", "folder", "directory", "app", "application", "program"])
      ? openName
      : "";
  const openRequiresClarification =
    wantsOpen &&
    !openName &&
    !containsAny(normalized, ["desktop", "documents", "document folder", "docs folder", "workspace", "repo", "project folder"]);
  const openTarget =
    explicitSourcePath && wantsOpen
      ? explicitSourcePath
  ...
  const moveName =
    wantsMove && ["my", "the", "a", "an", "document", "documents", "folder", "file", "item", "it"].includes(normalizePrompt(moveNameCandidate))
      ? renameName
      : moveNameCandidate;
  ...
  return buildPlan({
    ...
    artifacts: [],
    clarificationNeeded:
      steps.length === 1 && steps[0].kind === "confirm-approval"
...
const processPlan = (prompt: string, context: WorkflowPlanningContext): WorkflowPlan => {
  const normalized = normalizePrompt(prompt);
...
const uninstallPlan = (prompt: string, context: WorkflowPlanningContext): WorkflowPlan => {
  const normalized = normalizePrompt(prompt);
  const target = extractPromptValue(
    prompt,
    [
...
const generalPlan = (prompt: string, context: WorkflowPlanningContext): WorkflowPlan =>
  buildPlan({
    requestId: randomUUID(),
    prompt,
    normalizedPrompt: normalizePrompt(prompt),
...
export const buildWorkflowPlan = async (
  prompt: string,
  context: WorkflowPlanningContext
): Promise<WorkflowPlan> => {
  const normalized = normalizePrompt(prompt);
  const webContext =
    context.recentWebContext ??
  ...
  if (
    containsAny(normalized, ["launch", "run ", "start app", "open app", "open program"]) ||
