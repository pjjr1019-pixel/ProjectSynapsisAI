...
import { cloneValue, createActionResult, createAgentTask, createExecutionAttempt, createExecutionContext, createObservationEvidence, createObservationSnapshot, createRuntimeCheckpoint, createRuntimeId, createRuntimeJob, createRuntimeProgressEvent, nowIso, } from '../core';
import { createDefaultActionAdapters, executeActionProposal, prepareActionProposal, } from '../executor';
import { captureObservationSnapshot } from '../perception';
import { planAgentTask } from '../planner';
import { evaluateTaskPolicy, validateApprovalBinding } from '../policy';
...
const cloneStep = (step) => cloneValue(step);
const updateStepStatus = (step, status, patch = {}) => ({
    ...step,
    status,
    updatedAt: nowIso(),
...
const resolveTaskStatus = (resultStatus) => {
    switch (resultStatus) {
        case 'success':
        case 'skipped':
            return 'completed';
...
const resolveJobStatus = (resultStatus) => {
    switch (resultStatus) {
        case 'success':
        case 'skipped':
            return 'completed';
...
const getLatestCheckpointState = (checkpoint) => checkpoint?.state ?? {};
const extractInspectionState = (checkpoint) => {
...
const createInitialObservation = (input) => createObservationSnapshot({
...
const resolveAuditEvents = async (store, jobId, taskId) => {
...
const resolveAuditStore = (auditStore) => auditStore ?? new InMemoryAuditStore();
const resolveSkillRegistry = (registry) => registry ?? createDefaultSkillRegistry();
const resolveActionAdapters = (registry, adapters) => {
...
const appendAudit = async (store, job, event) => {
...
const emitProgressEvent = (options, input) => {
    const event = createRuntimeProgressEvent(input);
    options.emitProgress?.(event);
    return event;
};
const createCheckpointContinuation = (sourceCheckpointId, limitation, mode = 'reconstructed') => ({
    mode,
    resumable: false,
    sourceCheckpointId,
    limitation,
});
const buildInspection = async (stateStore, auditStore, jobId) => {
    if (!stateStore) {
        return null;
    }
...
const createServiceMethods = (baseOptions = {}) => {
    const runTask = async (taskInput, callOptions = {}) => {
        const options = {
            ...baseOptions,
            ...callOptions,
        };
        ...
        const observations = [beforeObservation];
        ...
        if (planned.plan.status === 'clarification_needed') {
            const clarificationSummary = planned.plan.clarificationNeeded.join(' ') || 'Clarification is required before execution.';
            ...
            if (policyDecision.type !== 'allow') {
                ...
                const blockedStep = updateStepStatus(executionStep, 'skipped', {
                    error: {
                ...
                const approvalValidationRequired = !prepared.proposal.dryRun &&
                ...
                if (approvalValidationRequired) {
                    ...
                    if (!approvalValidation.valid) {
                        ...
                        const verifyStepIndex = executionStepIndex + 2;
                        plannedSteps[verifyStepIndex] = updateStepStatus(plannedSteps[verifyStepIndex], 'failed', {
                            result: verification,
                        });
                        job.completedStepIds = [...job.completedStepIds, plannedSteps[verifyStepIndex].id];
                        await appendAudit(auditStore, job, {
                ...
                if (!result) {
                    ...
                    const stepStatus = execution.actionResult.status === 'executed' || execution.actionResult.status === 'simulated'
                        ? 'completed'
                    ...
                    const verifyStepIndex = executionStepIndex + 2;
                    plannedSteps[verifyStepIndex] = updateStepStatus(plannedSteps[verifyStepIndex], verification.status === 'passed' ? 'completed' : verification.status === 'failed' ? 'failed' : 'skipped', {
                        result: verification,
                    });
                    job.completedStepIds = [...job.completedStepIds, plannedSteps[verifyStepIndex].id];
                    await appendAudit(auditStore, job, {
        ...
        const terminalContinuation = createCheckpointContinuation(checkpoint.id, 'Terminal checkpoints can be recovered without replaying side effects.');
    ...
    return {
        ...
        async cancelJob(jobId) {
            ...
            if (inspection.job.status === 'completed' ||
            ...
            checkpoint.continuation = createCheckpointContinuation(inspection.latestCheckpoint?.id ?? checkpoint.id, 'Cancelled jobs can be recovered from their latest checkpoint without replaying side effects.');
        ...
        async resumeJob(jobId, callOptions = {}) {
            if (!baseOptions.stateStore) {
                return null;
            }
            const auditStore = resolveAuditStore(baseOptions.auditStore);
            ...
            const continuation = createCheckpointContinuation(inspection.latestCheckpoint.id, 'Resume reconstructs the persisted checkpoint instead of replaying the original job.');
            const resumedTask = {
            ...
            const resumedResult = {
                ...
                summary: `Resumed from checkpoint ${inspection.latestCheckpoint.id} without replaying side effects. ${inspection.result.summary ?? ''}`.trim(),
                continuation,
                metadata: {
            ...
            const resumedObservation = cloneValue(inspection.observations[inspection.observations.length - 1]) ??
            ...
            const resumedCheckpoint = createRuntimeCheckpoint({
                ...
                state: {
                    ...cloneValue(inspection.latestCheckpoint.state),
                    task: resumedTask,
            ...
            const executionStep = resumedPlannedSteps.find((step) => step.metadata?.['phase'] === 'execute') ??
        ...
        async recoverJob(jobId) {
            ...
            recoveredCheckpoint.continuation = createCheckpointContinuation(inspection.latestCheckpoint.id, 'Recovered checkpoints are for inspection only and do not replay side effects.');
            const recoveredJob = {
                ...inspection.job,
...
export const createAgentRuntimeService = (options = {}) => createServiceMethods(options);
