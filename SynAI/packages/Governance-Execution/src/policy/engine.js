...
const normalizeCommand = (request) => [request.command, ...(request.args ?? [])]
    .join(" ")
    .replace(/\s+/g, " ")
    .trim();
const normalizeMetadataString = (value) => typeof value === "string" && value.trim().length > 0 ? value.trim().toLowerCase() : null;
const isRegexMatch = (pattern, input) => {
    try {
...
export const compareRiskClass = (left, right) => RISK_WEIGHT[left] - RISK_WEIGHT[right];
export const maxRiskClass = (left, right) => compareRiskClass(left, right) >= 0 ? left : right;
...
export const isDestructiveCommand = (command) => destructiveCommandPatterns.some((pattern) => pattern.test(command));
...
const inferRiskClassFromMetadata = (request) => {
...
export const inferRiskClassFromCommand = (command) => {
...
const matchesRule = (rule, request, command, effectiveRiskClass, destructive) => {
    ...
    if (rule.commandIncludes && rule.commandIncludes.length > 0) {
        ...
        const allFound = rule.commandIncludes.every((needle) => normalized.includes(needle.toLowerCase()));
        if (!allFound) {
            return false;
        }
    }
...
export const evaluateGovernancePolicy = (request, rules = defaultPolicyRules) => {
    ...
    const actionKind = normalizeMetadataString(request.actionKind) ?? normalizeMetadataString(request.metadata?.actionKind);
    const targetKind = normalizeMetadataString(request.targetKind) ?? normalizeMetadataString(request.metadata?.targetKind);
    const destructive = Boolean(request.destructive) ||
