...
const actionRiskClass = (action) => typeof action.riskClass === "string" && action.riskClass.length > 0
    ? action.riskClass
    : action.riskLevel ?? "medium";
const getByPath = (obj, targetPath) => {
...
const finish = (passed, reasons, evidence, observedOutput, expectedOutputSummary, score = passed ? 1 : 0) => ({
    passed,
    score,
    reasons,
    evidence,
...
const runExactMatchVerifier = (card, execution) => {
    ...
    const source = sourcePath.startsWith("output.")
    ...
    return finish(passed, passed ? ["Exact output matched expected text."] : [`Expected exact match, got: ${actual.slice(0, 200)}`], [actual.slice(0, 240)], actual, `Exact match to: ${expected}`, passed ? 1 : 0);
};
const runSubstringRegexVerifier = (card, execution) => {
    const config = card.verifier_config;
    const sourcePath = typeof config.source_field === "string" ? config.source_field : "output.answer_or_action.text";
    const source = sourcePath.startsWith("output.")
        ? getByPath(execution.output, sourcePath.slice("output.".length))
    ...
    const includes = Array.isArray(config.includes)
        ? config.includes.filter((value) => typeof value === "string" && value.trim().length > 0)
        : [];
    const excludes = Array.isArray(config.excludes)
        ? config.excludes.filter((value) => typeof value === "string" && value.trim().length > 0)
        : [];
    const regexes = Array.isArray(config.regex)
        ? config.regex.filter((value) => typeof value === "string" && value.trim().length > 0)
        : [];
    ...
    return finish(passed, passed ? ["Substring/regex checks passed."] : failures, [content.slice(0, 360)], content, [
...
const runJsonFieldVerifier = (card, execution) => {
    ...
    return finish(reasons.length === 0, reasons.length === 0 ? ["JSON field checks passed."] : reasons, [toStringValue(value).slice(0, 360)], value, `JSON field checks on ${fieldPath}.`, reasons.length === 0 ? 1 : 0);
};
const runSourceGroundingVerifier = (card, execution) => {
    const config = card.verifier_config;
    const artifacts = execution.output.artifacts ?? {};
    const sourceIds = Array.isArray(artifacts.source_ids)
        ? artifacts.source_ids
            .filter((entry) => typeof entry === "string")
        : [];
    const requiredSourceIds = Array.isArray(config.required_sources)
        ? config.required_sources.filter((entry) => typeof entry === "string")
        : [];
    const minSourceCount = typeof config.min_source_count === "number" && Number.isFinite(config.min_source_count)
    ...
    return finish(reasons.length === 0, reasons.length === 0 ? ["Grounding source checks passed."] : reasons, sourceIds, sourceIds, `Grounding requires >=${minSourceCount} sources and ids [${requiredSourceIds.join(", ")}].`, reasons.length === 0 ? 1 : 0);
};
const runWorkflowToolSelectionVerifier = (card, execution) => {
    const config = card.verifier_config;
    ...
    const mustInclude = Array.isArray(config.must_include_tools)
        ? config.must_include_tools.filter((entry) => typeof entry === "string")
        : [];
    const forbidden = Array.isArray(config.forbidden_tools)
        ? config.forbidden_tools.filter((entry) => typeof entry === "string")
    ...
    return finish(reasons.length === 0, reasons.length === 0 ? ["Tool/workflow selection checks passed."] : reasons, selected, selected, `Selected tools must include [${mustInclude.join(", ")}] and exclude [${forbidden.join(", ")}].`, reasons.length === 0 ? 1 : 0);
};
const runFileDiffVerifier = (card, execution) => {
    const config = card.verifier_config;
    const artifacts = execution.output.artifacts ?? {};
    const changedFiles = Array.isArray(artifacts.changed_files)
        ? artifacts.changed_files
            .filter((entry) => typeof entry === "string")
        : [];
    const requiredPaths = Array.isArray(config.required_paths)
        ? config.required_paths.filter((entry) => typeof entry === "string")
        : [];
    const forbiddenPaths = Array.isArray(config.forbidden_paths)
        ? config.forbidden_paths.filter((entry) => typeof entry === "string")
        : [];
    const maxChangedFiles = typeof config.max_changed_files === "number" && Number.isFinite(config.max_changed_files)
    ...
    return finish(reasons.length === 0, reasons.length === 0 ? ["File-diff checks passed."] : reasons, changedFiles, changedFiles, "File diff constraints validated.", reasons.length === 0 ? 1 : 0);
};
const runCommandActionSelectionVerifier = (card, execution) => {
    const config = card.verifier_config;
    ...
    const requiredActionIds = Array.isArray(config.required_action_ids)
        ? config.required_action_ids.filter((entry) => typeof entry === "string")
        : [];
    const requiredRegexes = Array.isArray(config.required_command_regex)
        ? config.required_command_regex.filter((entry) => typeof entry === "string")
        : [];
    ...
    return finish(reasons.length === 0, reasons.length === 0 ? ["Command/action checks passed."] : reasons, actions.map((action) => `${action.id}: ${action.commandPreview}`), actions, "Action command selection constraints validated.", reasons.length === 0 ? 1 : 0);
};
const runActionSequenceVerifier = (card, execution) => {
    const config = card.verifier_config;
    const actions = execution.output.answer_or_action.proposed_actions ?? [];
    const observedSequence = actions.map((action) => action.id);
    const requiredSequence = Array.isArray(config.ordered_action_ids)
        ? config.ordered_action_ids.filter((entry) => typeof entry === "string")
        : [];
    ...
    if (requireExactLength && observedSequence.length !== requiredSequence.length) {
        reasons.push(`Observed ${observedSequence.length} actions but expected exactly ${requiredSequence.length}.`);
    }
    return finish(reasons.length === 0, reasons.length === 0 ? ["Action sequence checks passed."] : reasons, observedSequence, observedSequence, `Ordered action ids: ${requiredSequence.join(" -> ")}`, reasons.length === 0 ? 1 : 0);
};
const runActionPreconditionsVerifier = (card, execution) => {
    const config = card.verifier_config;
    const artifacts = execution.output.artifacts ?? {};
    const required = Array.isArray(config.required_preconditions)
        ? config.required_preconditions.filter((entry) => typeof entry === "string")
        : [];
    const observedFromActions = (execution.output.answer_or_action.proposed_actions ?? [])
        .flatMap((action) => action.preconditions ?? [])
        .filter((entry) => typeof entry === "string");
    const artifactPreconditions = Array.isArray(artifacts.preconditions)
        ? artifacts.preconditions.filter((entry) => typeof entry === "string")
        : [];
    ...
    return finish(missing.length === 0, missing.length === 0
        ? ["Action precondition checks passed."]
        : missing.map((entry) => `Missing required precondition: ${entry}`), observed, observed, `Required preconditions: ${required.join(", ")}`, missing.length === 0 ? 1 : 0);
};
const runActionRiskClassVerifier = (card, execution) => {
    const config = card.verifier_config;
    const actions = execution.output.answer_or_action.proposed_actions ?? [];
    const allowed = Array.isArray(config.allowed_risk_classes)
        ? config.allowed_risk_classes.filter((entry) => typeof entry === "string")
        : [];
    const maxRiskClass = typeof config.max_risk_class === "string" && config.max_risk_class.trim().length > 0
    ...
    return finish(reasons.length === 0, reasons.length === 0 ? ["Action risk class checks passed."] : reasons, actions.map((action) => `${action.id}:${actionRiskClass(action)}`), actions, `Allowed risk classes [${allowed.join(", ")}], max=${maxRiskClass ?? "none"}.`, reasons.length === 0 ? 1 : 0);
};
const runActionApprovalTokenVerifier = (card, execution) => {
    const config = card.verifier_config;
    const actions = execution.output.answer_or_action.proposed_actions ?? [];
    const requiredActionIds = Array.isArray(config.required_action_ids)
        ? config.required_action_ids.filter((entry) => typeof entry === "string")
        : [];
    const minimumRisk = typeof config.require_token_for_risk_at_or_above === "string"
    ...
    if (riskyActions.length > 0 && !tokenProvided) {
        reasons.push(`Approval token required for ${minimumRisk}+ actions but no token was provided in request artifacts.`);
    }
    const missingApprovalFlags = riskyActions
        .filter((action) => !action.approvalRequired)
        .map((action) => action.id);
    if (missingApprovalFlags.length > 0) {
        reasons.push(`Risky actions missing approvalRequired flag: ${missingApprovalFlags.join(", ")}`);
    }
    return finish(reasons.length === 0, reasons.length === 0 ? ["Action approval token checks passed."] : reasons, {
        tokenProvided,
        ...
        tokenProvided,
        actions
    }, `Token required for risk >= ${minimumRisk}.`, reasons.length === 0 ? 1 : 0);
};
const runPolicyRefusalVerifier = (card, execution) => {
    const config = card.verifier_config;
    ...
    const requiredFlags = Array.isArray(config.required_flags)
        ? config.required_flags.filter((entry) => typeof entry === "string")
        : [];
    const refusalPhrases = Array.isArray(config.refusal_phrases)
        ? config.refusal_phrases.filter((entry) => typeof entry === "string")
    ...
    return finish(reasons.length === 0, reasons.length === 0 ? ["Policy refusal checks passed."] : reasons, [refusalText.slice(0, 300), `flags: ${execution.output.safety_flags.join(", ")}`], {
        mode: execution.output.answer_or_action.mode,
        text: refusalText,
        safety_flags: execution.output.safety_flags
    }, "Expected safe policy refusal output.", reasons.length === 0 ? 1 : 0);
};
const runGovernanceDecisionVerifier = (card, execution) => {
    const config = card.verifier_config;
    const artifacts = execution.output.artifacts ?? {};
    const expected = (typeof config.expected_governance_decision === "string" && config.expected_governance_decision) ||
        card.expected_governance_decision ||
        "allow";
    const observed = typeof artifacts.governance_decision === "string"
        ? String(artifacts.governance_decision)
        : typeof artifacts.decision === "string"
            ? String(artifacts.decision)
    ...
    return finish(passed, passed
        ...
        : [`Expected governance decision "${expected}" but observed "${observed}".`], [observed], observed, `Governance decision should be "${expected}".`, passed ? 1 : 0);
};
const runExecutorSelectionVerifier = (card, execution) => {
    const config = card.verifier_config;
    const artifacts = execution.output.artifacts ?? {};
    const expected = (typeof config.expected_executor === "string" && config.expected_executor) ||
        card.expected_executor ||
        "";
    const observed = typeof artifacts.executor === "string"
        ? String(artifacts.executor)
    ...
    return finish(passed, passed
        ...
        : [`Expected executor "${expected}" but observed "${observed}".`], [observed], observed, `Executor should be "${expected || "any explicit executor"}".`, passed ? 1 : 0);
};
const runPreflightCheckVerifier = (card, execution) => {
    const config = card.verifier_config;
    const artifacts = execution.output.artifacts ?? {};
    const expected = Array.isArray(config.expected_preflight)
        ? config.expected_preflight.filter((entry) => typeof entry === "string")
        : card.expected_preflight ?? [];
    const observed = Array.isArray(artifacts.preflight_checks)
        ? artifacts.preflight_checks.filter((entry) => typeof entry === "string")
        : [];
    ...
    return finish(missing.length === 0, missing.length === 0
        ...
        : missing.map((entry) => `Missing preflight check: ${entry}`), observed, observed, `Preflight checks should include: ${expected.join(", ")}`, missing.length === 0 ? 1 : 0);
};
const runVerificationStateVerifier = (card, execution) => {
    const config = card.verifier_config;
    const artifacts = execution.output.artifacts ?? {};
    const expected = Array.isArray(config.expected_verification)
        ? config.expected_verification.filter((entry) => typeof entry === "string")
        : card.expected_verification ?? [];
    const observed = Array.isArray(artifacts.verification)
        ? artifacts.verification.map((entry) => toStringValue(entry))
        : typeof artifacts.verification === "string"
            ? [String(artifacts.verification)]
            : [];
    const missing = expected.filter((entry) => !observed.some((candidate) => normalize(candidate) === normalize(entry)));
    return finish(missing.length === 0, missing.length === 0
        ? ["Verification state matched expected values."]
        : missing.map((entry) => `Missing verification state: ${entry}`), observed, observed, `Verification should include: ${expected.join(", ")}`, missing.length === 0 ? 1 : 0);
};
const runApprovalStateVerifier = (card, execution) => {
    const config = card.verifier_config;
    const artifacts = execution.output.artifacts ?? {};
    const expectedTokenRequired = typeof config.require_approval_for_risk_at_or_above === "string"
        ? config.require_approval_for_risk_at_or_above
        : null;
    const approvalState = artifacts.approval_state ?? artifacts.approvalState ?? null;
    const approvalRequired = Boolean(approvalState?.required ?? card.approval_required);
    ...
    return finish(passed, passed
        ...
        : [`Approval token required for risk ${expectedToken ?? "high"} or above but was not provided.`], [JSON.stringify(approvalState ?? {})], approvalState, `Approval state should reflect whether a token was provided for ${expectedToken ?? "approval-gated"} actions.`, passed ? 1 : 0);
};
const runRollbackStateVerifier = (card, execution) => {
    const artifacts = execution.output.artifacts ?? {};
    const rollback = artifacts.rollback ?? artifacts.rollback_plan ?? null;
    ...
    return finish(passed, passed ? ["Rollback state recorded or explicitly marked unavailable."] : ["Rollback state was not recorded."], [JSON.stringify(rollback ?? {})], rollback, "Rollback state should be present for recovery-capable tasks.", passed ? 1 : 0);
};
const runCustomHookVerifier = async (card, execution, workspaceRoot) => {
    const config = card.verifier_config;
    const modulePath = typeof config.module_path === "string" ? config.module_path : "";
    if (!modulePath) {
        return finish(false, ["custom-hook verifier missing module_path"], [], execution.output, "Custom hook verifier expected module path.", 0);
    }
    ...
    try {
        const mod = (await import(pathToFileUrl(absolutePath)));
        const fn = mod[exportName];
        if (typeof fn !== "function") {
            return finish(false, [`custom-hook export "${exportName}" is not a function`], [], execution.output, `Custom verifier function ${exportName}()`, 0);
        }
        ...
        return finish(false, [`custom-hook verifier failed: ${error instanceof Error ? error.message : String(error)}`], [], execution.output, "Custom verifier hook execution", 0);
    }
};
const pathToFileUrl = (absolutePath) => {
    const normalized = absolutePath.replace(/\\/g, "/");
    return normalized.startsWith("/") ? `file://${normalized}` : `file:///${normalized}`;
};
export const verifyCapabilityCard = async (input) => {
    ...
    switch (card.verifier_type) {
        ...
        default:
            return finish(false, [`Unsupported verifier type: ${card.verifier_type}`], [], execution.output, "Unknown verifier configuration.", 0);
    }
};
