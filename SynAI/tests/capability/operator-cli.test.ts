...
const captureJsonOutput = async (argv: string[]): Promise<{ code: number; payload: unknown }> => {
  const logs: string[] = [];
  const spy = vi.spyOn(console, "log").mockImplementation((...entries: unknown[]) => {
...
const writePluginFixture = async (workspaceRoot: string): Promise<void> => {
...
describe("operator cli", () => {
  it("mines governed history into the backlog artifacts", async () => {
    const workspaceRoot = path.join(
      tmpdir(),
      `synai-operator-history-${Date.now()}-${Math.random().toString(16).slice(2)}`
    );
    ...
    try {
      ...
      expect(code).toBe(0);
      const output = payload as { findings: Array<{ recovered_intent: string }>; backlogPath: string };
  ...
  it("approves, enables, and lists runtime plugins", async () => {
    const workspaceRoot = path.join(
      tmpdir(),
      `synai-operator-plugins-${Date.now()}-${Math.random().toString(16).slice(2)}`
    );
    try {
      ...
      const approved = approveResult.payload as { updated: { approved: boolean; approvedBy: string | null } };
      ...
      const enabled = enableResult.payload as { updated: { enabled: boolean } };
      ...
      const listed = listResult.payload as {
        plugins: Array<{ loaded: boolean; manifest: { id: string } }>;
