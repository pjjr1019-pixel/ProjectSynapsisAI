...
import { compactPath, formatBytes, isWithinPath, normalizePathForMatch } from "../files/shared";
import { findControlPanelEntry, findRegistryZoneEntry, findSettingsMapEntry, searchControlPanelEntries, searchRegistryZoneEntries, searchSettingsMapEntries } from "../machine";
import { collectUiHighlights, findFocusedUiElement, formatWindowSummary, summarizeRecentEvents } from "../screen/shared";
import { formatResourceUsageMetric, parseResourceUsageTargets, pickResourceUsageFindings, resourceUsageSelectionTitle } from "./resource-usage";
const MODE_LIMITS = {
    short: 1,
    medium: 2,
    detailed: 4
...
const resourceHotspotDisplayLabel = (resource) => resource === "disk" ? "Disk" : resource.toUpperCase();
...
export const SYSTEM_STATS_QUERY_VARIANTS = SYSTEM_STATS_QUERY_INTROS.flatMap((intro) => SYSTEM_STATS_QUERY_TARGETS.map((target) => `${intro} ${target}`));
const INTENT_LABELS = {
...
const stripPunctuation = (value) => value.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
const uniqueStrings = (values) => [...new Set(values.map((value) => value.trim()).filter(Boolean))];
const limitStrings = (values, limit) => uniqueStrings(values).slice(0, limit);
const formatCpuSeconds = (value) => value != null ? `${value.toFixed(1)}s CPU` : "CPU n/a";
...
const normalizeQuery = (query) => stripPunctuation(query);
const tokenizeNormalized = (value) => value.split(/\s+/).map((token) => token.trim()).filter(Boolean);
const typoBudgetForToken = (token) => {
...
const boundedLevenshtein = (left, right, maxDistance) => {
    if (left === right) {
        return 0;
    }
    if (maxDistance <= 0) {
    ...
    let previousRow = new Array(right.length + 1);
...
const hasTypoTolerantPhraseMatch = (queryTokens, phraseTokens) => {
...
const queryMatchesPhrase = (normalizedQuery, queryTokens, phrase) => {
...
const includesAny = (query, phrases) => {
...
const isDriveCapacityStorageQuery = (query) => {
    const normalized = normalizeQuery(query);
    return includesAny(normalized, DRIVE_CAPACITY_PHRASES) || storageDrivePattern.test(normalized);
};
const isFileStorageQuery = (query) => includesAny(query, FILE_STORAGE_PHRASES);
const isAmbiguousStorageQuery = (query) => {
    const normalized = normalizeQuery(query);
    if (!includesAny(normalized, STORAGE_SURFACE_PHRASES)) {
        return false;
    }
    if (isDriveCapacityStorageQuery(normalized) ||
...
const resolveStorageFamilyFromConversation = (query, conversationContext) => {
    ...
    if (conversationContext.lastAwarenessIntentFamily === "live-usage" ||
        conversationContext.lastAwarenessIntentFamily === "hardware") {
        return "live-usage";
    }
    return null;
};
export const buildStorageClarification = (query, conversationContext) => isAmbiguousStorageQuery(query) && !resolveStorageFamilyFromConversation(query, conversationContext)
    ? {
    ...
    : null;
const extractTopCountFromQuery = (query, fallback = RESOURCE_HOTSPOT_DEFAULT_TOP_N) => {
    const normalized = normalizeQuery(query);
...
const resourceHotspotResourceForQuery = (query) => {
    const normalized = normalizeQuery(query);
    if (includesAny(normalized, ["gpu", "graphics", "graphics card", "video card", "vram", "video memory"]) &&
...
const resourceHotspotGroupingForQuery = (query) => {
    const normalized = normalizeQuery(query);
...
const isSystemStatsVariantQuery = (query) => includesAny(query, SYSTEM_STATS_QUERY_VARIANTS) ||
    includesAny(query, [
        "system stats",
        "system status",
        "system health",
...
const collectSignals = (query, phrases) => {
    const normalized = normalizeQuery(query);
    const queryTokens = tokenizeNormalized(normalized);
    return uniqueStrings(phrases.filter((phrase) => queryMatchesPhrase(normalized, queryTokens, phrase)));
};
const countKeywordHits = (query, phrases) => collectSignals(query, phrases).length;
const clamp = (value, min = 0, max = 1) => Math.max(min, Math.min(max, value));
const confidenceLevelFor = (confidence) => confidence >= 0.85 ? "high" : confidence >= 0.55 ? "medium" : "low";
...
const restampFreshness = (freshness, now = new Date()) => createFreshnessMetadata(freshness.capturedAt, freshness.generatedAt, now);
const makeEvidenceRef = (id, kind, label, pathValue) => ({
    id,
    kind,
    label,
    path: pathValue
});
const trimEvidenceRefs = (refs, limit = 4) => {
    const seen = new Set();
    const trimmed = [];
...
const chooseMode = (query, requestedMode) => {
    if (requestedMode) {
...
const isFeatureSummaryQuery = (query) => {
...
const chooseScopeForQuery = (query) => {
...
const buildSummaryText = (sections, mode) => {
    const limit = MODE_LIMITS[mode];
    return sections.slice(0, limit * 3).join(" | ");
};
const buildCompactSummary = (verifiedFindings, likelyInterpretation, uncertainty, suggestedNextChecks, safeNextAction, mode) => {
    const limit = MODE_LIMITS[mode];
    const parts = [
...
const buildEvidenceBundle = (input) => {
    ...
    return {
        ...
        evidenceRefs,
        affectedAreas: uniqueStrings(input.affectedAreas),
...
const applyOfficialKnowledge = (answer, input, route) => {
    ...
    const officialVerified = officialHits.map((hit) => `Microsoft: ${hit.title} — ${hit.extract}`);
    ...
    const officialEvidenceRefs = officialHits.map((hit) => makeEvidenceRef(hit.canonicalUrl, "official", hit.title, hit.canonicalUrl));
    const versionMatched = officialHits.some((hit) => hit.versionMatched);
    const localFirstFamilies = new Set([
        "live-usage",
        "hardware",
        "resource-hotspot",
        "performance-diagnostic",
    ...
    answer.bundle.officialVerified = limitStrings([...(answer.bundle.officialVerified ?? []), ...officialVerified], MODE_LIMITS[answer.mode] + 2);
    ...
    answer.bundle.evidenceTraceIds = uniqueStrings(answer.bundle.evidenceRefs.map((ref) => `${ref.kind}:${ref.id}`));
    if (!versionMatched) {
    ...
    if (!localFirstFamilies.has(route.family)) {
        if (answer.bundle.verifiedFindings.length === 0) {
            answer.bundle.verifiedFindings = limitStrings(officialVerified, MODE_LIMITS[answer.mode] + 1);
        }
        else if (officialVerified[0]) {
            answer.bundle.likelyInterpretation = limitStrings([...answer.bundle.likelyInterpretation, officialVerified[0]], MODE_LIMITS[answer.mode] + 2);
        }
    }
    else if (officialVerified[0]) {
        answer.bundle.likelyInterpretation = limitStrings([...answer.bundle.likelyInterpretation, `Microsoft documentation: ${officialHits[0].extract}`], MODE_LIMITS[answer.mode] + 2);
    }
    answer.bundle.compactSummary = buildCompactSummary(answer.bundle.verifiedFindings.length > 0
        ? answer.bundle.verifiedFindings
        : answer.bundle.officialVerified, answer.bundle.likelyInterpretation, answer.bundle.uncertainty, answer.bundle.suggestedNextChecks, answer.bundle.safeNextAction, answer.mode);
...
const buildClarificationAnswer = (input, route, clarification) => {
...
const buildIntentSecondaryNotes = (plan, current) => {
...
const buildCorrelationHighlights = (machine) => (machine.correlationGraph?.processLinks ?? [])
    .slice(0, 3)
    .map((entry) => entry.description)
    .filter(Boolean);
const describeDriveFreeSpace = (drive) => {
    if (!drive) {
...
const buildAwarenessAnswerCard = (answer) => {
    ...
    if (answer.intent.family === "live-usage") {
        ...
        return {
            ...
            metrics: selection.mode === "focused" && focusedLines.length > 0
    ...
    if (answer.bundle.resourceHotspots && answer.bundle.resourceHotspots.length > 0) {
        ...
        return {
            ...
            title: `${topHotspot.label} is the top ${topHotspot.resource.toUpperCase()} ${topHotspot.grouping === "program" ? "program" : "process"}`,
...
const scoreIntent = (query, family) => {
...
export const routeAwarenessIntent = (query, conversationContext) => {
    ...
    const scored = Object.keys(INTENT_KEYWORDS).map((family) => {
    ...
    const bareStartupSignals = normalized.includes("startup") &&
        !includesAny(normalized, [
            "windows",
            "startup apps",
            "app",
    ...
    const bareResourceComboQuery = resourceUsageSelection.mode === "focused" &&
    ...
    if ((systemStatsVariant || liveUsageSpecificSignals) &&
        liveUsageCandidate &&
...
const fileRootsByName = (snapshot, query) => {
    const normalized = normalizeQuery(query);
...
const filterChangesByPath = (changes, folderPath, onlyToday = false, now = new Date()) => {
    const normalized = normalizePathForMatch(folderPath);
    const today = now.toDateString();
    return changes.filter((change) => {
        const inFolder = isWithinPath(change.path, normalized) ||
...
const findMatchingFileKinds = (query) => {
    const normalized = normalizeQuery(query);
    const kinds = [];
...
const findLikelyProcessMatches = (snapshot, query) => {
    const normalized = normalizeQuery(query);
    const scored = snapshot.processSnapshot.processes
        ...
        .sort((left, right) => right.score - left.score || (right.process.memoryBytes ?? 0) - (left.process.memoryBytes ?? 0));
    return scored.slice(0, 3).map((item) => item.process);
};
const relatedServicesForProcess = (machine, process) => {
    const tokens = uniqueStrings([process.name, process.executablePath, process.windowTitle, process.publisher].filter((value) => Boolean(value))).map((value) => normalizeQuery(value));
    const related = machine.serviceSnapshot.services
        .filter((service) => {
        if (service.linkedProcessId === process.pid) {
            return true;
        }
        const haystack = normalizeQuery([service.serviceName, service.displayName, service.executablePath, service.account].filter(Boolean).join(" "));
...
const linkedProcessesForService = (machine, serviceName) => {
    const normalized = normalizeQuery(serviceName);
    const service = machine.serviceSnapshot.services.find((candidate) => normalizeQuery([candidate.serviceName, candidate.displayName].filter(Boolean).join(" ")).includes(normalized));
    return service?.linkedProcessId ? [service.linkedProcessId] : [];
};
const summarizeProcessFocus = (current, machine) => {
    const memory = current.memoryBytes != null ? formatBytes(current.memoryBytes) : "n/a";
    const cpu = current.cpuPercent != null
...
const findLikelyInstalledAppForProcess = (machine, process) => {
    const exactMatch = machine.installedAppsSnapshot.apps.find((app) => app.associatedProcessIds.includes(process.pid) || app.associatedProcessNames.includes(process.name));
    if (exactMatch) {
        return exactMatch;
    }
    const processTokens = uniqueStrings([process.name, process.executablePath, process.commandLine, process.windowTitle, process.publisher, process.signer]
        .filter((value) => Boolean(value))
        .flatMap((value) => tokenizeNormalized(normalizeQuery(value)))
        .filter((token) => token.length > 2));
    const scored = machine.installedAppsSnapshot.apps
        .map((app) => {
        const haystack = normalizeQuery([app.name, app.publisher, app.installLocation, app.displayIcon, app.uninstallCommand, app.quietUninstallCommand]
            .filter(Boolean)
            .join(" "));
        const score = (app.publisher && process.publisher && normalizeQuery(app.publisher) === normalizeQuery(process.publisher) ? 3 : 0) +
...
const formatHotspotAmount = (resource, value, options) => {
    switch (resource) {
        case "ram":
...
const buildResourceHotspotEntries = (input) => {
    ...
    const totalResourceValue = resource === "ram"
    ...
    for (const entry of processSignals) {
        ...
        const app = entry.app;
        const groupKey = grouping === "program" && app
    ...
    const entries = [...grouped.values()]
        ...
        .slice(0, topN)
        .map((entry, index) => {
        const resourceShare = totalResourceValue > 0 ? clamp(entry.resourceValue / totalResourceValue) : null;
...
const describeUiElement = (element) => {
    if (!element) {
        return null;
    }
...
const summarizeFileAwareness = (snapshot, query, mode, now = new Date()) => {
    if (!snapshot) {
        return {
            verifiedFindings: [],
            likelyInterpretation: [],
    ...
    if (folderSummary) {
        verifiedFindings.push(`${path.basename(folderSummary.path) || compactPath(folderSummary.path)} contains ${folderSummary.fileCount} files and ${formatBytes(folderSummary.totalSizeBytes)} total`);
        if (folderSummary.recentChangeCount > 0) {
            verifiedFindings.push(`${path.basename(folderSummary.path) || compactPath(folderSummary.path)} has ${folderSummary.recentChangeCount} recent changes`);
        }
    ...
    if (wantsLargest) {
        ...
        if (largest.length > 0) {
            verifiedFindings.push(`Largest files: ${largest
    ...
    if (wantsNewest) {
        const newest = [...snapshot.files]
            ...
            .slice(0, mode === "short" ? 2 : 4);
        if (newest.length > 0) {
            verifiedFindings.push(`Newest files: ${newest
                .slice(0, MODE_LIMITS[mode])
    ...
    if (wantsChanges) {
        ...
        if (changes.length > 0) {
            verifiedFindings.push(`Recent changes: ${changes
    ...
    if (mediaKinds.length > 0) {
        for (const kind of mediaKinds) {
            const matches = snapshot.media
                ...
                .slice(0, MODE_LIMITS[mode] + 1);
            if (matches.length > 0) {
                verifiedFindings.push(`${kind} files: ${matches
    ...
    if (folderSummary) {
        likelyInterpretation.push(`The folder hotspot appears to be ${path.basename(folderSummary.path) || compactPath(folderSummary.path)}, with ${folderSummary.recentChangeCount} recent changes and ${formatBytes(folderSummary.totalSizeBytes)} total.`);
...
const searchFileMatches = (snapshot, query, limit) => snapshot.files
    .filter((entry) => entry.privacyScope === "public metadata" || entry.privacyScope === "user-visible local content")
    .filter((entry) => {
    ...
    .sort((left, right) => (right.sizeBytes ?? 0) - (left.sizeBytes ?? 0))
    .slice(0, limit);
const searchFolderMatches = (snapshot, query, limit) => snapshot.folders
    .filter((entry) => entry.privacyScope === "public metadata" || entry.privacyScope === "user-visible local content")
    .filter((entry) => {
    ...
    .slice(0, limit);
const searchMediaMatches = (snapshot, query, limit) => snapshot.media
    .filter((entry) => entry.privacyScope === "public metadata" || entry.privacyScope === "user-visible local content")
    .filter((entry) => {
...
const sortEventLogEntries = (entries) => [...entries].sort((left, right) => {
...
const summarizeEventLogEntry = (entry, includeMessage = false) => {
...
const eventLogProviderSummary = (entries, limit = 4) => {
...
const buildEventLogDiagnostics = (input) => {
...
const ANOMALY_SEVERITY_WEIGHT = {
    low: 1,
    medium: 2,
    high: 3
};
const sortAnomalyFindings = (findings) => [...findings].sort((left, right) => ANOMALY_SEVERITY_WEIGHT[right.severity] - ANOMALY_SEVERITY_WEIGHT[left.severity] ||
    right.confidence - left.confidence);
const buildAnomalyFindings = (machine) => {
    const findings = [];
    const topProcess = [...machine.processSnapshot.processes].sort((left, right) => (right.memoryBytes ?? 0) - (left.memoryBytes ?? 0) || (right.cpuSeconds ?? 0) - (left.cpuSeconds ?? 0))[0];
    ...
    const providerWithRecurringSevere = eventLogProviderSummary(severeEvents, 3).find((provider) => provider.errorCount >= 2);
...
const buildAnomalyDiagnostics = (input) => {
    const machine = input.current.machineAwareness;
    const findings = buildAnomalyFindings(machine);
    const verifiedFindings = findings.length > 0
    ...
    const likelyInterpretation = findings.length > 0
        ? [
    ...
    return {
        ...
        summary: findings.length > 0
            ? buildSummaryText(findings
                .slice(0, 3)
                .map((finding) => `${finding.severity.toUpperCase()}: ${finding.title}`), "medium")
            : "No strong anomaly pattern was detected in the current machine snapshot.",
        verifiedFindings,
        likelyInterpretation,
        uncertainty,
...
const buildPerformanceDiagnostics = (input) => {
    const machine = input.current.machineAwareness;
    const eventLogs = machine.eventLogSnapshot;
    ...
    const availableMemoryRaw = machine.systemIdentity.hardware.memory.availableBytes ?? machine.systemIdentity.hardware.memory.freeBytes ?? null;
    ...
    const verifiedFindings = [
        ...
        ...topProcesses.slice(0, MODE_LIMITS.medium).map((entry) => summarizeProcessFocus(machine.processSnapshot.processes.find((process) => process.pid === entry.pid), machine))
    ];
    ...
    if (eventLogs.totalCount > 0) {
        verifiedFindings.push(`Event logs: ${severeEventCount} severe entries in ${eventLogs.totalCount} recent event-log entries`);
    }
    if (anomalyHints.length > 0) {
        verifiedFindings.push(`Anomaly hints: ${anomalyHints
    ...
    if (topCpuProcess) {
        verifiedFindings.push(`CPU hotspot: ${topCpuProcess.name}#${topCpuProcess.pid} (${formatCpuSeconds(topCpuProcess.cpuSeconds)})`);
    }
    const confidence = clamp(0.55 +
        Math.min(0.35, topProcesses.length * 0.05 +
...
const buildStartupDiagnostics = (input) => {
...
const buildStorageDiagnostics = (input) => {
    const snapshot = input.current.fileAwarenessSnapshot;
    const freshness = restampFreshness(snapshot?.freshness ?? input.current.fileAwarenessSummary?.freshness ?? input.current.digest.freshness, input.now);
    if (!snapshot) {
...
const buildCurrentUiDiagnostics = (input) => {
...
const formatPowerState = (value) => value == null ? "unknown" : value ? "on" : "off";
const buildSettingsControlRegistryAnswer = (input) => {
...
const buildLiveUsageAnswer = (input) => {
    const query = input.query;
    const route = input.route ?? routeAwarenessIntent(query);
    const scope = input.scope ?? "current-machine";
    const mode = chooseMode(query, input.mode);
    ...
    const usedMemoryPercent = usedMemory != null && hardware.memory.totalBytes > 0
        ? Math.round((usedMemory / hardware.memory.totalBytes) * 100)
        : null;
    const cpuLine = hardware.cpuLoadPercent != null
        ? `Current CPU load: ${hardware.cpuLoadPercent}%`
        : `CPU load: ${hardware.cpu.name} is available, but live CPU load was not captured.`;
    const ramLine = `Current RAM: ${usedMemory != null ? `${formatBytes(usedMemory)} used` : "usage unavailable"} of ${formatBytes(hardware.memory.totalBytes)}${usedMemoryPercent != null ? ` (${usedMemoryPercent}%)` : ""}${availableMemory != null ? ` | available ${formatBytes(availableMemory)}` : ""}`;
    const gpuLine = primaryGpu
        ? `GPU: ${primaryGpu.name}${primaryGpu.loadPercent != null ? ` | live load ${primaryGpu.loadPercent}%` : " | live load unavailable"}${primaryGpu.memoryBytes != null ? ` | VRAM ${formatBytes(primaryGpu.memoryBytes)}` : ""}`
        : "GPU: no GPU details were captured.";
    const diskLine = selection.mode === "focused" && selection.targets.includes("disk")
        ? describeDriveFreeSpace(primaryDrive)
        : primaryDrive
            ? `Disk: ${primaryDrive.deviceId}${primaryDrive.volumeLabel ? ` (${primaryDrive.volumeLabel})` : ""}${primaryDrive.totalBytes != null
...
const buildHardwareAnswer = (input) => {
    const query = input.query;
    const route = input.route ?? routeAwarenessIntent(query);
    const scope = input.scope ?? "current-machine";
    const mode = chooseMode(query, input.mode);
    ...
    const usedMemoryPercent = usedMemory != null && hardware.memory.totalBytes > 0
        ? Math.round((usedMemory / hardware.memory.totalBytes) * 100)
        : null;
    const ramUsageLine = `${formatBytes(hardware.memory.totalBytes)} RAM${usedMemory != null ? ` | used ${formatBytes(usedMemory)}${usedMemoryPercent != null ? ` (${usedMemoryPercent}%)` : ""}` : ""}${availableMemory != null ? ` | available ${formatBytes(availableMemory)}` : ""}`;
    const gpuDetails = hardware.gpus.length > 0
        ? hardware.gpus.slice(0, MODE_LIMITS[mode] + 1).map((gpu) => `${gpu.name}${gpu.loadPercent != null ? ` | live load ${gpu.loadPercent}%` : ""}${gpu.memoryBytes != null ? ` | VRAM ${formatBytes(gpu.memoryBytes)}` : ""}${gpu.driverVersion ? ` | driver ${gpu.driverVersion}` : ""}`)
        : ["No GPU summary was captured."];
    const windowsLine = `${identity.machineName} | ${identity.windowsEdition ?? identity.windowsVersion ?? "Windows"}${identity.windowsBuild ? ` build ${identity.windowsBuild}` : ""} | ${identity.architecture}`;
    const cpuLine = `${hardware.cpu.name} (${hardware.cpu.cores} cores / ${hardware.cpu.logicalCores} threads${hardware.cpu.speedMHz != null ? ` @ ${hardware.cpu.speedMHz}MHz` : ""})${hardware.cpuLoadPercent != null ? ` | live load ${hardware.cpuLoadPercent}%` : ""}`;
    const uptimeLine = `Uptime: ${formatUptime(identity.uptimeSeconds)}`;
    const topProcess = [...current.machineAwareness.processSnapshot.processes].sort((left, right) => (right.memoryBytes ?? 0) - (left.memoryBytes ?? 0) || (right.cpuSeconds ?? 0) - (left.cpuSeconds ?? 0))[0] ?? null;
    const topProcessLine = topProcess
        ? `Top process: ${topProcess.name}#${topProcess.pid} (${topProcess.memoryBytes != null ? formatBytes(topProcess.memoryBytes) : "mem n/a"}, ${formatCpuSeconds(topProcess.cpuSeconds)})`
        : "Top process: none";
    const gpuLine = hardware.gpus.length > 0 ? `GPU: ${gpuDetails.join(" | ")}` : "No GPU summary was captured.";
    const driveLine = hardware.drives.length > 0
    ...
    const prioritizedVerifiedFindings = selection.mode === "focused" && focusedResourceLines.length > 0
    ...
    const likelyInterpretation = selection.mode === "focused"
        ? []
        : [
    ...
    const uncertainty = selection.mode === "focused"
        ? []
        : [
...
const buildProcessServiceStartupAnswer = (input) => {
    const query = input.query;
    const route = input.route ?? routeAwarenessIntent(query);
    const scope = input.scope ?? "current-machine";
    const mode = chooseMode(query, input.mode);
    ...
    const serviceMatches = machine.serviceSnapshot.services
        .filter((service) => includesAny(query, [service.serviceName, service.displayName, service.executablePath ?? "", service.account ?? ""]))
    ...
    if (serviceMatches.length > 0) {
        verifiedFindings.push(`Service matches: ${serviceMatches
            .map((service) => `${service.displayName} [${service.state}, ${service.startupType}]`)
            .join(" | ")}`);
        const servicePaths = serviceMatches
            .map((service) => service.executablePath ? `${service.displayName || service.serviceName} -> ${service.executablePath}` : null)
            .filter((value) => Boolean(value));
        if (servicePaths.length > 0) {
            verifiedFindings.push(`Service executable paths: ${servicePaths.join(" | ")}`);
        }
        evidenceRefs.push(...serviceMatches.slice(0, 2).map((service) => makeEvidenceRef(service.serviceName, "api", `service-${service.serviceName}`)));
    }
    if (startupMatches.length > 0) {
        verifiedFindings.push(`Startup matches: ${startupMatches.map((entry) => `${entry.name} (${entry.source})`).join(" | ")}`);
        evidenceRefs.push(...startupMatches.slice(0, 2).map((entry) => makeEvidenceRef(entry.location, "api", `startup-${entry.source}`, entry.location)));
    }
...
function buildFileFolderMediaAnswer(input) {
    const mode = chooseMode(input.query, input.mode);
...
const buildPerformanceAnswer = (input) => {
    ...
    const prioritizedVerifiedFindings = eventFocused
        ...
        : cpuFocused
            ? [
                ...
                ...diagnostics.verifiedFindings.filter((line) => !line.toLowerCase().includes("cpu load") && !line.toLowerCase().includes("cpu hotspot"))
            ]
            : ramFocused
                ? [
...
const buildResourceHotspotAnswer = (input) => {
    const query = input.query;
    const route = input.route ?? routeAwarenessIntent(query);
    const scope = input.scope ?? "current-machine";
    const mode = chooseMode(query, input.mode);
    ...
    const currentResourceLine = resource === "ram"
        ? (() => {
            ...
            const usedMemory = availableMemory != null ? Math.max(0, machine.systemIdentity.hardware.memory.totalBytes - availableMemory) : null;
            const usedMemoryPercent = usedMemory != null && machine.systemIdentity.hardware.memory.totalBytes > 0
    ...
    if (hotspot.entries.length > 0) {
        ...
        verifiedFindings.push(`Top ${resourceLabel} ${grouping}: ${top.label} using ${top.resourceAmount}${top.resourceShare != null ? ` (${Math.round(top.resourceShare * 100)}% of ${shareLabel})` : ""}.`);
        verifiedFindings.push(...hotspot.entries.map((entry) => `${entry.rank}. ${entry.label} — ${entry.resourceAmount}${entry.resourceShare != null ? ` (${Math.round(entry.resourceShare * 100)}%)` : ""} | ${entry.description}`));
        ...
        evidenceRefs.push(...hotspot.entries.slice(0, 2).map((entry) => makeEvidenceRef(entry.pid != null ? String(entry.pid) : `${entry.label}-${entry.rank}`, "api", `${resource}-hotspot-${entry.rank}`, entry.executablePath ?? undefined)));
    }
    else if (resource === "gpu") {
        const gpu = machine.systemIdentity.hardware.gpus[0] ?? null;
        if (gpu) {
            verifiedFindings.push(`GPU device: ${gpu.name}${gpu.loadPercent != null ? ` | live load ${gpu.loadPercent}%` : ""}${gpu.memoryBytes != null ? ` | VRAM ${formatBytes(gpu.memoryBytes)}` : ""}`);
        }
        else {
    ...
    const confidence = clamp(hotspot.entries.length > 0
...
const describeRepoWorkingTreeEntry = (entry) => {
...
const buildWorkingTreeSummary = (repo, mode) => {
...
const repoEntryKey = (entry) => `${entry.previousPath ?? ""}->${entry.path}|${entry.kind}|${entry.stagedStatus ?? ""}|${entry.unstagedStatus ?? ""}`;
const compareWorkingTree = (currentRepo, referenceRepo) => {
...
const appendRepoComparison = (input) => {
    if (input.referenceRepo.headSha !== input.currentRepo.headSha) {
        input.verifiedFindings.push(`${input.referenceLabel} HEAD: ${input.referenceRepo.headSha?.slice(0, 8) ?? "none"} -> ${input.currentRepo.headSha?.slice(0, 8) ?? "none"}`);
    }
    if (input.referenceRepo.dirtyState !== input.currentRepo.dirtyState) {
        input.verifiedFindings.push(`${input.referenceLabel} dirty state: ${input.referenceRepo.dirtyState} -> ${input.currentRepo.dirtyState}`);
    }
    const delta = compareWorkingTree(input.currentRepo, input.referenceRepo);
    if (delta.introduced.length > 0) {
        input.verifiedFindings.push(`${delta.introduced.length} repo change(s) appeared since ${input.referenceLabel.toLowerCase()}: ${delta.introduced
            .slice(0, MODE_LIMITS[input.mode] + 1)
            .map(describeRepoWorkingTreeEntry)
            .join(" | ")}`);
    }
    if (delta.cleared.length > 0) {
        input.verifiedFindings.push(`${delta.cleared.length} repo change(s) cleared since ${input.referenceLabel.toLowerCase()}: ${delta.cleared
            .slice(0, MODE_LIMITS[input.mode] + 1)
            .map(describeRepoWorkingTreeEntry)
            .join(" | ")}`);
    }
...
const buildFileSummaryDelta = (currentSummary, referenceSummary, referenceLabel) => {
    if (!currentSummary || !referenceSummary) {
        return [];
    }
    const deltas = [];
    if (currentSummary.counts.recentChanges !== referenceSummary.counts.recentChanges) {
        deltas.push(`Recent file changes since ${referenceLabel.toLowerCase()}: ${referenceSummary.counts.recentChanges} -> ${currentSummary.counts.recentChanges}`);
    }
    ...
    if (currentSummary.changeCounts.renamed !== referenceSummary.changeCounts.renamed) {
        deltas.push(`Renamed files since ${referenceLabel.toLowerCase()}: ${referenceSummary.changeCounts.renamed} -> ${currentSummary.changeCounts.renamed}`);
    }
    return deltas;
};
...
const findChangelogPaths = (repo, limit) => uniqueStrings(repo.workingTree.entries
    .map((entry) => entry.path)
    .filter((filePath) => CHANGELOG_HINT_PATTERNS.some((pattern) => normalizeQuery(filePath).includes(pattern)))).slice(0, limit);
const summarizeChangelogHighlights = (repoRoot, mode) => {
    ...
    for (const changelogPath of candidatePaths) {
        ...
        const highlights = orderedSections
            .flatMap((section) => (recentBuckets.get(section) ?? []).slice(0, 2).map((bullet) => `${section}: ${bullet}`))
        ...
        const phaseHistoryBuckets = new Map();
...
const buildCommitDeltaLines = (currentRepo, referenceRepo, referenceLabel, mode) => {
...
const buildRepoChangeAnswer = (input) => {
    const query = input.query;
    const route = input.route ?? routeAwarenessIntent(query);
    const mode = chooseMode(query, input.mode);
    ...
    if (scope === "session") {
        ...
        if (currentFile) {
            ...
            evidenceRefs.push(makeEvidenceRef(input.paths.fileLatestSummaryPath ?? input.paths.fileCurrentCatalogPath ?? input.paths.currentSessionPath, "file", "file-summary", input.paths.fileLatestSummaryPath));
        }
    ...
    if (scope === "last-report") {
        ...
        if (lastReport) {
            ...
            if (lastReportSnapshot) {
                verifiedFindings.push(`Last reported repo: ${lastReportSnapshot.repo.branch ?? "no-git"}${lastReportSnapshot.repo.headSha ? ` @ ${lastReportSnapshot.repo.headSha.slice(0, 8)}` : ""} (${lastReportSnapshot.repo.dirtyState})`);
    ...
    if (scope === "current-machine") {
        ...
        verifiedFindings.push(`${current.machineAwareness.summary.counts.processes} processes, ${current.machineAwareness.summary.counts.services} services, ${current.machineAwareness.summary.counts.startupEntries} startup items, ${current.machineAwareness.summary.counts.eventLogErrors} event-log errors`);
    ...
    if (scope === "session" || scope === "previous-session" || scope === "last-report") {
        ...
        if (currentFile) {
            evidenceRefs.push(makeEvidenceRef(input.paths.fileCurrentCatalogPath ?? input.paths.currentSessionPath, "file", "file-catalog", input.paths.fileCurrentCatalogPath));
        }
        if (currentScreen) {
            evidenceRefs.push(makeEvidenceRef(input.paths.screenCurrentPath ?? input.paths.currentSessionPath, "screen", "screen-snapshot", input.paths.screenCurrentPath));
        }
        if (scope === "last-report" && input.paths.lastReportedSnapshotPath) {
            evidenceRefs.push(makeEvidenceRef(input.paths.lastReportedSnapshotPath, "session", "last-reported-snapshot", input.paths.lastReportedSnapshotPath));
        }
    }
    if (verifiedFindings.length === 0) {
    ...
    const bundle = buildEvidenceBundle({
        freshness: scope === "current-machine"
        ...
        uncertainty,
        suggestedNextChecks,
        safeNextAction: scope === "current-machine"
        ...
        evidenceRefs,
        affectedAreas: scope === "current-machine"
            ? ["machine"]
            : scope === "session"
...
const buildOnScreenAnswer = (input) => {
    const query = input.query;
    const route = input.route ?? routeAwarenessIntent(query);
    ...
    if (screen?.assistMode.enabled) {
        ...
        likelyInterpretation.push(screen.uiTree?.focusedElement
...
export const buildAwarenessQueryAnswer = (input) => {
    ...
    if (secondaryNotes.length > 0) {
        answer.bundle.likelyInterpretation = limitStrings([...answer.bundle.likelyInterpretation, ...secondaryNotes], MODE_LIMITS[answer.mode] + 3);
    }
    answer.bundle.compactSummary = buildCompactSummary(answer.bundle.verifiedFindings, answer.bundle.likelyInterpretation, answer.bundle.uncertainty, answer.bundle.suggestedNextChecks, answer.bundle.safeNextAction, answer.mode);
    answer.summary = `${answer.intent.label}: ${answer.bundle.compactSummary}`;
    if (input.scanTimedOut) {
        ...
        if (!answer.bundle.uncertainty.includes(timeoutNote)) {
            answer.bundle.uncertainty = limitStrings([...answer.bundle.uncertainty, timeoutNote], MODE_LIMITS[answer.mode] + 2);
            answer.bundle.compactSummary = buildCompactSummary(answer.bundle.verifiedFindings, answer.bundle.likelyInterpretation, answer.bundle.uncertainty, answer.bundle.suggestedNextChecks, answer.bundle.safeNextAction, answer.mode);
            answer.summary = `${answer.intent.label}: ${answer.bundle.compactSummary}`;
        }
    }
...
const buildCurrentViewForSummary = (current) => ({
...
const buildSessionSummary = (input) => {
...
const buildPreviousSessionSummary = (input) => {
    const mode = input.mode ?? "medium";
    const previous = input.previous;
...
const buildLastReportSummary = (input) => {
    const mode = input.mode ?? "medium";
...
const buildCurrentMachineSummary = (input) => {
    const mode = input.mode ?? "medium";
    const current = input.current;
    const machine = current.machineAwareness;
    const verifiedFindings = [
...
export const buildAwarenessScopeSummary = (input) => {
    switch (input.scope) {
        case "session":
...
export const buildAwarenessPerformanceDiagnostic = (input) => buildPerformanceDiagnostics(input);
export const buildAwarenessStartupDiagnostic = (input) => buildStartupDiagnostics(input);
export const buildAwarenessStorageDiagnostic = (input) => buildStorageDiagnostics(input);
export const buildAwarenessCurrentUiDiagnostic = (input) => buildCurrentUiDiagnostics(input);
export const buildAwarenessEventLogDiagnostic = (input) => buildEventLogDiagnostics(input);
export const buildAwarenessAnomalyDiagnostic = (input) => buildAnomalyDiagnostics(input);
export const buildAwarenessQueryContextSection = (answer, latestUserMessage, awarenessMode = "observe") => {
    if (!answer) {
        return null;
    }
    ...
    const includeOperatorGuidance = answer.bundle.verifiedFindings.length === 0 || answer.bundle.groundingStatus === "weak";
    const lines = [
