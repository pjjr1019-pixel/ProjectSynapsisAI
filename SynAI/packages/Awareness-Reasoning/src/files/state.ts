...
import type { FileAwarenessSnapshot } from "../contracts/awareness";
import {
...
const overlayVolumeState = (
  snapshot: FileAwarenessSnapshot,
  volumeState: Awaited<ReturnType<typeof buildVolumeAwarenessState>>
...
export const initializeFileAwareness = async (
  ...
  const catalogIsFresh =
    catalogAgeMs < FILE_FRESHNESS_WINDOW_MS &&
    currentOnDisk !== null &&
    (currentOnDisk.volumes?.length ?? 0) > 0;

  let currentSnapshot: FileAwarenessSnapshot;
