...
const SETTINGS_MAP_ENTRIES: SettingsMapEntry[] = [
  {
    category: "display",
    label: "Display",
  ...
  },
  {
    category: "startup",
    label: "Startup apps",
    aliases: ["startup", "boot apps", "launch on login"],
  ...
  },
  {
    ...
    aliases: ["bluetooth", "devices", "peripherals"],
...
const CONTROL_PANEL_ENTRIES: ControlPanelEntry[] = [
  {
    category: "system",
    label: "System",
  ...
  },
  {
    category: "services",
    label: "Services",
    aliases: ["service manager", "services.msc"],
...
const REGISTRY_ZONE_ENTRIES: RegistryZoneEntry[] = [
  {
    category: "startup",
    hive: "HKCU/HKLM",
...
const nowFreshness = (capturedAt: string, now: Date): ReturnType<typeof createFreshnessMetadata> =>
  createFreshnessMetadata(capturedAt, capturedAt, now);

const cloneSettingsEntry = (entry: SettingsMapEntry): SettingsMapEntry => ({
  ...entry,
  aliases: [...entry.aliases],
  relatedSettings: [...entry.relatedSettings]
});

const cloneControlPanelEntry = (entry: ControlPanelEntry): ControlPanelEntry => ({
  ...entry,
  aliases: [...entry.aliases],
  relatedPanels: [...entry.relatedPanels]
});

const cloneRegistryZoneEntry = (entry: RegistryZoneEntry): RegistryZoneEntry => ({
  ...entry,
  aliases: [...entry.aliases],
...
const entryMatchScore = (haystack: string, query: string): number => {
...
const pickBestEntries = <T>(query: string, entries: T[], scorer: (entry: T, query: string) => number): T[] =>
  [...entries]
...
const findExactEntry = <T>(query: string, entries: T[], termsForEntry: (entry: T) => string[]): T | null => {
  const normalizedQuery = normalizeQuery(query);
  if (!normalizedQuery) {
    return null;
  }
...
const scoreSettingsEntry = (entry: SettingsMapEntry, query: string): number =>
  entryMatchScore(
    [entry.category, entry.label, ...entry.aliases, entry.launchTarget, ...entry.relatedSettings, entry.description].join(" "),
    query
  );

const scoreControlPanelEntry = (entry: ControlPanelEntry, query: string): number =>
  entryMatchScore(
    [entry.category, entry.label, ...entry.aliases, entry.launchTarget, ...entry.relatedPanels, entry.description].join(" "),
    query
  );

const scoreRegistryZoneEntry = (entry: RegistryZoneEntry, query: string): number =>
  entryMatchScore(
    [entry.category, entry.hive, entry.path, ...entry.aliases, ...entry.valueNames, ...entry.notes].join(" "),
    query
  );

export const buildSettingsMap = (now = new Date()): SettingsMap => {
...
export const buildControlPanelMap = (now = new Date()): ControlPanelMap => {
  const capturedAt = now.toISOString();
  return {
    capturedAt,
    freshness: nowFreshness(capturedAt, now),
...
export const buildRegistryZoneMap = (now = new Date()): RegistryZoneMap => {
  const capturedAt = now.toISOString();
  return {
    capturedAt,
    freshness: nowFreshness(capturedAt, now),
...
export const findSettingsMapEntry = (query: string): SettingsMapEntry | null =>
  findExactEntry(query, SETTINGS_MAP_ENTRIES, (entry) => [entry.label, entry.category, ...entry.aliases]) ??
  pickBestEntries(query, SETTINGS_MAP_ENTRIES, scoreSettingsEntry)[0] ??
  null;

export const findControlPanelEntry = (query: string): ControlPanelEntry | null =>
  findExactEntry(query, CONTROL_PANEL_ENTRIES, (entry) => [entry.label, entry.category, ...entry.aliases]) ??
  pickBestEntries(query, CONTROL_PANEL_ENTRIES, scoreControlPanelEntry)[0] ??
  null;

export const findRegistryZoneEntry = (query: string): RegistryZoneEntry | null =>
  findExactEntry(query, REGISTRY_ZONE_ENTRIES, (entry) => [entry.category, entry.hive, entry.path, ...entry.aliases]) ??
  pickBestEntries(query, REGISTRY_ZONE_ENTRIES, scoreRegistryZoneEntry)[0] ??
  null;

export const searchSettingsMapEntries = (query: string, limit = 3): SettingsMapEntry[] =>
  pickBestEntries(query, SETTINGS_MAP_ENTRIES, scoreSettingsEntry)
    .slice(0, limit)
    .map(cloneSettingsEntry);

export const searchControlPanelEntries = (query: string, limit = 3): ControlPanelEntry[] =>
  pickBestEntries(query, CONTROL_PANEL_ENTRIES, scoreControlPanelEntry)
    .slice(0, limit)
    .map(cloneControlPanelEntry);

export const searchRegistryZoneEntries = (query: string, limit = 3): RegistryZoneEntry[] =>
  pickBestEntries(query, REGISTRY_ZONE_ENTRIES, scoreRegistryZoneEntry)
    .slice(0, limit)
    .map(cloneRegistryZoneEntry);

export const toMapSummary = (entry: SettingsMapEntry): string =>
  `${entry.label} -> ${entry.launchTarget}`;

export const toControlPanelSummary = (entry: ControlPanelEntry): string =>
  `${entry.label} -> ${entry.launchTarget}`;

export const toRegistryZoneSummary = (entry: RegistryZoneEntry): string =>
  `${entry.category}: ${entry.hive}${entry.path}`;
